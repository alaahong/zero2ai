...
whether the workspace's zero2ai session directory already holds a JSONL transcript.
"""
...
def test_build_extra_env_stages_agent_home(tmp_path: Path, settings: Settings, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    omp_agent_dir = stage_home / ".zero2ai" / "agent"
    ...
    assert (agent_home / ".zero2ai" / "agent" / "models.yml").is_file()
    ...
    assert (agent_home / ".zero2ai" / "agent").stat().st_mode & 0o777 == 0o755
    assert (agent_home / ".zero2ai" / "agent" / "models.yml").stat().st_mode & 0o777 == 0o644
...
async def test_run_rpc_uses_workspace_xdg_dirs_without_slot(tmp_path: Path, settings: Settings) -> None:
    ...
    xdg_root = inputs.workspace.root / ".zero2ai-xdg"
    for key in ("XDG_DATA_HOME", "XDG_STATE_HOME", "XDG_CACHE_HOME"):
        path = Path(env[key])
        assert path.is_relative_to(xdg_root)
        assert (path / "zero2ai").is_dir()
    tmpdir = inputs.workspace.root / ".zero2ai-tmp"
...
async def test_run_rpc_passes_slot_uid_user_slot_group_and_omp_extra_group(tmp_path: Path, settings: Settings) -> None:
    ...
    assert client_kwargs["extra_groups"] == ["zero2ai"]
...
async def test_run_rpc_hard_timeout_stops_client_and_fails(
    ...
    # `_closed_error` unset (zero2ai_rpc bug), so the worker would hang otherwise.
    assert len(fake.mark_closed_calls) == 1
    from zero2ai_rpc import RpcProcessExitError
...
async def test_run_rpc_cancel_hook_stops_and_marks_closed(
    ...
    an upstream zero2ai_rpc bug where `stop()` does not set `_closed_error`, leaving
...
async def test_run_rpc_review_pr_still_reminds_when_submit_fails(tmp_path: Path, settings: Settings) -> None:
    ...
    zero2ai_rpc normalizes xd:// device dispatches to the host-tool name, so a
