...
import httpx
import pytest
from zero2ai_rpc import HostToolContext, RpcCommandError
...
def test_repo_command_env_scrubs_secrets_and_uses_workspace_cache(
    ...
    assert env["BUN_INSTALL_CACHE_DIR"] == str(bindings.workspace.root / ".zero2ai-xdg" / "cache" / "bun-install")
    assert env["XDG_CACHE_HOME"] == str(bindings.workspace.root / ".zero2ai-xdg" / "cache")
    assert env["TMPDIR"] == str(bindings.workspace.root / ".zero2ai-tmp")
    ...
    assert (bindings.workspace.root / ".zero2ai-tmp").is_dir()
...
def test_run_repo_command_uses_slot_identity_kwargs(
    ...
    assert kwargs["env"]["BUN_INSTALL_CACHE_DIR"].endswith("/.zero2ai-xdg/cache/bun-install")
...
def _release_bindings(
    ...
    *,
    ...
    session_dir = root / ".zero2ai-session-v1.2.3"
    context_dir = root / "context"
    artifacts_dir = root / "artifacts"
