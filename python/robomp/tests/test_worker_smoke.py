...
Runs only when ROBOMP_INTEGRATION=1 and `zero2ai` is available on PATH (or via
...
- a real `zero2ai --mode rpc` subprocess driven by `worker.run_task`.
...
pytestmark = pytest.mark.skipif(
    not INTEGRATION,
    reason="ROBOMP_INTEGRATION=1 required to run the zero2ai-backed smoke test",
)
