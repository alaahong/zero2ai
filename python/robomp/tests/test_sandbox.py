...
def _workspace(root: Path) -> Workspace:
    return Workspace(
        root=root,
        repo_dir=root / "repo",
        session_dir=root / ".zero2ai-session",
...
def test_release_workspace_resets_to_remote_main_and_uses_tag_session(
    ...
    assert workspace.session_dir.name == ".zero2ai-session-v1.2.3"
    ...
    assert next_release.session_dir.name == ".zero2ai-session-v1.2.4"
...
def test_chown_workspace_runs_chown_and_chmod_as_root_on_linux(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # 2001 is the slot-private GID matching the slot UID, not the shared zero2ai group.
    assert calls == [
...
def test_prepare_slot_tmpdir_mkdirs_without_chown(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert tmpdir == tmp_path / ".zero2ai-tmp"
...
def test_provision_runtime_dirs_replaces_tmpdir_symlink_and_creates_xdg_tree(tmp_path: Path) -> None:
    ...
    assert target.is_dir()
    assert stat.S_IMODE(tmpdir.stat().st_mode) == 0o700
    for base in (tmp_path / ".zero2ai-xdg" / "data", tmp_path / ".zero2ai-xdg" / "state", tmp_path / ".zero2ai-xdg" / "cache"):
        assert base.is_dir()
        assert (base / "zero2ai").is_dir()
    assert (tmp_path / ".zero2ai-xdg" / "cache" / "bun-install").is_dir()
...
def test_prepare_slot_runtime_env_returns_workspace_private_paths_without_chown(
    ...
    bun_cache = ws.root / ".zero2ai-xdg" / "cache" / "bun-install"

    env = _prepare_slot_runtime_env(ws, 2001)

    assert env["TMPDIR"] == str(ws.root / ".zero2ai-tmp")
    assert env["XDG_CACHE_HOME"] == str(ws.root / ".zero2ai-xdg" / "cache")
    assert env["BUN_INSTALL_CACHE_DIR"] == str(bun_cache)
    for base in (ws.root / ".zero2ai-xdg" / "data", ws.root / ".zero2ai-xdg" / "state", ws.root / ".zero2ai-xdg" / "cache"):
        assert base.is_dir()
        assert (base / "zero2ai").is_dir()
...
def test_ensure_workspace_provisions_and_slot_owns_runtime_dirs(
    ...
    def record_chown(ws_root: Path, slot_uid: int | None) -> None:
        ...
        paths = [
            ws_root / ".zero2ai-tmp",
            ws_root / ".zero2ai-xdg" / "data",
            ws_root / ".zero2ai-xdg" / "data" / "zero2ai",
            ws_root / ".zero2ai-xdg" / "state",
            ws_root / ".zero2ai-xdg" / "state" / "zero2ai",
            ws_root / ".zero2ai-xdg" / "cache",
            ws_root / ".zero2ai-xdg" / "cache" / "zero2ai",
            ws_root / ".zero2ai-xdg" / "cache" / "bun-install",
        ]
    ...
    assert set(runtime_paths) == {
        ws.root / ".zero2ai-tmp",
        ws.root / ".zero2ai-xdg" / "data",
        ws.root / ".zero2ai-xdg" / "data" / "zero2ai",
        ws.root / ".zero2ai-xdg" / "state",
        ws.root / ".zero2ai-xdg" / "state" / "zero2ai",
        ws.root / ".zero2ai-xdg" / "cache",
        ws.root / ".zero2ai-xdg" / "cache" / "zero2ai",
        ws.root / ".zero2ai-xdg" / "cache" / "bun-install",
    }
...
def _seed_reclaimable_workspace(mgr: SandboxManager, repo: str, number: int) -> Path:
    ...
    for rel in (
        ...
        "repo/src",
        ".zero2ai-session",
        ".zero2ai-xdg/cache/bun-install",
        ".zero2ai-xdg/state/zero2ai",
        ".zero2ai-tmp",
        "artifacts",
    ):
    ...
    (ws_root / ".zero2ai-session/session.jsonl").write_text("{}", encoding="utf-8")
    (ws_root / ".zero2ai-xdg/cache/bun-install/pkg.tgz").write_text("x", encoding="utf-8")
    (ws_root / ".zero2ai-xdg/state/zero2ai/state.json").write_text("{}", encoding="utf-8")
    (ws_root / ".zero2ai-tmp/scratch").write_text("x", encoding="utf-8")
...
def test_reclaim_workspace_caches_strips_dep_caches_and_preserves_state(tmp_path: Path) -> None:
    ...
    assert not (ws_root / ".zero2ai-xdg/cache").exists()
    assert not (ws_root / ".zero2ai-tmp").exists()
    assert (ws_root / "repo/src/keep.ts").read_text(encoding="utf-8") == "keep"
    assert (ws_root / ".zero2ai-session/session.jsonl").exists()
    assert (ws_root / ".zero2ai-xdg/state/zero2ai/state.json").exists()
