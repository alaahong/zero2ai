...
1. **Workspace tree** (`/data/workspaces/<key>/`, including `repo/`,
   `.zero2ai-session/`, `context/`, `artifacts/`, `.zero2ai-tmp/`, `.zero2ai-xdg`):
   single-owner. Owned by the active slot UID/GID (`zero2ai-N`) when slot
...
2. **Clone pool** (`/data/workspaces/_pool/<owner>__<repo>/`): genuinely
   multi-slot. Owned by `root:zero2ai` (gid 2000) with setgid `02770`; cross-slot
...
3. **Language tool caches** (`/data/cache/{cargo,cargo-target,rustup,bun-cache}`):
   multi-slot. Owned by `root:zero2ai` with setgid `02770`; provisioned by
   `entrypoint.sh`.
...
def _prepare_slot_tmpdir(workspace: Workspace, slot_uid: int | None) -> Path:
    ...
    - replaces any non-directory at ``.zero2ai-tmp`` (symlink-protection: a user
    ...
    tmpdir = workspace.root / ".zero2ai-tmp"
    try:
        st = tmpdir.lstat()
...
def _prepare_slot_runtime_env(workspace: Workspace, slot_uid: int | None) -> dict[str, str]:
    ...
    and are group-shared via ``zero2ai``. Bun's install cache is explicitly
    ...
    xdg_root = workspace.root / ".zero2ai-xdg"
    ...
    for base in (xdg_data, xdg_state, xdg_cache):
        ...
        (base / "zero2ai").mkdir(parents=True, exist_ok=True)
...
def _provision_runtime_dirs(ws_root: Path) -> None:
    ...
    picks up ``.zero2ai-tmp`` and the per-workspace XDG tree. Without this,
    ...
    Symlink-safe on ``.zero2ai-tmp`` (replaces a planted non-directory in place).
    """
    tmpdir = ws_root / ".zero2ai-tmp"
    try:
        st = tmpdir.lstat()
    except FileNotFoundError:
        pass
...
def _share_git_metadata_with_slots(repo_dir: Path, slot_uid: int | None) -> None:
    ...
    `zero2ai-N` user, so the pool-side worktree gitdir, refs, reflogs, and object
    directories must stay writable through the shared `zero2ai` group.
    """
    if not _slot_permissions_active(slot_uid):
        return
...
class SandboxManager:
    ...
    def ensure_workspace(
        ...
        *,
        ...
        with self._repo_lock(repo):
            ...
            session_dir = ws_root / ".zero2ai-session"
            ...
            # keeps its `root:zero2ai` ownership (the slot reads through group `zero2ai`);
    ...
    def ensure_release_workspace(
        ...
        *,
        ...
        with self._repo_lock(repo):
            ...
            repo_dir = ws_root / "repo"
            session_dir = ws_root / f".zero2ai-session-{tag}"
            context_dir = ws_root / "context"
            artifacts_dir = ws_root / "artifacts"
            for path in (ws_root, session_dir, context_dir, context_dir / "repro", artifacts_dir):
                path.mkdir(parents=True, exist_ok=True)
    ...
    def _populate_natives_cache(self, workspace: Workspace, *, slot_uid: int | None = None) -> None:
        """Try to hardlink cached zero2ai-natives artifacts into the worktree.
        ...
        `.node` files are LEFT at `root:zero2ai` ownership — chowning them
        would chown the cache file too (shared inode), breaking the
        cross-slot sharing model. The slot reads them via group `zero2ai`.
        """
    ...
    def _chown_natives_for_slot(native_dir: Path, hit: CacheHit, *, slot_uid: int) -> None:
        ...
        hardlinks back into the root:zero2ai cache and the slot reads them via
        group `zero2ai`. Everything else (the directory itself, copied
