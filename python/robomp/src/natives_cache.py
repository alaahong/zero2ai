...
Ownership: cache root is provisioned ``root:zero2ai 02770`` by ``entrypoint.sh``
so slot subprocesses (group ``zero2ai``) can capture under setgid inheritance.
...
# napi build reads: all workspace crates (zero2ai-natives transitively depends on
# zero2ai-ast/zero2ai-iso/zero2ai-shell), the workspace Cargo manifest + lock, the rust
...
class NativesCache:
    """Per-repo content-addressed cache of zero2ai-natives build outputs."""
        ...
        *,
        ...
        with _flock(self.lockfile(repo)):
            ...
            try:
                ...
                # the setgid `zero2ai` model that lets other slots read it.
                # A copy creates a fresh inode owned by the orchestrator
                # (root) and inherits gid `zero2ai` from the setgid 2770
                # cache root.
