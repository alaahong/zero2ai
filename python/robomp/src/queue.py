...
class WorkerPool:
    ...
    async def stop(self, *, drain_timeout: float = 25.0, kill_timeout: float = 5.0) -> None:
        ...
        `reset_stuck_running()`. The resumed zero2ai session then picks up via
        ...
        #    if one was registered (kills the zero2ai subprocess); otherwise
        ...
        for task in still_running:
            ...
            if hook is not None:
                ...
                continue
            # No hook armed yet — the worker hasn't reached the zero2ai spawn
    ...
    async def cancel_event(self, delivery_id: str) -> bool:
        ...
        # doesn't stall the event loop for up to the zero2ai shutdown grace period.
        try:
            await asyncio.to_thread(hook)
        except Exception:
    ...
    async def _run_event(self, row: EventRow) -> None:
        ...
        try:
            ...
            if row.delivery_id in self._shutdown_cancelled:
                ...
                # resumed zero2ai session picks up via `--continue`.
