...
`roboomp` is a self-hosted GitHub triage-and-fix bot that drives [`zero2ai --mode rpc`](https://github.com/can1357/oh-my-pi) as a subprocess. On every issue opened in an allowlisted repository it classifies the issue, applies labels, then branches into one of: reproduce → fix → PR (`bug` / `documentation`), single-comment answer (`question`), single thoughtful comment (`enhancement` / `proposal`), or brief comment (`invalid` / `duplicate`). Follow-up comments and PR review comments resume the same zero2ai session so the agent keeps its prior reasoning. If the orchestrator restarts mid-task, the dispatcher resumes the same session via `zero2ai --continue` from the per-issue `session_dir`, so an interrupted task re-enters its prior reasoning instead of restarting from scratch. The orchestrator runs as a single FastAPI process inside Docker with SQLite-backed durable event state.

## Architecture & Data Flow

Webhook → durable queue → async dispatcher → per-issue git worktree → zero2ai RPC subprocess + host tools.
...
6. `tasks.*` dispatchers build `TaskInputs` and call `worker.run_task()` which spawns `zero2ai --mode rpc` with `cwd=worktree`, persistent `session_dir`, and a randomly-picked task-appropriate model pool. Existing `<session_dir>/*.jsonl` resumes with `--continue`.
7. Inside the subprocess the agent uses **built-in zero2ai tools** (read/edit/write/bash/lsp, scoped to the worktree) and **host tools** from `host_tools.py` (the only surface allowed to mutate GitHub or write audit rows).
...
- `/Dockerfile` (pi root) — produces `oh-my-pi/pi:dev` (pi runtime image: python + bun + rustup + zero2ai-natives + zero2ai_rpc + `/usr/local/bin/zero2ai` shim + the full pi source under `/pi`). Stages: `natives-builder` → `wheel-builder` → `zero2ai-base` → `zero2ai-runtime` (default). Built via `bun run pi:image`. Robomp's image extends `zero2ai-base` via `FROM ${ZERO2AI_BASE}` in `/Dockerfile.robomp`.
...
```
bun run test:py                   # pytest -x python/zero2ai-rpc/tests python/robomp/tests
bun run robomp:test:integration   # ROBOMP_INTEGRATION=1, requires zero2ai on PATH
...
- **Async style**: FastAPI handlers and `queue.WorkerPool` are async. `worker.run_task` is **synchronous** and runs in a worker thread because `zero2ai-rpc` is blocking — keep it that way; don't try to async it. CLI commands wrap with `asyncio.run`.
...
- `src/worker.py` — synchronous zero2ai RPC driver, prompt assembly via `persona`.
...
- `/Dockerfile.robomp` (pi root) — robomp's image. `FROM ${ZERO2AI_BASE}` (default `oh-my-pi/pi:dev`), adds the SolidJS dashboard bundle, the robomp Python package, and the `robomp-entrypoint` shim. Tini entrypoint, exposes `8080`, `VOLUME /data`. The toolchain (python + bun + rustup + zero2ai-natives + zero2ai_rpc + `zero2ai` shim) comes from `zero2ai-base` — no duplication in this file.
- `docker-compose.yml` — `build.args.ZERO2AI_BASE`, mounts `$ZERO2AI_ROOT:/work/pi:ro`, `./data:/data`, `~/.zero2ai/agent/models.container.yml:ro` (mapped to `models.yml` inside the container — kept separate from the host's `~/.zero2ai/agent/models.yml` so the host zero2ai doesn't pick up gateway routing intended only for the container), `extra_hosts: llm-gateway.internal:host-gateway`.
- `entrypoint.sh` — validates `ZERO2AI_ROOT`, creates `/data/{workspaces,logs}` + build caches.
...
- **Container runtime**: Docker Compose v2. The image embeds Bun 1.3.14 + a rustup launcher and exposes `zero2ai` via a `/usr/local/bin/zero2ai` shim; `ROBOMP_OMP_COMMAND=zero2ai` should not need changing.
...
- **ZERO2AI_ROOT resolution**: roboomp lives inside the oh-my-pi monorepo at `python/robomp/`. `bun run pi:image` builds the parent monorepo (`../..`) as its docker build context to produce `oh-my-pi/pi:dev`; `docker-compose.yml` extends that image via `ZERO2AI_BASE` and mounts the same parent path read-only at `/work/pi` for the orchestrator to see live source. Override `ZERO2AI_ROOT` only when pointing the build/mount at a different oh-my-pi checkout. Inside the container the path is always `/work/pi`. Build invalidation stays bounded: Python-only edits in roboomp never trigger a natives recompile.
...
- **Integration**: `tests/test_worker_smoke.py` is gated by `ROBOMP_INTEGRATION=1` (uses `pytestmark.skipif`) and needs `zero2ai` on `PATH`. Don't enable it in default `bun run test:py`.
