...
from zero2ai_rpc.client import _RpcFrameDecoder
...
class RpcClientTests(unittest.TestCase):
    ...
    def test_xd_dispatched_custom_tool_events_carry_host_tool_name(self) -> None:
        ...
        With `tools.xdev` on, zero2ai invokes a custom tool through `write
...
class TerminatesProcessGroupTests(unittest.TestCase):
    ...
    the zero2ai process. Before the fix, stop() signalled only the leader pid, so
    such grandchildren reparented to the container init and kept running —
    once ballooning to tens of GB of RAM. zero2ai is now spawned in its own
    ...
    def test_stop_kills_grandchild_spawned_by_server(self) -> None:
        ...
        # Fake zero2ai server: spawn the long-lived grandchild, signal ready, then
