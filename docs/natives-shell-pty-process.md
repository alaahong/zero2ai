...
This document covers execution/process/terminal primitives in `@zero2ai/natives`: `shell`, `pty`, `ps`, and `keys`, using the architecture terms from `docs/natives-architecture.md`.

## Implementation files

- `crates/zero2ai-natives/src/shell.rs`
- `crates/zero2ai-shell/src/shell.rs`
- `crates/zero2ai-shell/src/cancel.rs`
- `crates/zero2ai-builtins` (embedded-shell builtins: bash builtins plus in-process utility/process commands)
- `crates/zero2ai-shell/src/windows.rs` (Windows-only PATH enrichment)
- `crates/zero2ai-shell/src/process.rs`
- `crates/zero2ai-natives/src/pty.rs`
- `crates/zero2ai-natives/src/ps.rs`
- `crates/zero2ai-natives/src/keys.rs`
...
- **Rust N-API module layer** (`crates/zero2ai-natives/src/*`): JS-facing shell/PTY/process/key exports and callback bridging.
- **Runtime core** (`crates/zero2ai-shell/src/*`): brush shell execution, cancellation cleanup, minimizer integration, command fixups, and cross-platform process references.
...
- process builtins registered unconditionally from `zero2ai_builtins::process_builtins()` — `nohup`, `pgrep`, `pkill`, `pidwait`, `ps`, `sleep`, `timeout`, and `top` (`nohup` is withheld when `ZERO2AI_DISABLE_NOHUP_BUILTIN` is set; `kill` comes from the default bash-mode set, where `zero2ai-builtins`' richer implementation replaces brush's original),
- in-process utility builtins registered from `zero2ai_builtins::utility_builtins()` (see the next section),
...
- Windows-only path enrichment (`zero2ai-shell/src/windows.rs`) appends discovered Git-for-Windows paths when present and not already included.
...
Beyond the bash builtins, session creation registers the in-process command-line utility builtins implemented in the `zero2ai-builtins` crate (`crates/zero2ai-builtins`) — in-house ports of uutils coreutils/findutils/sed and jaq built on `uucore` 0.8.0. The set includes `cat`, `head`, `tail`, `wc`, `sort`, `uniq`, `ls`, `find`, `grep`, `mkdir`, `rm`, `mv`, `ln`, `sed`, `jq`, `fd`, `diff`, the checksum/`tr`/`cut`/`date` families, and more; `zero2ai_builtins::utility_builtins()` is the authoritative list.

Three search-related builtins are worth calling out:

- `grep` is implemented on the ripgrep libraries (`grep-regex`/`grep-searcher`), with recursive directory walks served by `zero2ai-walker`.
...
- `fd` is backed by `zero2ai-walker`, `globset`, and `regex`.

Each builtin runs inside the shell process (no `fork`/`exec`) against the `zero2ai-builtins` `Host` view of the shell (`src/host.rs`): stdio routes through the command's (possibly piped/redirected) file descriptors, path operands resolve against the shell working directory, the shell's exported environment is visible, and abort/timeout cancellation is honored. Because these builtins shadow system binaries, registration is gated in `crates/zero2ai-shell/src/shell.rs`:

- `ZERO2AI_DISABLE_UUTILS_BUILTINS` disables the whole utility set (bare names resolve to system binaries again),
- `ZERO2AI_DISABLE_UUTILS_DESTRUCTIVE` disables the destructive shadows (`rm`, `mv`, and `ln`, which can clobber via `-f`) together,
- `ZERO2AI_DISABLE_RM_BUILTIN` / `ZERO2AI_DISABLE_MV_BUILTIN` disable `rm`/`mv` individually.
...
- `CancelToken` is constructed from `timeoutMs` and optional `AbortSignal`, then converted into the shared `zero2ai_shell::cancel::CancelToken`.
...
The platform-specific implementation lives in `zero2ai_shell::process`; `crates/zero2ai-natives/src/ps.rs` is a N-API shim plus re-exports used by PTY termination.
