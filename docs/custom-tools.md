...
1. Capability providers (`toolCapability`), including:
   - Native ZERO2AI config (`~/.zero2ai/agent/tools`, `.zero2ai/tools`)
   - Claude config (`~/.claude/tools`, `.claude/tools`)
   - Codex config (`~/.codex/tools`, `.codex/tools`)
   - Claude marketplace plugin cache provider
2. Installed plugin manifests (`~/.zero2ai/plugins/node_modules/*` via plugin loader)
...
```ts
import type { CustomToolFactory } from "@zero2ai/coding-agent";
...
Parameter schemas may use the Zod-compatible schema builder (`pi.zod`), native schema builder (`pi.arktype`), or legacy-compatible TypeBox shim (`pi.typebox`) and flow through the shared validation/wire pipeline.
...
- `arktype`: injected schema `type(...)` builder
- `typebox`: compatibility shim for legacy TypeBox-style schemas
- `pi`: injected `@zero2ai/coding-agent` exports
...
- `params` is statically typed from its schema or TypeBox schema via `Static<TParams>`.
