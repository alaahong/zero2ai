# zero2ai-native auth-gateway transport

`zero2ai-native` is the lossless transport between a zero2ai-ai client and an
`zero2ai auth-gateway`. It is **not a textual tool-call dialect**: there is no
`<call:NAME>` grammar, parser, renderer, or `ZERO2AI_DIALECT=zero2ai-native` value in the
current implementation. Tool calls remain canonical zero2ai-ai `ToolCall` content
blocks inside `Context` and `AssistantMessageEvent`.

Use this transport when the client already speaks zero2ai-ai and the gateway owns
provider credentials—for example, a containerized zero2ai talking to a host
gateway or a robomp slot talking to its sidecar. OpenAI/Anthropic-compatible
routes translate and can lose zero2ai-specific fields; zero2ai-native sends the
...
### Removed zero2ai tool-call dialect

Historically, "zero2ai-native" named an in-band tool-call serialization: an XML
...
`ZERO2AI_DIALECT=pi`). Nothing in `packages/ai` emits or parses either spelling.
Old references to `<call:…>` blocks or `§` headers as "the zero2ai tool-call
...
`baseUrl` MUST identify an `zero2ai auth-gateway` (or compatible service). Missing
`baseUrl` fails with:

```text
zero2ai-native transport requires `baseUrl` on model MODEL_ID (set it on the provider config in models.yml)
```

When `model.transport === "zero2ai-native"`, `streamSimple` bypasses the normal
...
`ZERO2AI_STREAM_FIRST_EVENT_TIMEOUT_MS` / `ZERO2AI_STREAM_IDLE_TIMEOUT_MS` policy.
...
Provider/handler failures that reach the zero2ai-native route use:
...
- `packages/ai/src/stream.ts` — zero2ai-native dispatch
- `packages/ai/src/providers/zero2ai-native-client.ts` — request, auth, SSE and
  timeout behavior
- `packages/ai/src/providers/zero2ai-native-server.ts` — request validation,
