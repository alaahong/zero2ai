...
- Examples (adjust to match the actual packages you are porting):
  - `@mariozechner/pi-coding-agent` → `@zero2ai/coding-agent`
  - `@mariozechner/pi-agent-core` → `@zero2ai/agent-core`
  - `@mariozechner/pi-tui` → `@zero2ai/tui`
  - `@mariozechner/pi-ai` → `@zero2ai/ai`
  - `@mariozechner/pi-utils` → `@zero2ai/utils`
- Some upstream packages publish under the `@earendil-works/*` scope instead of `@mariozechner/*`. Map it the same way (`@earendil-works/pi-coding-agent` → `@zero2ai/coding-agent`, and so on).
- The bare `typebox` package is not an `@zero2ai/*` scope; do not rewrite it as one. See the Extensions divergence in section 15 for how tool-parameter schemas map.
...
- In `packages/coding-agent`, use `logger` from `@zero2ai/utils` for internal/runtime logging; CLI command files may use `console.*` for intentional user-facing output.
...
- Prefer existing helpers and utilities over new ad-hoc code.
  ...
  - TypeScript packages generally use source files directly; `@zero2ai/natives` exports generated native bindings from `packages/natives/native`.
...
| `ctx.ui.addAutocompleteProvider()`          | Wired in interactive mode; no-op stubs in ACP/RPC/headless contexts | Factory wrapping matches upstream; zero2ai's editor has no custom `triggerCharacters`, so wrapped providers surface at the built-in trigger points |
...
| Upstream                                           | Our Fork                                                  | Reason                                        |
| -------------------------------------------------- | --------------------------------------------------------- | --------------------------------------------- |
| `clipboard.ts` + `clipboard-image.ts` (tool files) | `src/utils/clipboard.ts` backed by `@zero2ai/natives` | Native implementation with a small TS wrapper |
...
| `pkg.pi` manifest field                                                | `pkg.zero2ai` preferred; fallback to `pkg.pi` remains                                                                                                                                                                                                                                            |
| `StringEnum` from `zero2ai-ai`                                              | `Type.Enum` from `pi.typebox`, or `pi.arktype.enumerated(...)`; `zero2ai-ai` no longer exports `StringEnum`                                                                                                                                                                                       |
| `formatSize` from `zero2ai-coding-agent`                                    | `formatBytes` from `@zero2ai/utils`                                                                                                                                                                                                                                                      |
...
- `clipboard-image.ts` — image clipboard support is exposed through `src/utils/clipboard.ts` backed by `@zero2ai/natives`
