...
- Key collaborators:
  - `packages/utils/src/ar` (`@zero2ai/utils/ar`) — unified archive registry: `parseArchivePathCandidates()` parses archive selectors, `readArchiveEntries()`/`writeArchive()` rewrite containers atomically.
