...
| `pty` | `boolean` | No | Request PTY mode. Default `false`. PTY is used only when `pty: true`, `ZERO2AI_NO_PTY !== "1"`, and the tool context has a UI. |
...
3. Foreground PTY
   - Requires `pty: true`, UI context, and `ZERO2AI_NO_PTY !== "1"`.
...
- Subprocesses / native bindings / client terminal
  - Non-PTY local execution uses native shell execution via `@zero2ai/natives` (`Shell.run()` or `executeShell()`).
...
- PTY mode is ignored in non-UI contexts and when `ZERO2AI_NO_PTY=1` (gated by `canUseInteractiveBashPty()`); the tool falls back to non-PTY execution and appends a `pty requested but unavailable in this environment; ran without a terminal` notice.
