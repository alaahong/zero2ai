# schema Guide (schema authoring in this repo)

Internal schemas use **`@zero2ai/schema`** — an ArkType-compatible validator
with a lazy JIT runtime (`packages/schema`). Author types with
`import { type } from "@zero2ai/schema"`.


## Why schema (perf contract)
...
- **schema** = a callable function with `.toJsonSchema` and `.assert` methods (`isArkSchema`).
...
- Failure returns an `Zero2AiErrors` (array of `Zero2AiError`); `type.errors === Zero2AiErrors`.
...
- Definition mistakes (bad DSL, illegal composition) throw `Zero2AiTypeError` at
  `type()` time.
...
(`packages/schema/src/type.ts`, `scope()` / `type.scope()`):
...
TypeBox-style and Zod-style authoring are backed by the schema runtime:

```ts
import { Type, type Static } from "@zero2ai/schema/typebox";
import { z } from "@zero2ai/schema/zod";

const User = z.object({ name: z.string() });
type User = z.infer<typeof User>;
```

These produce real schema schemas with JIT validation and `toJsonSchema`.
