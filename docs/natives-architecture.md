...
`@zero2ai/natives` combines a JavaScript ESM loader with a Rust Node-API addon:
...
- `crates/zero2ai-natives/src/lib.rs` and its modules
...
| -------------------------------- | --------------------- | ----------------------- | --------------------------------------------------------------------------------------- |
| `@zero2ai/natives`           | `native/index.js`     | `native/index.d.ts`     | Loads the addon immediately, then binds every generated class/function and enum object. |
| `@zero2ai/natives/desktop`   | `native/desktop.js`   | `native/desktop.d.ts`   | Exposes `createDesktopSession(options)` and defers addon loading until it is called.    |
| `@zero2ai/natives/clipboard` | `native/clipboard.js` | `native/clipboard.d.ts` | Exposes lazy `copyToClipboard` and `readImageFromClipboard` wrappers.                   |
...
Compiled mode is detected by a populated embedded manifest, `ZERO2AI_COMPILED`, or a Bun embedded marker in `import.meta.url`. It probes the versioned cache and legacy user-data directory before package/executable locations. `getNativesDir()` is `$XDG_DATA_HOME/zero2ai/natives` only when `$XDG_DATA_HOME/zero2ai` already exists; otherwise it is `~/.zero2ai/natives`.
...
Set `ZERO2AI_DEBUG_STARTUP` to emit synchronous `[startup]` markers to stderr around addon loading, extraction, and runtime installation.

## Rust module ownership

`crates/zero2ai-natives/src/lib.rs` registers the current modules:
...
- language metadata re-exported from `zero2ai_ast::language`.
...
- **`zero2ai-natives` and supporting crates** own algorithms, native resources, platform behavior, cancellation, and N-API conversion.
