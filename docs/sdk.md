...
The SDK is the in-process integration surface for `@zero2ai/coding-agent`.
...
The package root, `@zero2ai/coding-agent`, is the complete embedding surface. It includes `createAgentSession` and the focused `/sdk` exports, plus lower-level session, auth, model, mode, extension, and tool APIs.
...
- `cwd`: `getProjectDir()`
- `agentDir`: `~/.zero2ai/agent` (via `getAgentDir()`)
...
```ts
import { createAgentSession, SessionManager } from "@zero2ai/coding-agent";
...
```ts
import { SessionManager } from "@zero2ai/coding-agent";
...
```ts
import type { AgentSession } from "@zero2ai/coding-agent";
