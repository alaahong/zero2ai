...
use zero2ai_shell::{ShellExecuteOptions, cancel::CancelToken, execute_shell};
...
async fn session_start_skips_non_utf8_value_and_preserves_env() {
	...
	let _corrupt = ScopedEnvVar::set_corrupt("ZERO2AI_TEST_CORRUPT_8925", GHOSTTY_BIN_DIR_BYTES);
	let _sentinel = ScopedEnvVar::set("ZERO2AI_TEST_SENTINEL_8925", "sentinel-value");

	// Starts with a corrupt var present: must not panic.
	let (code, output) = run("echo $ZERO2AI_TEST_SENTINEL_8925").await;
