...
pub struct MinimizerOptions {
	...
	/// `ZERO2AI_MINIMIZER_LEGACY_FILTERS` environment variable (truthy = "1",
...
pub struct MinimizerConfig {
	...
	/// `MinimizerOptions.legacy_filters` or the `ZERO2AI_MINIMIZER_LEGACY_FILTERS`
...
impl MinimizerConfig {
	...
	pub fn from_options(opts: &MinimizerOptions) -> Self {
		...
		let legacy_requested = resolve_legacy_filters(
			opts.legacy_filters,
			std::env::var("ZERO2AI_MINIMIZER_LEGACY_FILTERS")
				.ok()
				.as_deref(),
		);
			...
			if let Ok(contents) = fs::read_to_string(&expanded) {
					...
					&& !expected.is_empty()
				{
					...
					if !actual_hex.eq_ignore_ascii_case(expected) {
						eprintln!(
							"[zero2ai-natives minimizer] settings_hash mismatch for {} (expected {}, got {}); \
							 ignoring file",
							expanded.display(),
							expected,
							actual_hex
				...
				match pipeline::parse_file(&contents, "user") {
					...
					Err(err) => {
						eprintln!("[zero2ai-natives minimizer] user filters: {err}");
					},
				}
			}
		}
...
impl SettingsFile {
	fn merge_into(self, cfg: &mut MinimizerConfig) {
		...
		{
			eprintln!(
				"[zero2ai-natives minimizer] unsupported schema_version {v} in settings file (expected \
...
/// and the raw `ZERO2AI_MINIMIZER_LEGACY_FILTERS` env value.
///
...
mod tests {
	...
	fn explicit_disabled_option_overrides_enabled_settings_file() {
		let path = std::env::temp_dir()
			.join(format!("zero2ai-minimizer-config-disabled-{}.toml", std::process::id()));
	...
	fn explicit_legacy_true_overrides_disabled_settings_file() {
		let path = std::env::temp_dir()
			.join(format!("zero2ai-minimizer-config-legacy-{}.toml", std::process::id()));
