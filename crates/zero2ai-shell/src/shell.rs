...
use bytes::Bytes;
use flume::Sender;
use zero2ai_builtins::{BuiltinSet, default_builtins};
#[cfg(not(unix))]
...
async fn create_session_for_run(
	...
	// Process inspection and control (see `zero2ai_builtins::process_builtins`).
	// `nohup` is withheld when ZERO2AI_DISABLE_NOHUP_BUILTIN asks for the system one;
	// `kill` already comes from the default set, where our richer implementation
	// replaced brush's.
	for (name, registration) in zero2ai_builtins::process_builtins() {
	...
	// `zero2ai_builtins::utility_builtins`): consistent, cross-platform
	...
	// (falling back to system binaries) via ZERO2AI_DISABLE_UUTILS_BUILTINS; the
	// destructive trio additionally honors ZERO2AI_DISABLE_UUTILS_DESTRUCTIVE, and
	// `rm`/`mv` have their own switches.
	if !uutils_env_disabled(config, "ZERO2AI_DISABLE_UUTILS_BUILTINS") {
		let destructive_disabled = uutils_env_disabled(config, "ZERO2AI_DISABLE_UUTILS_DESTRUCTIVE");
		let rm_disabled =
			destructive_disabled || uutils_env_disabled(config, "ZERO2AI_DISABLE_RM_BUILTIN");
		let mv_disabled =
			destructive_disabled || uutils_env_disabled(config, "ZERO2AI_DISABLE_MV_BUILTIN");
		for (name, registration) in zero2ai_builtins::utility_builtins() {
...
async fn source_snapshot(
	...
	let source_info = SourceInfo::from("zero2ai-natives:snapshot");
...
async fn run_shell_command_single(
		...
		&& let Some(config) = options.minimizer.as_ref()
	{
		...
		if !buffered.exceeded {
			...
			// `apply_shell_minimizer` in `zero2ai-natives`. A supported filter that
...
async fn run_shell_command_once(
	...
	let source_info = SourceInfo::from("zero2ai-natives:command");
	let result = session
		.shell
		.run_string(command, &source_info, &params)
		.await;
...
async fn run_shell_command_streams(
	...
	ensure_trailing_newline_for_heredoc(&mut command);
	let source_info = SourceInfo::from("zero2ai-shell:streams");
	let result = session
		.shell
		.run_string(command, &source_info, &params)
		.await;
...
/// `crates/zero2ai-vcs/src/git/cli.rs`.
...
/// backpressure: on a bounded channel (the zero2ai-natives JS bridge) the send
...
/// `ZERO2AI_DISABLE_NOHUP_BUILTIN` (session env or process env) asks otherwise.
fn nohup_builtin_disabled(config: &ShellConfig) -> bool {
	uutils_env_disabled(config, "ZERO2AI_DISABLE_NOHUP_BUILTIN")
}
...
mod tests {
	...
	async fn session_env_does_not_export_git_repo_location_overrides() {
		...
		env.insert("ZERO2AI_GIT_ENV_PROBE".to_string(), "kept".to_string());
		let config =
		...
		let command = format!(
			"printf '%s|%s' \"${{GIT_DIR-unset}}\" \"$ZERO2AI_GIT_ENV_PROBE\" > {}",
			quote_arg(out.to_str().expect("utf8 probe path"))
		);
		session
			.shell
			.run_string(command, &SourceInfo::from("zero2ai-natives:test"), &params)
			.await
			.expect("run_string");
	...
	async fn wait_for_process_name(pid: i32, expected: &str) {
		time::timeout(Duration::from_secs(2), async {
			loop {
				if zero2ai_builtins::ProcInfo::all().into_iter().any(|process| {
					process.pid() == pid
						&& process.match_name() == expected
						&& process.status() == zero2ai_builtins::ProcessStatus::Running
				}) {
					return;
				}
	...
	fn process_test_child(command: &std::path::Path, duration: Duration) -> Command {
		...
		child
			...
			.env("ZERO2AI_SHELL_PROCESS_TEST_SLEEP_MS", duration_ms.to_string())
	...
	fn process_test_sleeper() {
		let Ok(duration_ms) = std::env::var("ZERO2AI_SHELL_PROCESS_TEST_SLEEP_MS") else {
			return;
		};
	...
	async fn kill_builtin_accepts_numeric_signal_for_multiple_processes() {
		...
		let source_info = SourceInfo::from("zero2ai-natives:test");
	...
	async fn kill_builtin_signals_every_process_in_a_jobspec_pipeline() {
		const MARKER: &str = "ZERO2AI_SHELL_TEST_KILL_JOBSPEC_PIPELINE";
	...
	async fn kill_builtin_refuses_own_pid() {
		const MARKER: &str = "ZERO2AI_SHELL_TEST_KILL_OWN_PID";
		if std::env::var_os(MARKER).is_none() {
	...
	async fn kill_builtin_refuses_own_process_group() {
		const MARKER: &str = "ZERO2AI_SHELL_TEST_KILL_OWN_GROUP";
		if std::env::var_os(MARKER).is_none() {
			run_isolated_kill_test(
	...
	async fn pkill_builtin_excludes_own_pid() {
		const MARKER: &str = "ZERO2AI_SHELL_TEST_PKILL_OWN_PID";
		if std::env::var_os(MARKER).is_none() {
	...
	async fn uutils_sort_compress_program_uses_shell_path_and_stderr() {
		...
		let tmp = std::env::temp_dir().join(format!("zero2ai-sort-compress-{}", std::process::id()));
		...
		let shim = bin.join("zero2ai-test-compress");
		...
		let script = format!(
			"export PATH={}\nsort -S 1K --compress-program=zero2ai-test-compress in.txt > out.txt 2> \
			 err.txt",
	...
	async fn uutils_mkdir_resolves_cwd_and_displays_operand() {
		let tmp = std::env::temp_dir().join(format!("zero2ai-mkdir-{}", std::process::id()));
		let _ = std::fs::remove_dir_all(&tmp);
	...
	async fn command_v_iterates_all_operands() {
		let tmp = std::env::temp_dir().join(format!("zero2ai-command-v-{}", std::process::id()));
		let _ = std::fs::remove_dir_all(&tmp);
		std::fs::create_dir_all(&tmp).expect("temp dir");
		let tmp_str = tmp.to_str().expect("utf8 temp path");
	...
	/// Every utility `zero2ai-builtins` offers must actually be dispatchable in
	/// process, under the name it is registered as.
	///
	/// The registry (`zero2ai_builtins::utility_builtins`) maps a name to a
	...
	async fn every_utility_builtin_is_registered_under_its_own_name() {
		let tmp = std::env::temp_dir().join(format!("zero2ai-utility-help-{}", std::process::id()));
		let _ = std::fs::remove_dir_all(&tmp);
		std::fs::create_dir_all(&tmp).expect("temp dir");
		...
		let mut names: Vec<&'static str> =
			zero2ai_builtins::utility_builtins::<brush_core::extensions::DefaultShellExtensions>()
				.into_iter()
				.map(|(name, _)| name)
				.collect();
		names.sort_unstable();
	...
	/// Every process builtin `zero2ai-builtins` offers must be installed and
	/// dispatch in process.
	///
	...
	async fn every_process_builtin_is_installed_and_dispatches() {
		let tmp = std::env::temp_dir().join(format!("zero2ai-process-builtins-{}", std::process::id()));
		let _ = std::fs::remove_dir_all(&tmp);
		std::fs::create_dir_all(&tmp).expect("temp dir");
		...
		let mut registered: Vec<&'static str> =
			zero2ai_builtins::process_builtins::<brush_core::extensions::DefaultShellExtensions>()
				.into_iter()
				.map(|(name, _)| name)
				.collect();
	...
	async fn uutils_mkdir_expands_tilde_for_every_brace_element() {
		let base = std::env::temp_dir().join(format!("zero2ai-mkdir-brace-{}", std::process::id()));
	...
	async fn uutils_mkdir_help_and_bad_flag_do_not_exit_process() {
		let tmp = std::env::temp_dir().join(format!("zero2ai-mkdir-help-{}", std::process::id()));
		let _ = std::fs::remove_dir_all(&tmp);
		std::fs::create_dir_all(&tmp).expect("temp dir");
		let tmp_str = tmp.to_str().expect("utf8 temp path");
	...
	async fn uutils_head_streams_stdin_and_reads_files() {
		let tmp = std::env::temp_dir().join(format!("zero2ai-head-{}", std::process::id()));
		let _ = std::fs::remove_dir_all(&tmp);
		std::fs::create_dir_all(&tmp).expect("temp dir");
	...
	async fn uutils_head_help_and_bad_flag_do_not_exit_process() {
		let tmp = std::env::temp_dir().join(format!("zero2ai-head-help-{}", std::process::id()));
		let _ = std::fs::remove_dir_all(&tmp);
		std::fs::create_dir_all(&tmp).expect("temp dir");
		let tmp_str = tmp.to_str().expect("utf8 temp path");
	...
	async fn uutils_filters_listing_find_grep() {
		let tmp = std::env::temp_dir().join(format!("zero2ai-utils-{}", std::process::id()));
		let _ = std::fs::remove_dir_all(&tmp);
		...
		params.set_fd(OpenFiles::STDOUT_FD, null_file().expect("null"));
		params.set_fd(OpenFiles::STDERR_FD, null_file().expect("null"));
		let si = SourceInfo::from("zero2ai-natives:test");
	...
	async fn rg_builtin_uses_ripgrep_defaults() {
		let tmp = std::env::temp_dir().join(format!("zero2ai-rg-defaults-{}", std::process::id()));
		let _ = std::fs::remove_dir_all(&tmp);
	...
	async fn fd_builtin_uses_fd_defaults() {
		let tmp = std::env::temp_dir().join(format!("zero2ai-fd-defaults-{}", std::process::id()));
		let _ = std::fs::remove_dir_all(&tmp);
		std::fs::create_dir_all(tmp.join("sub")).expect("sub dir");
	...
	async fn rg_builtin_defaults_to_cwd_unless_stdin_is_pipeline() {
		let tmp = std::env::temp_dir().join(format!("zero2ai-rg-stdin-{}", std::process::id()));
		let _ = std::fs::remove_dir_all(&tmp);
		std::fs::create_dir_all(&tmp).expect("temp dir");
	...
	async fn grep_quiet_and_line_regexp() {
		let tmp = std::env::temp_dir().join(format!("zero2ai-grepq-{}", std::process::id()));
		let _ = std::fs::remove_dir_all(&tmp);
		std::fs::create_dir_all(&tmp).expect("temp dir");
		std::fs::write(tmp.join("data.txt"), "foo\nbar\nbaz\n").expect("data");
		let tmp_str = tmp.to_str().expect("utf8");
	...
	async fn uutils_rm_and_mv_operate_on_shell_cwd() {
		let tmp = std::env::temp_dir().join(format!("zero2ai-rmmv-{}", std::process::id()));
		let _ = std::fs::remove_dir_all(&tmp);
	...
	async fn uutils_rm_missing_file_prints_single_diagnostic() {
		let tmp = std::env::temp_dir().join(format!("zero2ai-rm-missing-{}", std::process::id()));
		let _ = std::fs::remove_dir_all(&tmp);
		std::fs::create_dir_all(&tmp).expect("temp dir");
		let tmp_str = tmp.to_str().expect("utf8");
	...
	async fn uutils_find_display_and_actions_split_paths() {
		let tmp = std::env::temp_dir().join(format!("zero2ai-find-split-{}", std::process::id()));
		let _ = std::fs::remove_dir_all(&tmp);
		...
		session
			.shell
			.run_string(
				"export ZERO2AI_EXEC_ENV=zed; find . -name keep.log -exec sh -c 'echo \"$ZERO2AI_EXEC_ENV $1\"' \
				 sh {} ';' > cap.txt",
				&si,
				&params,
			)
	...
	async fn uutils_sed_substitutes_streams_and_edits_in_place() {
		let tmp = std::env::temp_dir().join(format!("zero2ai-sed-{}", std::process::id()));
		let _ = std::fs::remove_dir_all(&tmp);
		std::fs::create_dir_all(&tmp).expect("temp dir");
	...
	async fn uutils_xargs_children_feed_pipeline_and_report_failure() {
		let tmp = std::env::temp_dir().join(format!("zero2ai-xargs-{}", std::process::id()));
		let _ = std::fs::remove_dir_all(&tmp);
		std::fs::create_dir_all(&tmp).expect("temp dir");
		let tmp_str = tmp.to_str().expect("utf8");
	...
	async fn uutils_jq_filters_json_and_propagates_exit_status() {
		let tmp = std::env::temp_dir().join(format!("zero2ai-jq-{}", std::process::id()));
		let _ = std::fs::remove_dir_all(&tmp);
		std::fs::create_dir_all(&tmp).expect("temp dir");
	...
	async fn uutils_builtins_respect_disable_env() {
		...
		let mut all_off = create_session(&session_with(&[("ZERO2AI_DISABLE_UUTILS_BUILTINS", "1")]))
			.await
			.expect("create_session");
		...
		let mut rm_off = create_session(&session_with(&[("ZERO2AI_DISABLE_RM_BUILTIN", "1")]))
			.await
			.expect("create_session");
	...
	///
	/// Lives in `zero2ai-natives` because the brush-core crate is excluded from the
	...
	fn unique_temp_dir(prefix: &str) -> std::path::PathBuf {
		...
		let nonce = std::time::SystemTime::now()
			...
			.expect("system time")
			.as_nanos();
		path.push(format!("zero2ai-shell-{prefix}-{}-{nonce}", std::process::id()));
...
"#,
	...
	/// "zero2ai-natives:command: syntax error at end of input". The segmented chain
	...
	async fn embedded_external_command_runs_in_its_own_session() {
		...
		// Build the same kind of session zero2ai-natives uses in production.
		let config = ShellConfig { session_env: None, snapshot_path: None, minimizer: None };
		let mut session = create_session(&config).await.expect("create_session");
	...
	/// leave the embedded host (ZERO2AI) stopped on its next tty read. The earlier
