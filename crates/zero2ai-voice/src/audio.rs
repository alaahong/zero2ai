...
//! mono `f32` contract: the N-API classes in zero2ai-natives adapt it to
...
mod tests {
	...
	fn opt_in_default_playback_initializes_and_stops() {
		if env::var_os("ZERO2AI_NATIVE_AUDIO_PLAYBACK_TEST").is_none() {
			return;
		}
	...
	fn opt_in_default_capture_receives_frames() {
		if env::var_os("ZERO2AI_NATIVE_AUDIO_CAPTURE_TEST").is_none() {
			return;
		}
