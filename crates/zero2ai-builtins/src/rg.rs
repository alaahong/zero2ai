...
struct RgWalk {
	request: zero2ai_walker::WalkRequest,
	filters: PathFilters,
}
...
impl PathFilters {
	fn includes(&self, path: &Path, file_type: zero2ai_walker::FileType, size: Option<f64>) -> bool {
		let is_dir = file_type == zero2ai_walker::FileType::Dir;
		...
		if file_type != zero2ai_walker::FileType::File {
			return true;
		}
		if !explicitly_included
			&& self
...
fn build_walk(host: &mut Host, cli: &Rg, root: &Path) -> Result<RgWalk, String> {
	...
	let request = zero2ai_walker::WalkRequest::new(root)
		.hidden(include_hidden)
		.gitignore(!no_ignore)
		.skip_git(!no_ignore)
		.skip_node_modules(false)
		.follow_links(zero2ai_walker::FollowLinks::from(cli.follow && !cli.no_follow))
		.detail(if filters.max_filesize.is_some() {
			zero2ai_walker::WalkDetail::Full
		} else {
			zero2ai_walker::WalkDetail::Minimal
		})
		.order(order)
		.emit_root(false)
	...
	let streamed = match walk.request.for_each_entry_with_heartbeat(
		...
		|entry| {
			...
			if !walk.filters.includes(path, entry.file_type, entry.size) {
				return Ok(if entry.file_type == zero2ai_walker::FileType::Dir {
			...
			Ok(if opts.quiet && any_match.get() {
				zero2ai_walker::WalkDecision::Stop
			} else {
				zero2ai_walker::WalkDecision::Include
			})
		},
		|error| {
			had_error.set(true);
...
fn collect_filtered_files(host: &mut Host, cli: &Rg, root: &Path) -> io::Result<Vec<PathBuf>> {
	...
	match result {
		Ok(zero2ai_walker::WalkStatus::Complete | zero2ai_walker::WalkStatus::Stopped) => Ok(files),
		Err(zero2ai_walker::WalkError::Interrupted(_)) if host.is_cancelled() => {
