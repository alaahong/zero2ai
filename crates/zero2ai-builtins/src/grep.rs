...
//!
//! Matching uses `grep-regex`/`grep-searcher`; recursive walks use `zero2ai-walker`.
...
#[command(
	name = "grep",
	version = concat!("grep (zero2ai-uu-grep) ", env!("CARGO_PKG_VERSION")),
...
fn resolve_follow_links(cli: &GrepArgs, matches: &ArgMatches) -> zero2ai_walker::FollowLinks {
	let mut selected = (0, zero2ai_walker::FollowLinks::Roots);
	choose_latest(&mut selected, last_index(matches, "recursive"), zero2ai_walker::FollowLinks::Roots);
	choose_latest(
		&mut selected,
		last_index(matches, "dereference_recursive"),
		zero2ai_walker::FollowLinks::Always,
	);
	if cli.directories == Some(DirectoryAction::Recurse) {
		choose_latest(
			&mut selected,
			last_index(matches, "directories"),
			zero2ai_walker::FollowLinks::Roots,
		);
	}
	choose_latest(
		&mut selected,
	...
	choose_latest(&mut selected, last_index(matches, "no_follow"), zero2ai_walker::FollowLinks::Never);
	choose_latest(&mut selected, last_index(matches, "follow_all"), zero2ai_walker::FollowLinks::Always);
	selected.1
}
...
fn grep_walk_request(root: &Path, follow_links: zero2ai_walker::FollowLinks) -> zero2ai_walker::WalkRequest {
	zero2ai_walker::WalkRequest::new(root)
		...
		.order(zero2ai_walker::WalkOrder::Unordered)
		.emit_root(true)
		.depth(0, usize::MAX)
		...
		.cache(false)
		.filter(zero2ai_walker::WalkFilter::all())
}
...
fn search_dir<M: Matcher, W: Write>(
	...
	opts: &Options,
	rules: &PathRules,
	follow_links: zero2ai_walker::FollowLinks,
	out: &mut W,
	had_error: &mut bool,
) -> io::Result<bool> {
	...
	let walk = request.for_each_entry_with_heartbeat(
		...
		|entry: zero2ai_walker::EntryMeta<'_>| {
			if opts.quiet && any {
				return Ok(zero2ai_walker::WalkDecision::Stop);
			}
			if entry.file_type == zero2ai_walker::FileType::Dir {
			...
			if entry.file_type != zero2ai_walker::FileType::File
		...
		|error: zero2ai_walker::DirectoryError<'_>| {
	...
	*had_error |= had_error_state.get();
	match walk {
		Ok(zero2ai_walker::WalkStatus::Complete | zero2ai_walker::WalkStatus::Stopped) => Ok(any),
		// Propagate BrokenPipe to stop the walk; the host maps its status.
		Err(zero2ai_walker::WalkError::Interrupted(error))
			if error.kind() == io::ErrorKind::BrokenPipe =>
		{
			Err(error)
		},
		Err(zero2ai_walker::WalkError::Interrupted(_)) if host.is_cancelled() => {
		...
		Err(zero2ai_walker::WalkError::InvalidData { path, message }) => {
			*had_error = true;
			if !opts.no_messages {
				let display_path = display_path_for_operand(operand, resolved, &path);
				let _ = writeln!(
...
mod tests {
	...
	fn version_is_reported_on_stdout() {
		...
		assert_eq!(code, 0);
		assert!(err.is_empty());
		assert!(out.contains("grep") && out.contains("zero2ai-uu-grep"));
	}
