...
impl Default for Config {
	fn default() -> Self {
		Self {
			...
			today_start:       false,
			// Directory information and traversal are handled by zero2ai_walker,
...
fn walker_follow_links(follow: Follow) -> zero2ai_walker::FollowLinks {
	match follow {
		Follow::Never => zero2ai_walker::FollowLinks::Never,
		Follow::Roots => zero2ai_walker::FollowLinks::Roots,
		Follow::Always => zero2ai_walker::FollowLinks::Always,
	}
}

fn build_find_walk_request(config: &Config, root: &Path) -> zero2ai_walker::WalkRequest {
	zero2ai_walker::WalkRequest::new(root)
		...
		.visit_order(if config.depth_first {
			zero2ai_walker::VisitOrder::ContentsFirst
		} else {
			zero2ai_walker::VisitOrder::PreOrder
		})
...
fn process_dir_walk_request(
	...
	let status = request.for_each_entry_with_heartbeat(
		...
		|entry: zero2ai_walker::EntryMeta<'_>| {
			...
			if host.sigpipe_hit() {
				...
				Ok(zero2ai_walker::WalkDecision::SkipDescend)
			} else {
	...
	match status {
		...
		Err(zero2ai_walker::WalkError::Interrupted(err)) => {
			ret_value = 1;
...
/// zero2ai-uutils: BSD `find -E` compatibility (macOS muscle memory).
///
