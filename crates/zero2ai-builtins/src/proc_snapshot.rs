...
//! builtins above stay platform-agnostic. Lifted out of `zero2ai-shell` when the
//! process builtins moved into this crate; `zero2ai-shell` keeps its own
//! session/teardown process management (`zero2ai_shell::process`), which is a
...
///
/// Shared by the process-table snapshots here and by `zero2ai-shell`'s own
