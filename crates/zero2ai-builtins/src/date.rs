...
mod format_modifiers {
	...
	impl fmt::Display for FormatError {
		fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
			match self {
				...
				// zero2ai-uutils: literalized en-US translation of
	...
	// zero2ai-uutils: `LazyLock` instead of upstream's function-local `OnceLock`.
...
mod tests {
	...
	fn reference_prefers_existing_file_over_epoch() {
		let dir = std::env::temp_dir().join(format!("zero2ai-date-r-{}", std::process::id()));
