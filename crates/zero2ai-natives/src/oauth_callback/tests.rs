...
fn temp_home(label: &str) -> PathBuf {
	...
	let path =
		std::env::temp_dir().join(format!("zero2ai-native-oauth-{label}-{}-{id}", std::process::id()));
...
fn invalid_scheme_is_rejected_before_filesystem_access() {
	...
	assert!(validate_scheme("ZERO2AI").is_err());
	assert!(validate_scheme("zero2ai+oauth.test").is_ok());
}
...
fn remote_start_is_unsupported_without_creating_storage() {
	...
	assert!(!home.join(".zero2ai").exists());
...
fn activation_failure_restores_even_after_mutating_os_state() {
	...
	assert!(!home.join(".oauth-owner-zero2ai-test").exists());
	assert!(
		!storage_root(&home, "zero2ai-test")
...
fn stale_journal_is_recovered_before_successor_activation() {
	...
	let root = storage_root(&home, "zero2ai-test");
	...
	assert_eq!(
		fs::read_to_string(home.join(".oauth-owner-zero2ai-test")).unwrap(),
...
async fn cancellation_prevents_start_and_wait_claims_once() {
	...
	fs::write(&callback, b"zero2ai-test://callback?code=one").unwrap();
	...
	.await
	.unwrap();
	assert_eq!(url, "zero2ai-test://callback?code=one");
...
fn journal_rejects_traversal_and_unknown_fields() {
	...
	let json = br#"{
		...
		"scheme":"zero2ai-test",
		"environment":{},
		"snapshot":{"version":1,"id":"0123456789abcdef0123456789abcdef","scheme":"zero2ai-test"},
		"extra":true
	}"#;
