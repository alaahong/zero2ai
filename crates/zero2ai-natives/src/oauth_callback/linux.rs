...
const DESKTOP_ID_PREFIX: &str = "dev.zero2ai.oauth-callback.";
...
fn desktop_file(snapshot: &Snapshot) -> anyhow::Result<String> {
	Ok([
		...
		"Name=zero2ai OAuth Callback".to_owned(),
...
mod tests {
	...
	impl TempDir {
		fn new() -> Self {
			...
			let path = std::env::temp_dir()
				.join(format!("zero2ai-native-linux-oauth-{}-{serial}", std::process::id()));
	...
	fn build_context(root: &TempDir, desktop: &str, inherited: &str) -> (Context, CommandLog) {
		...
		context.runner = Some(Arc::new(move |program, args| {
			...
			assert_eq!(args[2], "x-scheme-handler/zero2ai-auth");
			let current = fs::read_to_string(&preference).unwrap_or_default();
			let (_, entry) = parse_default_entry(&current, "x-scheme-handler/zero2ai-auth")?;
			Ok(if entry.present {
	...
	fn activation_and_restore_preserve_unrelated_concurrent_edits() {
		...
		fs::write(
			&preference_path,
			"[Default Applications]\nx-scheme-handler/zero2ai-auth=previous.desktop;\ntext/html=browser.\
			 desktop;\n",
		)
		.unwrap();
		...
		assert!(restored.contains("x-scheme-handler/zero2ai-auth=previous.desktop;"));
	...
	fn restore_preserves_external_scheme_choice() {
		...
		let restored = fs::read_to_string(&snapshot.preference_path).unwrap();
		assert!(restored.contains("x-scheme-handler/zero2ai-auth=external.desktop;"));
		assert!(!snapshot.desktop_path.exists());
	}
