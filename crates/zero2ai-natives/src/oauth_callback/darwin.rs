...
const BUNDLE_PREFIX: &str = "dev.zero2ai.oauth-callback.";
const APP_PREFIX: &str = "zero2ai OAuth Callback ";
const STAGING_APP_NAME: &str = "ZERO2AI OAuth Callback.app";
...
<dict>
	...
	<string>zero2ai OAuth Callback</string>
...
fn remove_own_registration(
	...
	if matches!(
		...
		bail!("macOS still resolves {} URLs to the temporary zero2ai application", context.scheme);
	}
	if !same_application(&after, expected_handler) {
		bail!(
			"the external {} URL handler changed while removing the zero2ai application",
			context.scheme
		);
	}
...
fn recover_legacy_registration(context: &Context) -> Result<()> {
	...
	let expected_after = match &before {
		ApplicationState::Found { app_path, bundle_id } if bundle_id == &record.bundle_id => {
			if app_path != &record.app_path {
				bail!("legacy zero2ai callback bundle resolves to an unexpected application path");
			}
			if record.previous_handler.is_empty() {
				...
				if matches!(
					...
					bail!("macOS still uses the legacy zero2ai callback application");
				}
				after
			} else {
				...
				if !same_application(&after, &restored) {
					bail!("legacy URL handler changed while unregistering the zero2ai application");
				}
				after
			}
		},
		ApplicationState::Found { bundle_id, .. } if bundle_id.starts_with(BUNDLE_PREFIX) => {
			bail!("an unrelated zero2ai callback application owns the legacy URL scheme")
		},
	...
	if matches!(
		...
		bail!("macOS still resolves the legacy zero2ai callback application");
	}
...
pub(super) fn prepare(context: &Context) -> Result<Snapshot> {
	...
	let result = (|| {
		...
		{
			bail!(
				"stale zero2ai callback handler {bundle_id} has no recovery journal; remove it and retry"
			);
		}
		Ok(Snapshot {
	...
	let own_active = match &active {
		ApplicationState::Found { app_path, bundle_id } if bundle_id == &snapshot.bundle_id => {
			if app_path != &snapshot.app_path {
				bail!("active zero2ai callback bundle has an unexpected application path");
			}
			true
		},
		...
		match &snapshot.previous {
			...
			ApplicationState::Absent => {
				...
				if matches!(
					...
					bail!("macOS still resolves URLs to the temporary zero2ai application");
				}
				remove_transaction_applications(context, snapshot)
			},
...
mod tests {
	...
	impl TempDir {
		fn new() -> Self {
			...
			let path = std::env::temp_dir()
				.join(format!("zero2ai-native-darwin-oauth-{}-{serial}", std::process::id()));
	...
	fn prepare_builds_private_application_and_captures_handler() {
		...
		assert!(plist.contains("<string>zero2ai-auth</string>"));
	...
	fn legacy_recovery_restores_bundle_and_removes_owned_artifacts() {
		...
		let legacy_app = root
			.0
			.join("home/Applications/zero2ai OAuth Callback abandoned.app");
		fs::create_dir_all(&legacy_app).unwrap();
		let legacy_bundle = "dev.zero2ai.oauth-callback.aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
	...
	fn invalid_legacy_record_is_retained_and_generated_app_is_removed() {
		...
		fs::write(
            &recovery_path,
            br#"{"appPath":"/tmp/unowned.app","bundleId":"dev.zero2ai.oauth-callback.bad","pid":999999,"previousHandler":"","scheme":"zero2ai-auth"}"#,
        )
        .unwrap();
	...
	fn live_legacy_owner_is_not_recovered() {
		let root = TempDir::new();
		let legacy_app = root.0.join("home/Applications/zero2ai OAuth Callback live.app");
		fs::create_dir_all(&legacy_app).unwrap();
		let (context, _) = context(&root, ApplicationState::Absent);
		let recovery_path = legacy_recovery_path(&context);
		fs::create_dir_all(recovery_path.parent().unwrap()).unwrap();
		fs::write(
			...
			serde_json::to_vec(&json!({
				 "appPath": legacy_app,
				 "bundleId": "dev.zero2ai.oauth-callback.aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
				 "pid": std::process::id(),
				 "previousHandler": "",
				 "scheme": "zero2ai-auth"
			}))
