...
const MARKER_NAME: &str = "zero2ai OAuth Callback Transaction";
...
mod tests {
	...
	impl DisposableScheme {
		fn new() -> (Self, Context) {
			...
			let scheme = format!("zero2ai-oauth-test-{unique}");
			let directory = std::env::temp_dir().join(format!("zero2ai oauth callback {unique}"));
	...
	fn command_quotes_native_paths_and_url_as_data() {
		let helper = Path::new(r#"C:\Program Files\zero2ai\callback "helper".exe"#);
		let callback = Path::new(r"C:\OAuth callbacks\pending\");
		let command = relay_command(helper, callback).unwrap();
		assert_eq!(
			command,
			OsString::from(
				r#""C:\Program Files\zero2ai\callback \"helper\".exe" "C:\OAuth callbacks\pending\\" "%1""#
			)
		);
	}
	...
	fn command_rewrites_canonicalized_verbatim_paths_the_shell_cannot_launch() {
		let command = relay_command(
			Path::new(r"\\?\C:\Users\dev\.zero2ai\oauth\callback-helper.exe"),
			Path::new(r"\\?\C:\Users\dev\.zero2ai\oauth\callback.url"),
		)
		.unwrap();
		assert_eq!(
			command,
			OsString::from(
				r#""C:\Users\dev\.zero2ai\oauth\callback-helper.exe" "C:\Users\dev\.zero2ai\oauth\callback.url" "%1""#
			)
		);
	}
	...
	fn recovery_falls_back_to_legacy_commands_for_volume_paths_the_shell_refuses() {
		...
		let verbatim = Context::new(
			context.home,
			PathBuf::from(r"\\?\Volume{d0e5f6a7-0000-0000-0000-000000000000}\zero2ai-oauth-test"),
			context.scheme,
			context.id,
			BTreeMap::new(),
			CancelToken::default(),
