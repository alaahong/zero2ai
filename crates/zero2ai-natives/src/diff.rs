...
//! `zero2ai-diff`. This module preserves JavaScript's UTF-16 code units at the N-API
...
fn diff_lines_impl(old_text: &[u16], new_text: &[u16]) -> Vec<DiffChange> {
	zero2ai_diff::diff_lines_u16(old_text, new_text)
		.into_iter()
...
fn diff_line_runs_impl(old_text: &[u16], new_text: &[u16]) -> Vec<DiffRun> {
	...
	let (old_ids, new_ids) = zero2ai_diff::intern(&old_tokens, &new_tokens);
	zero2ai_diff::myers_diff(&old_ids, &new_ids)
		.into_iter()
...
fn stream_result(old_text: &[u16], new_text: &[u16], context: Option<u32>) -> DiffStreamResult {
	let old_tokens = zero2ai_diff::line_tokens_u16(old_text);
	let new_tokens = zero2ai_diff::line_tokens_u16(new_text);
	let runs = zero2ai_diff::line_runs_u16(old_text, new_text);
	let exposed_runs = runs
		.iter()
		.map(|run| DiffRun { count: run.count, added: run.added, removed: run.removed })
		.collect();
	let hunks =
		zero2ai_diff::structured_patch_hunks_from_runs_u16(context, &old_tokens, &new_tokens, &runs)
			.into_iter()
			.map(patch_hunk)
			.collect();
	DiffStreamResult {
...
fn structured_patch_hunks_impl(
	...
	zero2ai_diff::structured_patch_hunks_u16(old_text, new_text, context)
		.into_iter()
		.map(patch_hunk)
		.collect()
}

fn patch_hunk(hunk: zero2ai_diff::Hunk) -> PatchHunk {
	PatchHunk {
...
fn diff_words_impl(old_text: &[u16], new_text: &[u16]) -> Vec<DiffChange> {
	zero2ai_diff::diff_words_u16(old_text, new_text)
		.into_iter()
		.map(|change| DiffChange {
			value:   change.value.into(),
			count:   change.count,
...
mod tests {
	...
	fn native_file_stream_reads_without_js_text_round_trip() {
		let path = std::env::temp_dir().join(format!(
			"zero2ai-native-diff-stream-{}-{}.txt",
			std::process::id(),
