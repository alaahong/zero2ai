...
/// PCRE2 JIT toggle: `ZERO2AI_PCRE2_JIT=1` forces JIT on, `0`/`false` forces it
...
static PCRE2_JIT_ENABLED: LazyLock<bool> = LazyLock::new(|| match std::env::var("ZERO2AI_PCRE2_JIT") {
...
fn build_grep_walk_request(
	...
	order: zero2ai_walker::WalkOrder,
) -> Result<zero2ai_walker::WalkRequest> {
	let mut filter = zero2ai_walker::WalkFilter::files_only();
	if let Some(glob) = glob.map(str::trim).filter(|value| !value.is_empty()) {
		let pattern = glob_util::build_glob_pattern(glob, true);
		let compiled = zero2ai_walker::CompiledWalkGlob::new([pattern])
			.map_err(|err| Error::from_reason(format!("Invalid glob pattern: {err}")))?;
		filter = filter.glob(compiled);
	}

	Ok(zero2ai_walker::WalkRequest::new(search_path)
		.hidden(include_hidden)
		.gitignore(use_gitignore)
		.skip_git(true)
		.skip_node_modules(skip_node_modules)
		.follow_links(zero2ai_walker::FollowLinks::Never)
		.detail(zero2ai_walker::WalkDetail::Minimal)
		.size_hints(zero2ai_walker::SizeHintPolicy::WhenCheap)
		.order(order)
		.emit_root(false)
		.depth(1, usize::MAX)
		.directory_errors(zero2ai_walker::DirectoryErrorMode::SkipSkippable)
		.cache(false)
		.filter(filter))
}
...
fn collect_grep_candidates(
	...
	order: zero2ai_walker::WalkOrder,
	ct: &task::CancelToken,
) -> Result<Option<Vec<zero2ai_walker::FileCandidate>>> {
...
struct PassState {
	...
	deferred:          Mutex<Vec<zero2ai_walker::FileCandidate>>,
...
fn search_one_file<M: Matcher + Sync>(
	...
	matcher: &M,
	file: &zero2ai_walker::FileCandidate,
...
fn run_pass<M: Matcher + Sync>(
	candidates: &[zero2ai_walker::FileCandidate],
	matcher: &M,
	file_params: SearchParams,
	policy: ReadPolicy,
	...
	if parallel_allowed && zero2ai_walker::should_parallelize(candidates.len()) {
		zero2ai_walker::execute_candidates_init(
			candidates,
...
fn process_candidates<M: Matcher + Sync>(
	candidates: Vec<zero2ai_walker::FileCandidate>,
	matcher: &M,
	params: SearchParams,
	parallel_allowed: bool,
	stop_after_matches: Option<u64>,
...
fn run_parallel_streaming_grep<M: Matcher + Sync>(
	...
	let request = build_grep_walk_request(
		...
		use_gitignore,
		skip_node_modules,
		zero2ai_walker::WalkOrder::Unordered,
	)?;
	let file_params = per_file_params(params);
	let state = PassState::default();
	...
	request
		.for_each_file_candidate_parallel(
			|file| {
				...
				{
					return Ok(zero2ai_walker::ParallelWalkControl::Continue);
				}
				...
				Ok(zero2ai_walker::ParallelWalkControl::Continue)
			},
			|| ct.heartbeat(),
		)
...
fn flush_stream_window<M: Matcher + Sync>(
	window: &mut Vec<zero2ai_walker::FileCandidate>,
	...
	request
		.for_each_entry_with_heartbeat(
			...
			|entry| {
				...
				window.push(zero2ai_walker::FileCandidate {
			...
			|_| Ok(zero2ai_walker::WalkDecision::Include),
		)
		.map_err(iofs::map_walker_error)?;
...
fn run_streaming_grep<M: Matcher + Sync>(
	...
	match stop_after_matches {
		...
		Some(stop) if stop <= ORDERED_STREAMING_STOP_MAX_COUNT || zero2ai_walker::walk_workers() <= 1 => {
			run_sequential_grep(
				search_path,
				matcher,
				glob,
...
mod tests {
	...
	impl TempDirGuard {
		fn new() -> Self {
			...
			let path = std::env::temp_dir().join(format!("zero2ai-grep-test-{pid}-{nanos}-{seq}"));
