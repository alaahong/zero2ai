...
/// Resolves the `zero2ai` state directory (`$XDG_STATE_HOME/zero2ai` or
/// `~/.local/state/zero2ai`) that holds portal tokens.
fn omp_state_dir() -> Option<PathBuf> {
	let base = std::env::var_os("XDG_STATE_HOME")
		.map(PathBuf::from)
		.or_else(|| std::env::var_os("HOME").map(|home| PathBuf::from(home).join(".local/state")))?;
	Some(base.join("zero2ai"))
}
...
mod tests {
	...
	fn removes_orphaned_remote_desktop_token() {
		let dir = std::env::temp_dir().join(format!("zero2ai-token-test-{}", std::process::id()));
