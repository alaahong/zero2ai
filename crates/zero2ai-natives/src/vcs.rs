...
/// a machine-readable `code` (the [`zero2ai_vcs::Error::kind`] discriminant), and
/// the CLI result fields (`exitCode`/`stdout`/`stderr`; non-CLI failures
/// mirror the message into `stderr` and synthesize `exitCode: 1`).
fn rich_error(env: Env, err: zero2ai_vcs::Error) -> napi::Error {
	let message = err.to_string();
	let kind = err.kind();
	let (exit_code, stdout, stderr) = match err {
		zero2ai_vcs::Error::Cli { exit_code, stdout, stderr, .. } => (exit_code, stdout, stderr),
...
fn vcs_future<'env, T: ToNapiValue + Send + 'static>(
	env: &'env Env,
	tag: &'static str,
	work: impl Future<Output = zero2ai_vcs::Result<T>> + Send + 'static,
) -> Result<Object<'env>> {
...
/// [`zero2ai_vcs::Error`] for the JS-thread rich-error conversion. Named
/// `Promise` so the napi macro emits a plain `Promise<T>` TS return type.
type Promise<T> = task::MappedPromise<T, zero2ai_vcs::Error>;
...
/// Convert a panic escaping a VCS operation into a typed [`zero2ai_vcs::Error`].
///
...
/// failure mode.
fn catch_panic<T>(tag: &'static str, f: impl FnOnce() -> zero2ai_vcs::Result<T>) -> zero2ai_vcs::Result<T> {
	...
	match std::panic::catch_unwind(std::panic::AssertUnwindSafe(f)) {
		...
		Err(payload) => {
			...
			Err(zero2ai_vcs::Error::backend(tag, format!("native panic: {message}")))
		},
	}
}
...
fn blocking<T: Send + 'static + ToNapiValue + TypeName>(
	tag: &'static str,
	repo: Arc<zero2ai_vcs::git::GitRepo>,
	signal: Option<Unknown>,
	f: impl FnOnce(&zero2ai_vcs::git::GitRepo) -> zero2ai_vcs::Result<T> + Send + 'static,
) -> Promise<T> {
	let ct = task::CancelToken::new(None, signal);
	task::blocking_mapped(tag, ct, rich_error, move |ct| {
		if ct.heartbeat().is_err() {
			return Err(zero2ai_vcs::Error::Canceled);
		}
...
fn repo_blocking<T: Send + 'static + ToNapiValue + TypeName>(
	tag: &'static str,
	repo: zero2ai_vcs::Repo,
	signal: Option<Unknown>,
	f: impl FnOnce(&zero2ai_vcs::Repo) -> zero2ai_vcs::Result<T> + Send + 'static,
) -> Promise<T> {
	let ct = task::CancelToken::new(None, signal);
	task::blocking_mapped(tag, ct, rich_error, move |ct| {
		if ct.heartbeat().is_err() {
...
pub fn vcs_discover_for_display(env: Env, dir: String) -> Result<Option<VcsRepo>> {
	zero2ai_vcs::detect_for_display(Path::new(&dir))
		.map(|repo| repo.map(|inner| VcsRepo { inner }))
		.map_err(|err| rich_error(env, err))
}
...
impl VcsRepo {
	...
	pub fn supports(&self, env: Env, feature: String) -> Result<bool> {
		let parsed = zero2ai_vcs::Feature::parse(&feature).ok_or_else(|| {
			rich_error(
				env,
				zero2ai_vcs::Error::backend(
					"vcs supports",
	...
	pub fn status_porcelain(
		...
		repo_blocking("vcs.statusPorcelain", self.inner.clone(), signal, move |repo| {
			repo.status_porcelain(&options.map_err(|error| zero2ai_vcs::Error::Backend {
...
pub fn vcs_git_repo_info(env: Env, dir: String) -> Result<Option<VcsGitRepoInfo>> {
	zero2ai_vcs::git::discover_info(Path::new(&dir))
...
impl VcsGitRepo {
	...
	pub fn status_porcelain(
		...
		blocking("vcs.statusPorcelain", self.inner.clone(), signal, move |r| {
			r.status_porcelain(&options.map_err(|error| zero2ai_vcs::Error::Backend {
	...
	pub fn stage_hunks(
		...
		blocking("vcs.stageHunks", self.inner.clone(), signal, move |r| {
			r.stage_hunks(
				&selections.map_err(|error| zero2ai_vcs::Error::Backend {
	...
	pub fn reset(
		...
		blocking("vcs.reset", self.inner.clone(), signal, move |r| {
			r.reset(
				mode.map_err(|error| zero2ai_vcs::Error::Backend {
...
pub fn vcs_git_clone<'e>(
	...
	vcs_future(env, "vcs.clone", async move {
		zero2ai_vcs::git::clone(&url, Path::new(&target), &options, cancel).await
	})
}
...
pub fn vcs_detach_git_dir(
	...
	task::blocking_mapped("vcs.detachGitDir", ct, rich_error, move |ct| {
		...
		zero2ai_vcs::git::detach_git_dir(Path::new(&worktree_root), Path::new(&source_common_dir)).map(
			|v| {
				match v {
...
pub fn vcs_join_patches(parts: Vec<String>) -> String {
	zero2ai_vcs::git::join_patches(&parts)
}
...
pub fn vcs_validate_hunk_selections(
	...
	let selections: Vec<_> = selections
		...
		.collect();
	zero2ai_vcs::git::validate_hunk_selections(&raw_diff, &selections)
		.into_iter()
...
mod tests {
	...
	fn catch_panic_converts_native_panics_into_backend_errors() {
		let err = catch_panic("vcs.test", || -> zero2ai_vcs::Result<()> {
			panic!("valid name: Os {{ code: 1455 }}")
		})
		.unwrap_err();
		match err {
			zero2ai_vcs::Error::Backend { context, message } => {
