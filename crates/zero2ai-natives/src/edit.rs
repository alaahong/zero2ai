//! N-API surface for the Rust edit engine (`zero2ai_edit`).
//!
//! # Overview
...
impl EditStore {
	...
	pub fn record_snapshot(
		...
		let (_, body) = zero2ai_edit::text::strip_bom(&text);
		self
			.inner
...
pub fn edit_inspect(mode: String, args_json: String) -> Result<EditInspection> {
	...
	let engine = zero2ai_edit::modes::engine_for(mode, true, 0.95, false);
	...
	Ok(EditInspection {
		...
		file_ops: inspection
			...
			.into_iter()
			.map(|op| match op {
				zero2ai_edit::FileOpIntent::Delete { path } => {
					EditFileOpIntent { kind: "delete".into(), path, to: None }
				},
				zero2ai_edit::FileOpIntent::Move { from, to } => {
...
pub fn edit_description(mode: String) -> Result<String> {
	Ok(zero2ai_edit::description(parse_mode(&mode)?).to_owned())
}

/// Lark grammar for `mode`, when it has a custom wire format.
#[napi]
pub fn edit_grammar(mode: String) -> Result<Option<String>> {
	Ok(zero2ai_edit::grammar(parse_mode(&mode)?).map(str::to_owned))
}
...
pub fn notebook_to_editable_text(json: String, display_path: String) -> Result<String> {
	zero2ai_edit::notebook::notebook_to_editable_text(&json, &display_path).map_err(reason)
}
