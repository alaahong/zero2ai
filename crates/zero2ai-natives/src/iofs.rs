...
//!
//! `zero2ai-walker` owns traversal and cache policy. This module keeps only the
...
fn walker_error_to_napi<E: std::fmt::Display>(err: zero2ai_walker::WalkError<E>) -> Error {
	match err {
		zero2ai_walker::WalkError::Interrupted(err) => Error::from_reason(err.to_string()),
		zero2ai_walker::WalkError::InvalidData { path, message } => Error::from_reason(format!(
...
pub(crate) const fn from_walker_file_type(file_type: zero2ai_walker::FileType) -> FileType {
	match file_type {
		zero2ai_walker::FileType::File => FileType::File,
		zero2ai_walker::FileType::Dir => FileType::Dir,
		zero2ai_walker::FileType::Symlink => FileType::Symlink,
	}
}

impl From<zero2ai_walker::CollectedEntry> for GlobMatch {
	fn from(entry: zero2ai_walker::CollectedEntry) -> Self {
		Self {
			path:      entry.path,
...
pub(crate) fn map_walker_error<E: std::fmt::Display>(err: zero2ai_walker::WalkError<E>) -> Error {
...
pub fn invalidate_fs_scan_cache(path: Option<JsString>) -> Result<()> {
	match path {
		Some(path) => zero2ai_walker::invalidate_path_string(&js::utf8(path)?),
		None => zero2ai_walker::invalidate_all(),
	}
	Ok(())
}
