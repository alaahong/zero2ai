...
//!
//! # Overview
//! Resolves a search root, scans entries via `zero2ai-walker`, applies glob matching
...
fn collect_ranked_matches(
	...
	let outcome = request
		.collect_ranked_with_heartbeat(
			zero2ai_walker::WalkRank::MtimeDescPathAsc,
			config.max_results,
			|| ct.heartbeat(),
		)
...
fn run_glob(
	...
	let walk_glob = zero2ai_walker::CompiledWalkGlob::new([walk_glob_pattern])
	...
	let base_request = zero2ai_walker::WalkRequest::new(config.root.clone())
		...
		.cache(config.cache)
		.empty_recheck(zero2ai_walker::EmptyRecheck::Configured)
		.filter(
			zero2ai_walker::WalkFilter::all()
				.glob(walk_glob)
...
pub fn glob(
	...
	task::blocking("glob", ct, move |ct| {
		run_glob(
			GlobConfig {
				root: zero2ai_walker::resolve_search_path(&path).map_err(iofs::map_walker_error)?,
...
mod tests {
	...
	impl TempDirGuard {
		fn new() -> Self {
			...
			let path = std::env::temp_dir().join(format!("zero2ai-glob-test-{timestamp}-{counter}"));
