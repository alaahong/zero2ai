...
static WALK_WORKERS: LazyLock<usize> = LazyLock::new(|| {
	normalize_worker_count(env_uint("ZERO2AI_WALK_WORKERS", DEFAULT_WALK_WORKERS, 0, usize::MAX))
});
static WALK_POOL: LazyLock<Option<ThreadPool>> = LazyLock::new(|| {
	...
	rayon::ThreadPoolBuilder::new()
		.num_threads(workers)
		.thread_name(|index| format!("zero2ai-walker-{index}"))
		.build()
		.ok()
});
...
///
/// `ZERO2AI_WALK_WORKERS=0` means auto-detect; `ZERO2AI_WALK_WORKERS=1` forces serial
/// work.
...
mod tests {
	...
	impl TempDirGuard {
		fn new() -> Self {
			...
			let path = std::env::temp_dir().join(format!("zero2ai-fs-cache-test-{timestamp}-{counter}"));
