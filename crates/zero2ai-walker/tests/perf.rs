//! Ignored deterministic timing harness for zero2ai-walker.
//!
//! Run with:
//! cargo nextest run --cargo-profile ci -p zero2ai-walker --test perf \
...
use zero2ai_walker::{WalkDetail, WalkOrder, WalkRequest};
...
#[test]
#[ignore = "run with: cargo nextest run --cargo-profile ci -p zero2ai-walker --test perf --run-ignored \
...
fn build_synthetic_tree() -> PathBuf {
	let root = unique_temp_root("zero2ai-walker-perf");
