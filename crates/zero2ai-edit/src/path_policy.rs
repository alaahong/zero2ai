...
impl PathPolicy {
	...
	pub fn recover_missing(&self, authored: &str) -> Option<Resolved> {
		...
		let glob = zero2ai_walker::CompiledWalkGlob::new([format!("**/{escaped}")]).ok()?;
		let started = Instant::now();
		let request = zero2ai_walker::WalkRequest::new(&self.cwd)
			.hidden(true)
			.gitignore(true)
			.skip_git(true)
			...
			.cache(false)
			.limit(2)
			.filter(zero2ai_walker::WalkFilter::all().glob(glob));
		let result = request
