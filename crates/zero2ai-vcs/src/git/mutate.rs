...
impl GitRepo {
	...
	pub fn worktree_add(
		...
		if let Some(preferred) = preferred {
			for kind in zero2ai_iso::clone_candidates(preferred) {
...
mod tests {
	...
	fn clone_first_worktree_reconciles_source_state_to_target_tree() {
		...
		if result.cloned_with.is_some() {
			...
			assert!(
				result.clone_error.is_some() || zero2ai_iso::clone_candidates(None).is_empty(),
	...
	/// on `dir`'s filesystem. `zero2ai_iso::clone_candidates` is a host-level
	...
	fn clone_works_on(dir: &Path) -> bool {
		...
		let works = zero2ai_iso::clone_candidates(None).into_iter().any(|kind| {
