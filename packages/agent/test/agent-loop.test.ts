...
} from "@zero2ai/agent-core/agent-loop";
import { SpeculativeOperationCoordinator } from "@zero2ai/agent-core/speculative-execution";
import type {
	AgentContext,
	AgentEvent,
	AgentLoopConfig,
	...
	ToolCallContext,
} from "@zero2ai/agent-core/types";
import { ASIDE_MESSAGE_COMMIT, ASIDE_MESSAGE_DISCARD, SPECULATIVE_STREAM_SESSION } from "@zero2ai/agent-core/types";
import type { AssistantMessage, AssistantMessageEvent, Context, Message, ToolResultMessage } from "@zero2ai/ai";
...
declare module "@zero2ai/agent-core/types" {
...
describe("agentLoop kCursorExecResolved (issue #4348)", () => {
	it("skips execute for a toolCall block marked as already run by Cursor's exec channel", async () => {
		const { kCursorExecResolved } = await import("@zero2ai/ai/utils/block-symbols");
