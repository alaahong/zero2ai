...
installStatsTestIsolation("@zero2ai-stats-port-conflict-");
...
async function startBunHolder(responseExpr: string, options?: { hostname?: string; statsOwned?: boolean }) {
	...
	if (options?.statsOwned) args.push("zero2ai-stats");
...
describe("startServer port conflicts", () => {
	...
	it("refuses to stop a newer dashboard without command identity", async () => {
		...
		await expect(startServer(holder.port)).rejects.toThrow("not identifiable as an zero2ai stats dashboard");
