 ...
 *
 * The default profile's stats.db is redirected to `$XDG_DATA_HOME/zero2ai/stats.db`
 * by {@link DirResolver} whenever `agentDirOverride === defaultAgent`. Tests
 * that only set `ZERO2AI_CONFIG_DIR` + `setAgentDir(<home>/<config>/agent)` resolve
 ...
 * `ZERO2AI_CONFIG_DIR` for the test, points the agent directory at a fresh
...
import { closeDb } from "@zero2ai/stats/db";
