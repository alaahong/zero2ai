...
```bash
npm install @zero2ai/ai
```

## Quick Start

```typescript
import { getModel, stream, complete, Context, Tool, type } from "@zero2ai/ai";

// Fully typed with auto-complete support for both providers and models
const model = getModel("openai", "gpt-4o-mini");

// Define tools with schema schemas for type safety and validation
const tools: Tool[] = [
	{
		name: "get_time",
...
```typescript
import { type Tool, type } from "@zero2ai/ai";
...
When using `agentLoop`, tool arguments are automatically validated against their schema schemas before execution. Validation failures are returned to the model as tool results so it can retry.

When implementing your own tool execution loop with `stream()` or `complete()`, use `validateToolCall` to validate arguments before passing them to your tools:

```typescript
import { stream, validateToolCall, Tool } from "@zero2ai/ai";
...
```typescript
import * as fs from "node:fs";
import { getModel, complete } from "@zero2ai/ai";
...
```typescript
import { getModel, streamSimple, completeSimple } from "@zero2ai/ai";
...
```typescript
import { getModel, stream } from "@zero2ai/ai";
...
```typescript
import { getProviders, getModels, getModel } from "@zero2ai/ai";
...
```typescript
import { Model, stream } from "@zero2ai/ai";
...
```typescript
import { getModel, complete, Context } from "@zero2ai/ai";
...
```typescript
import { Context, getModel, complete } from "@zero2ai/ai";
...
`/login cloudflare-ai-gateway` collects and stores the gateway token, account ID, and gateway ID. For environment configuration, set all three Cloudflare values above. ZERO2AI derives provider endpoints from the account and gateway IDs.
...
bundles. Bun's `fetch` does not consume the env var natively, so zero2ai injects
...
```typescript
import { getEnvApiKey } from "@zero2ai/ai";
...
Authenticate via the [`zero2ai`](https://omp.sh) coding-agent CLI, which drives this library's OAuth/API-key flows in-process and persists into `agent.db`:

```bash
zero2ai auth-broker login              # interactive provider selection
zero2ai auth-broker login anthropic    # login to a specific provider
zero2ai auth-broker login vllm         # store vLLM API key (or placeholder for local no-auth)
zero2ai auth-broker list               # list supported providers
zero2ai auth-broker logout             # interactive — pick a stored credential to remove
```
...
```typescript
import { getProviderDefinition } from "@zero2ai/ai";
import * as fs from "node:fs";
...
```typescript
import { getModel, complete, getOAuthApiKey } from "@zero2ai/ai";
import * as fs from "node:fs";
