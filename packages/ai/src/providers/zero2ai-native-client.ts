/**
 * Client half of the zero2ai-native auth-gateway protocol.
 *
 * Dispatches a {@link streamSimple}-shaped request to an `zero2ai auth-gateway`
 ...
 * way.
 *
 * Activated when a {@link Model} has `transport: "zero2ai-native"` set; the
 * dispatch hook lives in `streamSimple()` (see `../stream.ts`). Used by
 * containerized zero2ai deployments (such as robomp slots) that
 * route every LLM call through a credential-holding sidecar so the slot
 * itself stays credential-free.
 */
import * as os from "node:os";
import { getAppName, getInstallId, readSseJson } from "@zero2ai/utils";
 ...
 * the baseUrl is missing (transport=zero2ai-native without a gateway target is
 * a configuration error, not a runtime recoverable one).
 */
function resolveStreamUrl(model: Model<Api>): string {
	if (!model.baseUrl) {
		throw new AIError.ConfigurationError(
			`zero2ai-native transport requires \`baseUrl\` on model ${model.id} (set it on the provider config in models.yml)`,
		);
	}
...
function buildHeaders(model: Model<Api>, apiKey: string | undefined): Record<string, string> {
	const headers: Record<string, string> = {
		...
		// Attribution-only — the gateway never forwards x-zero2ai-* upstream. Header
		// values must stay ISO-8859-1-safe, hence the hostname scrub.
		"x-zero2ai-install-id": getInstallId(),
		"x-zero2ai-hostname": os.hostname().replace(/[^\x20-\x7e]/g, "?"),
		"x-zero2ai-app": getAppName(),
		...model.headers,
	};
...
/**
 * Stream a turn through an `zero2ai auth-gateway` over the zero2ai-native protocol.
 *
	...
	void (async () => {
		...
		try {
			...
			const watchedSource = iterateWithIdleTimeout(source, {
				...
				errorMessage: ZERO2AI_NATIVE_STREAM_IDLE_TIMEOUT_ERROR,
				firstItemErrorMessage: ZERO2AI_NATIVE_STREAM_FIRST_EVENT_TIMEOUT_ERROR,
				onIdle: () =>
