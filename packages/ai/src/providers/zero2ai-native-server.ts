 ...
 * wire shapes through zero2ai-ai's canonical {@link Context}, this module accepts
 * the canonical shape *directly* — for clients that already speak zero2ai-ai
 * (containerized zero2ai, robomp's sidecar auth-gateway).
 ...
 * translations impose on first-class zero2ai-ai fields (service tier, cache
 * markers, thinking budgets, tool-choice variants, …).
 *
 * The streaming wire is {@link AssistantMessageEvent} serialized verbatim and
 * SSE-framed. Same type zero2ai-ai already produces internally; the client feeds
 ...
 * `SimpleStreamOptions` from older / newer zero2ai builds without per-version
 * conditionals.
 */
...
/**
 * Parse a zero2ai-native request body. Validation is intentionally minimal — only
 * the shape the gateway itself reads is checked (`modelId`, `context.messages`
 * array, options is an object). Everything downstream is the canonical zero2ai-ai
...
export function parseRequest(body: unknown, _headers?: Headers): PiNativeParsedRequest {
	...
	// `stream` defaults to true — zero2ai-native clients overwhelmingly stream, and
 ...
 *
 * No per-event re-shaping: the zero2ai-native client is zero2ai-ai itself, so the
 ...
 * targets (containerized zero2ai → host gateway, robomp slot → zero2ai-auth-gateway
 ...
 * provider-specific status taxonomy — zero2ai-native callers consume `type`
 * directly.
 */
