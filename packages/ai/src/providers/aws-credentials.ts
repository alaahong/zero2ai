...
import { $env, isEnoent, logger } from "@zero2ai/utils";
...
async function stsAssumeRole(
	...
	const body = new URLSearchParams({
		...
		Version: "2011-06-15",
		RoleArn: roleArn,
		RoleSessionName: opts.sessionName || `zero2ai-${process.pid}`,
	});
...
async function assumeRoleWithWebIdentity(
	...
	const body = new URLSearchParams({
		...
		RoleSessionName: params.sessionName || `zero2ai-${process.pid}`,
