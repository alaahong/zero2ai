 ...
 *
 * `ZERO2AI_OPENAI_STREAM_IDLE_TIMEOUT_MS` is accepted as a backward-compatible alias.
 * Set `ZERO2AI_STREAM_IDLE_TIMEOUT_MS=0` to disable the watchdog.
 *
 ...
 *
 * `ZERO2AI_OPENAI_STREAM_IDLE_TIMEOUT_MS` takes precedence over the generic
 * `ZERO2AI_STREAM_IDLE_TIMEOUT_MS` because some deployments tune OpenAI-compatible
 ...
 *
 * Set `ZERO2AI_STREAM_FIRST_EVENT_TIMEOUT_MS=0` to disable the watchdog.
 *
...
export function getStreamFirstEventTimeoutMs(
	...
	return normalizeIdleTimeoutMs($env.ZERO2AI_STREAM_FIRST_EVENT_TIMEOUT_MS, fallback);
}

/**
 * Returns the first-event timeout used for OpenAI-family streaming transports.
 *
 * Precedence: explicit `ZERO2AI_OPENAI_STREAM_FIRST_EVENT_TIMEOUT_MS` (including a
 ...
 * `streamIdleTimeoutMs` or `ZERO2AI_OPENAI_STREAM_IDLE_TIMEOUT_MS` resolved
 * upstream) floors the first-event budget so slow OpenAI-compatible servers
 * are not undercut by a shorter `ZERO2AI_STREAM_FIRST_EVENT_TIMEOUT_MS` or the
...
export function getOpenAIStreamFirstEventTimeoutMs(
	...
	const openAIFirstEventRaw = $env.ZERO2AI_OPENAI_STREAM_FIRST_EVENT_TIMEOUT_MS;
	if (openAIFirstEventRaw !== undefined) {
		return normalizeIdleTimeoutMs(openAIFirstEventRaw, fallbackMs) ?? 0;
	}
	const base = normalizeIdleTimeoutMs($env.ZERO2AI_STREAM_FIRST_EVENT_TIMEOUT_MS, fallbackMs);
