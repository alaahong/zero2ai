...
import { clearCustomApis, registerCustomApi } from "@zero2ai/ai/api-registry";
import { stream, streamSimple } from "@zero2ai/ai/stream";
import type { AssistantMessage, FetchImpl, Model, ModelSpec } from "@zero2ai/ai/types";
import { AssistantMessageEventStream } from "@zero2ai/ai/utils/event-stream";
import { transportFetch } from "@zero2ai/ai/utils/transport-fetch";
...
beforeEach(async () => {
	previousDebugFlag = Bun.env.ZERO2AI_REQ_DEBUG;
	previousCwd = process.cwd();
	tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zero2ai-req-debug-"));
...
afterEach(async () => {
	...
	if (previousDebugFlag === undefined) delete Bun.env.ZERO2AI_REQ_DEBUG;
	else Bun.env.ZERO2AI_REQ_DEBUG = previousDebugFlag;
...
/** Find the latest rr-session-*.json written to the temp dir by ZERO2AI_REQ_DEBUG
...
describe("ZERO2AI_REQ_DEBUG request/response recording", () => {
	it("writes nothing when the flag is disabled", async () => {
		delete Bun.env.ZERO2AI_REQ_DEBUG;
