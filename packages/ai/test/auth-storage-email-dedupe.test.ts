...
import { AuthStorage, type FetchImpl, type OAuthCredential, SqliteAuthCredentialStore } from "@zero2ai/ai";
...
describe("AuthStorage openai-codex email dedupe", () => {
	...
	beforeEach(async () => {
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zero2ai-ai-auth-email-dedupe-"));
...
describe("AuthStorage OAuth login upgrade and multi-account coexistence", () => {
	...
	beforeEach(async () => {
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zero2ai-auth-login-test-"));
	});
...
describe("AuthStorage persistent session stickiness", () => {
	...
	beforeEach(async () => {
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zero2ai-auth-test-"));
