...
import { streamPiNative } from "@zero2ai/ai/providers/zero2ai-native-client";
...
describe("streamPiNative request shape", () => {
	...
	it("forwards a model-configured User-Agent override across the zero2ai-native wire", async () => {
		const captured: { init?: RequestInit } = {};
		const fetchImpl: FetchImpl = (async (_input, init) => {
			captured.init = init;
			return fakeResponse([{ type: "done", reason: "stop", message: baseAssistant() }]);
	...
	it("forwards `model.headers` and lets a caller-supplied Authorization win", async () => {
		...
		await streamPiNative(
			fakeModel({ headers: { "x-zero2ai-slot": "robomp-1", Authorization: "Bearer model-wins" } }),
			baseContext,
			{ apiKey: "options-loses", fetch: fetchImpl },
		).result();

		const headers = captured.init?.headers as Record<string, string>;
		expect(headers["x-zero2ai-slot"]).toBe("robomp-1");
...
describe("streamPiNative event flow", () => {
	...
	it("uses ZERO2AI_STREAM_FIRST_EVENT_TIMEOUT_MS for silent zero2ai-native streams", async () => {
		const previous = Bun.env.ZERO2AI_STREAM_FIRST_EVENT_TIMEOUT_MS;
		...
		try {
			...
			if (previous === undefined) {
				...
				Bun.env.ZERO2AI_STREAM_FIRST_EVENT_TIMEOUT_MS = previous;
			}
		}
	});

	it("rejects when a zero2ai-native stream stalls after semantic progress", async () => {
		const partial = baseAssistant({ content: [{ type: "text", text: "hi" }] });
		const chunks = [
	...
	it("does not time out a healthy zero2ai-native stream that keeps making semantic progress", async () => {
	...
	it("rejects when the SSE stream closes before a terminal event", async () => {
		...
		expect(error).toMatchObject({
			message: "zero2ai-native stream read error: stream closed before a terminal response event",
			provider: "anthropic",
