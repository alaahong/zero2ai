...
import { completeSimple, stream } from "@zero2ai/ai";
import { buildAnthropicClientOptions } from "@zero2ai/ai/providers/anthropic";
import { resolveOpenAIRequestSetup } from "@zero2ai/ai/providers/openai-shared";
...
describe("opencode and gpt session header on OpenAI transports", () => {
	...
	it("applies zero2ai's common User-Agent as the global inference default", async () => {
...
describe("usage fetch carries attribution headers", () => {
	it("sends User-Agent and a stable x-opencode-session on usage polls", async () => {
		...
		// OpenCode attribution working (required from 09/06), and zero2ai's UA
