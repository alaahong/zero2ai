 ...
 *     → anthropic-messages parser → zero2ai Context (cacheRetention derived)
 *     → zero2ai-ai openai-codex-responses provider
 ...
 * directly — caching only works if zero2ai-ai's codex transport reaches Codex
 ...
 * (override via `ZERO2AI_E2E_GATEWAY_URL`) AND the bearer token file exists at
 * `~/.zero2ai/auth-gateway.token`.
 *
...
const MODEL = Bun.env.ZERO2AI_E2E_CODEX_MODEL ?? "gpt-5.3-codex";
...
the zero2ai auth-gateway's cross-protocol prompt-caching pipeline. The request
...
describe.skipIf(!gateway.ok)("auth-gateway: anthropic-messages → openai-codex caching e2e", () => {
	...
	it("caches the system prefix across a cross-protocol translate (messages→codex)", async () => {
		...
		//   - the anthropic-messages encoder forgot to map zero2ai-ai's
