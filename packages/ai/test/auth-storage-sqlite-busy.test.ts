 ...
 *
 * Concurrent zero2ai startups can race against WAL recovery so the auth-store init
...
import { isSqliteBusyError, SqliteAuthCredentialStore } from "@zero2ai/ai/auth-storage";
...
describe("SqliteAuthCredentialStore.open SQLITE_BUSY handling", () => {
	...
	beforeEach(async () => {
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zero2ai-ai-sqlite-busy-"));
	});
