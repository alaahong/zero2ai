...
} from "@zero2ai/ai/utils/idle-iterator";
...
const ENV_KEYS = [
	...
	"ZERO2AI_OPENAI_STREAM_FIRST_EVENT_TIMEOUT_MS",
] as const;
...
describe("getStreamIdleTimeoutMs(fallbackMs)", () => {
	...
	it("lets ZERO2AI_STREAM_IDLE_TIMEOUT_MS override the per-provider fallback", () => {
		Bun.env.ZERO2AI_STREAM_IDLE_TIMEOUT_MS = "42";
		expect(getStreamIdleTimeoutMs(300_000)).toBe(42);
	});

	it("treats ZERO2AI_STREAM_IDLE_TIMEOUT_MS=0 as a watchdog disable", () => {
		Bun.env.ZERO2AI_STREAM_IDLE_TIMEOUT_MS = "0";
...
describe("getOpenAIStreamIdleTimeoutMs(fallbackMs)", () => {
	...
	it("lets ZERO2AI_OPENAI_STREAM_IDLE_TIMEOUT_MS override the fallback before the generic env var", () => {
		Bun.env.ZERO2AI_STREAM_IDLE_TIMEOUT_MS = "42";
		Bun.env.ZERO2AI_OPENAI_STREAM_IDLE_TIMEOUT_MS = "84";
		expect(getOpenAIStreamIdleTimeoutMs(600_000)).toBe(84);
	});

	it("treats ZERO2AI_OPENAI_STREAM_IDLE_TIMEOUT_MS=0 as a watchdog disable", () => {
		Bun.env.ZERO2AI_OPENAI_STREAM_IDLE_TIMEOUT_MS = "0";
...
describe("getStreamFirstEventTimeoutMs(idleTimeoutMs, fallbackMs)", () => {
	...
	it("lets ZERO2AI_STREAM_FIRST_EVENT_TIMEOUT_MS override the per-provider fallback", () => {
		Bun.env.ZERO2AI_STREAM_FIRST_EVENT_TIMEOUT_MS = "42";
		expect(getStreamFirstEventTimeoutMs(undefined, 300_000)).toBe(42);
	});

	it("treats ZERO2AI_STREAM_FIRST_EVENT_TIMEOUT_MS=0 as a watchdog disable", () => {
		Bun.env.ZERO2AI_STREAM_FIRST_EVENT_TIMEOUT_MS = "0";
...
describe("getOpenAIStreamFirstEventTimeoutMs(idleTimeoutMs, fallbackMs)", () => {
	...
	it("honors ZERO2AI_OPENAI_STREAM_FIRST_EVENT_TIMEOUT_MS even when caller pins per-call idle", () => {
		Bun.env.ZERO2AI_OPENAI_STREAM_FIRST_EVENT_TIMEOUT_MS = "42";
		Bun.env.ZERO2AI_STREAM_FIRST_EVENT_TIMEOUT_MS = "20";
		expect(getOpenAIStreamFirstEventTimeoutMs(5_000, 100_000)).toBe(42);
	});

	it("treats ZERO2AI_OPENAI_STREAM_FIRST_EVENT_TIMEOUT_MS=0 as an explicit watchdog disable", () => {
		Bun.env.ZERO2AI_OPENAI_STREAM_FIRST_EVENT_TIMEOUT_MS = "0";
	...
	it("respects ZERO2AI_STREAM_FIRST_EVENT_TIMEOUT_MS=0 disable when no OpenAI override is set", () => {
		Bun.env.ZERO2AI_STREAM_FIRST_EVENT_TIMEOUT_MS = "0";
		expect(getOpenAIStreamFirstEventTimeoutMs(1500, 100_000)).toBe(0);
	});
});
