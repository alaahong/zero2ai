...
import { isRecord, logger, postmortem, ptree, setProcessName } from "@zero2ai/utils";
...
export class LspMuxServer {
	...
	async #handleServerRequest(server: ServerInstance, message: LspJsonRpcRequest): Promise<void> {
		...
		// Client-side effects such as applyEdit must reach exactly one, most recently active zero2ai.
...
export async function startLspMuxFromEnvironment(): Promise<void> {
	...
	setProcessName("zero2ai lsp mux");
