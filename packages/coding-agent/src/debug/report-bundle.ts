...
import type { WorkProfile } from "@zero2ai/natives";
import { APP_NAME, getLogPath, getLogsDir, getReportsDir, isEnoent } from "@zero2ai/utils";
...
export async function createReportBundle(options: ReportBundleOptions): Promise<ReportBundleResult> {
	...
	const outputPath = path.join(reportsDir, `zero2ai-report-${timestamp}.tar.gz`);
 ...
 * after a crash still captures the fatal PID's `zero2ai.<date>.<pid>.log`. Files
