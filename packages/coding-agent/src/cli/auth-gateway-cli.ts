/**
 * `zero2ai auth-gateway` command handlers.
 *
 ...
 * `ZERO2AI_AUTH_BROKER_URL` / `auth.broker.url` precedence used elsewhere).
 *
 * Sub-verbs:
...
import { DEFAULT_AUTH_GATEWAY_BIND, startAuthGateway } from "@zero2ai/ai/auth-gateway";
import { type GeneratedProvider, getBundledModels } from "@zero2ai/catalog/models";
import { getConfigRootDir, isEnoent, logger, VERSION } from "@zero2ai/utils";
...
async function runServe(flags: AuthGatewayCommandArgs["flags"]): Promise<void> {
	...
	if (!brokerConfig) {
		throw new Error(
			"`zero2ai auth-gateway serve` requires ZERO2AI_AUTH_BROKER_URL (or `auth.broker.url`/`auth.broker.token` in config.yml). The gateway is itself a broker client.",
		);
	}
	...
	// cached + broker-discovered), so the discovery-only models zero2ai itself
	...
	// a client-requested `model` field into a zero2ai-ai `Model<Api>` before dispatch;
...
async function runStatus(flags: AuthGatewayCommandArgs["flags"]): Promise<void> {
	...
	if (!brokerConfig) {
		...
		if (flags.json) {
			...
			process.stdout.write(`${chalk.yellow("No broker configured.")} Set ZERO2AI_AUTH_BROKER_URL.\n`);
			process.stdout.write(
	...
	try {
		...
		if (flags.json) {
			...
			if (!tokenPresent) {
				process.stdout.write(
					"Run `zero2ai auth-gateway token` or `zero2ai auth-gateway serve` to create a bearer token.\n",
				);
			}
		}
 ...
 * - `cursor-agent` and `zero2ai-native` (gateway forwarding) have transport quirks
 ...
 * zero2ai-native transport (would loop through the gateway), and placeholder /
...
function pickProbeCandidates(provider: string): Model<Api>[] {
	...
	const candidates = bundled.filter(model => {
		if (model.transport === "zero2ai-native") return false;
...
/**
 * `zero2ai auth-gateway check` — probe each broker-supplied credential and print
...
async function runCheck(flags: AuthGatewayCommandArgs["flags"]): Promise<void> {
	...
	if (!brokerConfig) {
		throw new Error(
			"`zero2ai auth-gateway check` requires ZERO2AI_AUTH_BROKER_URL (or `auth.broker.url`/`auth.broker.token` in config.yml). It probes the same credentials the gateway would serve.",
		);
	}
