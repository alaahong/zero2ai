 ...
 *
 * Handles `zero2ai plugin <command>` subcommands for plugin lifecycle management.
 */

import * as path from "node:path";
import { APP_NAME, getPluginsNodeModules, getProjectDir } from "@zero2ai/utils";
	...
	try {
		if (pluginId) {
			...
			if (flags.scope) {
				console.error(
					chalk.yellow(
						`Warning: --scope is ignored when upgrading all plugins. Use 'zero2ai plugin upgrade <id> --scope ${flags.scope}' to target a specific plugin and scope.`,
					),
				);
			}
...
async function handleInstall(
	...
	if (packages.length === 0) {
		...
		console.error(chalk.dim(`  ${APP_NAME} plugin install @zero2ai/exa`));
	...
	for (const spec of packages) {
		...
		if (target.type === "local") {
			...
			// `zero2ai plugin link <path>` so users can use either verb interchangeably.
			if (flags.scope) {
				console.error(
					chalk.yellow(
...
${chalk.bold("Examples:")}
  ${APP_NAME} plugin install @zero2ai/exa[search]
