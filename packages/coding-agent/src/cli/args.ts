...
import { $env, APP_NAME, logger } from "@zero2ai/utils";
 ...
 * setter table itself can stay free of `@zero2ai/utils` runtime imports
...
export function parseArgs(inputArgs: string[], extensionFlags?: Map<string, { type: "boolean" | "string" }>): Args {
	...
	const result: Args = {
		...
		unrecognizedFlags: [],
		sessionDir: $env.ZERO2AI_CODING_AGENT_SESSION_DIR || undefined,
	};
