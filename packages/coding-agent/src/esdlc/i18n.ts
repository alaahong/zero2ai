...
const EN = {
	...
	"cli.askSkipped":
		'stdin is not a terminal, so this question is being skipped — re-run with {flag} "<answer>" to supply it, or answer in a terminal.',
	"cli.askSourceClosed":
		'stdin closed before an answer arrived — re-run with {flag} "<answer>" (or answer in a terminal) to supply it.',
	"screen.keys": "up/down select · Enter run · r refresh · q quit",
	"screen.enterToRun": "Enter to run · Esc to cancel",
	"screen.answerKeys": "Enter to send the answer · Esc to skip",
	...
	"live.title": "Live run detail",
	"live.hint": "Output streams here while the stage runs; the tail is what the command printed, in order.",
	"live.follow": "Follow",
	"live.elapsed": "elapsed {duration}",
	"live.streaming": "streaming",
	"live.truncated": "earlier output trimmed",
	"live.empty": "(no output yet)",
	"live.loading": "loading…",

	"graph.tab": "Code graph",
	"graph.title": "Change impact",
	"graph.hint":
		"Derived live from the project's own import statements and the uncommitted change set; click a node to open the file.",
	"graph.loading": "Computing impact from the working tree…",
	"graph.changedCount": "{count} changed",
	"graph.impactedCount": "{count} impacted",
	"graph.dependencyCount": "{count} dependencies",
	"graph.scanned": "from {count} source file(s)",
	"graph.capped": "file cap reached",
	"graph.empty": "No build report yet — run the build stage to compute impact.",
	"graph.none": "This build produced no code graph (no changed source files).",
	"graph.noneInDirection": "Nothing in this direction.",
	"graph.impactList": "Impacted by the change (transitively imports a changed file)",
	"graph.dependencyList": "The change depends on",
	"graph.external": "External packages the change relies on",
	"graph.depth": "depth {depth}",
	"graph.changedTag": "changed",
	"config.title": "Config",
...
const ZH: Record<EsdlcMessageKey, string> = {
	...
	"cli.askHuman": "需要人工确认",
	"cli.askSkipped": '当前 stdin 不是终端，该问题将被跳过——可加 {flag} "<回答>" 重新运行，或在终端中回答。',
	"cli.askSourceClosed": 'stdin 在收到回答前已关闭——可加 {flag} "<回答>" 重新运行（或在终端中回答）。',
	"screen.keys": "↑↓ 选择 · Enter 运行 · r 刷新 · q 退出",
	"screen.enterToRun": "Enter 运行 · Esc 取消",
	"screen.answerKeys": "Enter 提交回答 · Esc 跳过",
	...
	"live.title": "实时运行细节",
	"live.hint": "阶段运行期间输出会实时流到这里；显示的是命令按顺序打印的最后一段。",
	"live.follow": "跟随最新",
	"live.elapsed": "已用 {duration}",
	"live.streaming": "流式输出中",
	"live.truncated": "较早输出已截断",
	"live.empty": "（暂无输出）",
	"live.loading": "加载中…",

	"graph.tab": "代码图",
	"graph.title": "变更影响",
	"graph.hint": "实时由项目自身的 import 关系与当前未提交变更派生；点击节点可打开该文件。",
	"graph.loading": "正在根据工作区计算影响面…",
	"graph.changedCount": "改动 {count}",
	"graph.impactedCount": "受影响 {count}",
	"graph.dependencyCount": "依赖 {count}",
	"graph.scanned": "来自 {count} 个源文件",
	"graph.capped": "已达文件数上限",
	"graph.empty": "尚无构建报告——运行构建阶段后即可计算影响面。",
	"graph.none": "本次构建未产出代码图（没有改动的源文件）。",
	"graph.noneInDirection": "该方向没有文件。",
	"graph.impactList": "受变更影响（传递性 import 了改动文件）",
	"graph.dependencyList": "变更所依赖",
	"graph.external": "变更依赖的外部包",
	"graph.depth": "层级 {depth}",
	"graph.changedTag": "已改动",
	"config.title": "配置",
	"config.global": "全局设置",
