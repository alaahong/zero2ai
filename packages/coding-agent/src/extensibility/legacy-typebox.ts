...
import { IR_BRAND } from "@zero2ai/schema/ir";
import {
	type AnySchema,
	type ObjectOpts,
	Type as Zero2AiType,
	type TypeBuilder as Zero2AiTypeBuilder,
	type TUnsafe,
} from "@zero2ai/schema/typebox";
import { upgradeJsonSchemaTo202012, validateJsonSchemaValue } from "@zero2ai/ai/utils/schema";

export * from "@zero2ai/schema/typebox";
 ...
 * JSON Schema, lowering any embedded schema schema to its wire JSON. Legacy
 * Pi extensions were written against real TypeBox, whose `Type.*` builders
 * return plain JSON-Schema objects; schema's builders return callable schema
 ...
 *    `DataCloneError: The object can not be cloned.` (issue #8420) and schema
 *    would drop its `toJsonSchema()` override during composition anyway.
 *  - Spreading — `Type.Unsafe({ ...Schema, description })`. Spreading a
 *    callable copies schema's internal fields (`ir`, `run`, `$`, …) instead
 ...
 *    caller's own additions (everything not an schema internal) are overlaid.
 */
...
function unsafe<T = unknown>(jsonSchema: Record<string, unknown> = {}): LegacyUnsafeSchema<T> {
	...
	// schema builder to its wire form), so it doubles as the detaching clone.
...
const object = ((properties: Record<string, unknown>, opts?: ObjectOpts) => {
	...
	if (!hasRawProperty) {
		return Zero2AiType.Object(properties as Record<string, AnySchema>, normalizedOpts);
	}
	...
	if (additionalProperties !== undefined && typeof additionalProperties !== "boolean") {
		// schema index signatures validate every string key, including declared
		...
		const document = Zero2AiType.Object(normalizedProperties, objectOpts).toJsonSchema();
	...
	return Zero2AiType.Object(normalizedProperties, normalizedOpts);
}) as typeof Zero2AiType.Object;

export const Type = { ...Zero2AiType, Object: object, Unsafe: unsafe } as unknown as Zero2AiTypeBuilder;
export type TypeBuilder = Zero2AiTypeBuilder;

const legacyTypeBox: { Type: Zero2AiTypeBuilder } = { Type };
