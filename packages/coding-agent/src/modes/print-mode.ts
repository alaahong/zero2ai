 ...
 *
 * Used for:
 * - `zero2ai -p "prompt"` - text output
 * - `zero2ai --mode json "prompt"` - JSON event stream
 */
import type { AgentMessage } from "@zero2ai/agent-core";
import type { ImageContent } from "@zero2ai/ai";
import { logger, postmortem, sanitizeText } from "@zero2ai/utils";
...
async function runPrintModeCore(
	...
	// an ZERO2AI-owned Chromium cannot survive the exit (issue #5643).
	//
