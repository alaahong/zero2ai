...
import { $which, getToolsDir, logger, ptree, TempDir, USER_AGENT } from "@zero2ai/utils";
import { extractArchive } from "@zero2ai/utils/ar";
...
async function downloadTool(tool: ToolName, signal?: AbortSignal): Promise<string> {
	...
	// Extract
	const tmp = await TempDir.create("@zero2ai-tools-extract-");
