...
import type { Context, ImageContent, Message, Model, ProviderPayload, TextContent } from "@zero2ai/ai";
import { rasterizeSvg } from "@zero2ai/natives";
import {
	formatBytes,
	isRecord,
	logger,
 ...
 * to PNG/JPEG instead — the automatic equivalent of `ZERO2AI_NO_WEBP=1`.
 */
 ...
 * `ZERO2AI_NO_WEBP` env fallback in {@link resizeImage} still applies. Feed straight
...
export interface LoadImageInputOptions {
	...
	/** Force non-WebP output (e.g. for Ollama). Leave unset to honor `ZERO2AI_NO_WEBP`. */
	excludeWebP?: boolean;
}
