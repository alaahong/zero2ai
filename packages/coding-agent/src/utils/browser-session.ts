...
import { LoginCancelledError } from "@zero2ai/ai/error";
import type { OAuthBrowserSessionRequest } from "@zero2ai/ai/oauth/types";
import { logger, withTimeout } from "@zero2ai/utils";
...
export async function captureBrowserSession(
	...
	try {
		...
		userDataDir = await fs.mkdtemp(path.join(os.tmpdir(), "zero2ai-sso-profile-"));
