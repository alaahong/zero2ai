import { Args, type CommandMetadata, Flags } from "@zero2ai/utils/cli";
...
export const launchHelp = {
	...
	flags: {
		...
		smol: Flags.string({ description: "Smol/fast model for lightweight tasks (or ZERO2AI_SMOL_MODEL env)" }),
		slow: Flags.string({ description: "Slow/reasoning model for thorough analysis (or ZERO2AI_SLOW_MODEL env)" }),
		plan: Flags.string({ description: "Plan model for architectural planning (or ZERO2AI_PLAN_MODEL env)" }),
		...
		"from-claude": Flags.boolean({ description: "Import a Claude Code session into ZERO2AI" }),
		"from-codex": Flags.boolean({ description: "Import a Codex session into ZERO2AI" }),
	...
	examples: [
		...
		`# Create a shell shortcut for a work profile\n  ${APP_NAME} --profile work --alias zero2ai-work`,
		`# Use different model (fuzzy matching)\n  ${APP_NAME} --model opus "Help me refactor this code"`,
		`# Limit model cycling to specific models\n  ${APP_NAME} --models claude-sonnet,claude-haiku,gpt-4o`,
		`# Export a session file to HTML\n  ${APP_NAME} --export ~/.zero2ai/agent/sessions/--path--/session.jsonl`,
	],
