...
import { Args, CliUsageError, Command, Flags } from "@zero2ai/utils/cli";
...
export async function runOnboardingSetup(deps: OnboardingSetupDependencies = {}): Promise<void> {
	...
	if (!stdinIsTTY || !stdoutIsTTY) {
		(deps.writeStderr ?? (text => process.stderr.write(text)))("zero2ai setup requires an interactive TTY.\n");
