import type * as DomNs from "@zero2ai/utils/dom";
import type * as ReadabilityNs from "@zero2ai/utils/readability";
...
async function loadReadability(): Promise<typeof ReadabilityNs> {
	if (!readabilityModule) {
		readabilityModule = await import("@zero2ai/utils/readability");
	}
...
async function loadDom(): Promise<typeof DomNs> {
	if (!domModule) {
		domModule = await import("@zero2ai/utils/dom");
	}
	return domModule;
}
