 ...
 * lightweight CLI runner from zero2ai-utils.
 */
import type * as WorkerThreads from "node:worker_threads";
import type { MessagePort } from "node:worker_threads";
import type { Process, ProcessStatus } from "@zero2ai/natives";
import type { CliConfig, CommandMetadata } from "@zero2ai/utils/cli";
import type * as Postmortem from "@zero2ai/utils/postmortem";
import {
	APP_NAME,
	getActiveProfile,
	MIN_BUN_VERSION,
...
import { declareWorkerHostEntry, installWorkerInbox, isWorkerHostSelector } from "@zero2ai/utils/worker-host";
...
// the process entry, so the define-folded ZERO2AI_COMPILED marker stands in.
const isProcessEntry = import.meta.main || process.env.ZERO2AI_COMPILED === "true";
...
async function setFullProcessName(): Promise<void> {
	...
	const { setProcessName } = await import("@zero2ai/utils/process-name");
	setProcessName(APP_NAME);
}

/** Install ZERO2AI_PROXY handling before any command implementation can make a provider request. */
async function installNetworkBootstrap(): Promise<void> {
	...
	const { installGlobalProxyFetch } = await import("@zero2ai/ai/utils/proxy");
...
// `@zero2ai/utils/env` eagerly loads `.env` from the agent directory at
...
async function runSmokeTest(): Promise<void> {
	const { smokeTestSyncWorker, startServer } = await import("@zero2ai/stats");
...
const TINY_WORKER_ARG = "__zero2ai_worker_tiny_inference";
const STATS_SYNC_WORKER_ARG = "__zero2ai_worker_stats_sync";
const TAB_WORKER_ARG = "__zero2ai_worker_tab";
const JS_EVAL_WORKER_ARG = "__zero2ai_worker_js_eval";
const JS_EVAL_PROCESS_ARG = "__zero2ai_worker_js_eval_process";
const STT_WORKER_ARG = "__zero2ai_worker_stt";
const TTS_WORKER_ARG = "__zero2ai_worker_tts";
const MNEMOPI_EMBED_WORKER_ARG = "__zero2ai_worker_mnemopi_embed";
...
async function runWorkerEntrypoint(arg: string | undefined): Promise<boolean> {
	...
	if (arg === STATS_SYNC_WORKER_ARG) {
		...
		await import("@zero2ai/stats/sync-worker");
	...
	if (arg === JS_EVAL_PROCESS_ARG) {
		...
		const { interceptUnhandledRejections }: typeof Postmortem = require("@zero2ai/utils/postmortem.js");
...
async function runIpcSubprocessWorker<In, Out>(
	...
	if (process.platform === "win32" && initialParentPid <= 0) {
		...
		try {
			if (!process.env.ZERO2AI_TEST_NO_NATIVES) {
				const natives = await import("@zero2ai/natives");
				parentProcess = natives.Process.fromPid(initialParentPid);
				runningStatus = natives.ProcessStatus.Running;
			}
		} catch {}

		// Note on container environments (Docker/Kubernetes): zero2ai often runs as
 ...
 * detached process owning that model's socket (`ZERO2AI_TINY_WORKER_SOCKET`),
 * shared by every zero2ai process on the machine and exiting on its own when
 * idle. It exists so `onnxruntime-node` (loaded transitively by
 * `@huggingface/transformers`) never runs in an zero2ai address space — its NAPI
...
export async function runCli(argv: string[]): Promise<void> {
	...
	try {
		...
		if (extracted.profile !== undefined) {
			...
			// No explicit --profile: activate any ZERO2AI_PROFILE/ZERO2AI_PROFILE inherited
			...
			// ZERO2AI_PROFILE after importing this module (profile aliases/tests). Surfacing
			// validation here turns `ZERO2AI_PROFILE=.. zero2ai --version` into a clean error;
			// calling setProfile keeps every later path helper on the env-selected
			// profile instead of the default agent directory.
			setProfile(resolveProfileEnv(process.env.ZERO2AI_PROFILE, process.env.ZERO2AI_PROFILE));
		}
		if (extracted.aliasName !== undefined) {
			...
			if (!profile) {
				throw new Error("--alias requires --profile <name> or ZERO2AI_PROFILE");
			}
	...
	// `@zero2ai/utils/env` here would snapshot the wrong agent `.env`.
...
if (isProcessEntry || !Bun.isMainThread) {
	// A one-shot CLI run (`zero2ai --help | head`, `zero2ai --version | true`, `zero2ai <sub> | grep -m1`)
	...
	if (isProcessEntry) {
		const { registerStdioDisconnectHandling }: typeof Postmortem = require("@zero2ai/utils/postmortem.js");
	...
	runCli(process.argv.slice(2)).catch(async error => {
		...
		const { fatal } = await import("@zero2ai/utils/postmortem");
		fatal(error);
	});
}
