 ...
 *
 * Re-runs the `ZERO2AI_TIMING=x` cold-boot benchmark under hyperfine and fails when
 * the median regresses past `baseline * THRESHOLD`. `ZERO2AI_TIMING=x` runs the full
...
const BENCH_COMMAND = "ZERO2AI_TIMING=x ZERO2AI_STRICT_EDIT_MODE=1 bun src/cli.ts";
