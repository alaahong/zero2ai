 ...
 * `process.exit()` at the top level must not terminate the host ZERO2AI process.
 *
 * The harness intercepts the load via `withHostGuard`; this test pins that the
 * intercepted error surfaces as a per-module load failure (so ZERO2AI keeps going)
...
import { loadHooks } from "@zero2ai/coding-agent/extensibility/hooks/loader";
import { ExtensionExitError, withHostGuard } from "@zero2ai/coding-agent/extensibility/utils";
...
describe("extension/hook loader process.exit guard (#3680)", () => {
	...
	beforeEach(() => {
		project = TempDir.createSync("@zero2ai-exit-guard-");
	});
...
import { withHostGuard } from "@zero2ai/coding-agent/extensibility/utils";
