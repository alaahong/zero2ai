...
import { isArkSchema } from "@zero2ai/ai/utils/schema";
...
describe("applyToolProxy", () => {
	...
	it("preserves schema callables: parameters stay wire-detectable through the wrapper", () => {
		// Regression: schema schemas are plain functions carrying toJsonSchema/
	...
	it("preserves bind-capable schema callables from external arktype copies", () => {
		...
		// (unlike schema). Binding it returned a bare bound function with no
