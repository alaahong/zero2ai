...
// `@zero2ai/*` imports through virtual modules backed by live host module
...
describe("legacy-pi bundled virtual module synthesizer (issue #3423)", () => {
	...
	it("forwards `default` through `export default` so default imports survive", () => {
		const src = __synthesizeLegacyPiBundledSourceWithModules("@zero2ai/utils", modules);
	...
	it("throws when asked to synthesize a key the bundled modules do not cover", () => {
		expect(() => __synthesizeLegacyPiBundledSourceWithModules("@zero2ai/not-bundled", modules)).toThrow(
	...
	it("routes Bun plugin resolution through the bundled namespace so onLoad can serve extension imports", async () => {
		using tempDir = TempDir.createSync("@zero2ai-legacy-pi-bundled-virtual-");
		const entryPath = tempDir.join("extension-entry.ts");
		const bundlePath = tempDir.join("extension-entry.bundle.mjs");

		await Bun.write(
			entryPath,
			['export { legacyAnswer } from "zero2ai-legacy-pi-bundled:@zero2ai/utils";', ""].join("\n"),
		);

		expect(resolveBundledVirtualSpecifier("@zero2ai/utils")).toEqual({
			namespace: "zero2ai-legacy-pi-bundled",
			path: "@zero2ai/utils",
		});
		expect(resolveBundledVirtualSpecifier("zero2ai-legacy-pi-bundled:@zero2ai/utils")).toEqual({
			namespace: "zero2ai-legacy-pi-bundled",
			path: "@zero2ai/utils",
		});
		...
		const plugin: BunPlugin = {
			name: "zero2ai-legacy-pi-bundled-virtual-regression",
			setup(build) {
				build.onResolve({ filter: /^zero2ai-legacy-pi-bundled:.+$/, namespace: "file" }, args =>
					resolveBundledVirtualSpecifier(args.path),
				);
				build.onResolve({ filter: /.*/, namespace: "zero2ai-legacy-pi-bundled" }, args =>
					resolveBundledVirtualSpecifier(args.path),
				);
				build.onLoad({ filter: /.*/, namespace: "zero2ai-legacy-pi-bundled" }, args => {
		...
		expect(onLoadPaths).toEqual(["@zero2ai/utils"]);
		...
		expect(bundledModule.legacyAnswer).toBe("served:@zero2ai/utils");
	});
});
