...
// compiled-binary mode now routes through `zero2ai-legacy-pi-bundled:` virtual
...
describe("legacy pi compat package-root override validation (issue #2168)", () => {
	it("keeps overrides whose filesystem targets exist", () => {
		const candidates = {
			"@zero2ai/ai": "/tmp/exists-ai.js",
			"@zero2ai/utils": "/tmp/exists-utils.js",
		};
	...
	it("drops overrides whose filesystem targets are missing on disk", () => {
		const candidates = {
			"@zero2ai/ai": "/tmp/exists-ai.js",
			"@zero2ai/coding-agent": "/tmp/exists-shim.js",
			"@zero2ai/utils": "/$bunfs/root/packages/utils/src/index.js",
			"@zero2ai/tui": "/$bunfs/root/packages/tui/src/index.js",
		};
		...
		expect(result).not.toHaveProperty("@zero2ai/utils");
		expect(result).not.toHaveProperty("@zero2ai/tui");
	});
	...
	it("keeps virtual zero2ai-legacy-pi-bundled: entries without touching the filesystem (issue #3423)", () => {
		...
		let probed = false;
		const candidates = {
			"@zero2ai/ai": "zero2ai-legacy-pi-bundled:@zero2ai/ai",
			"@zero2ai/coding-agent": "zero2ai-legacy-pi-bundled:@zero2ai/coding-agent",
			"@zero2ai/agent-core": "zero2ai-legacy-pi-bundled:@zero2ai/agent-core",
			"@zero2ai/natives": "zero2ai-legacy-pi-bundled:@zero2ai/natives",
			"@zero2ai/tui": "zero2ai-legacy-pi-bundled:@zero2ai/tui",
			"@zero2ai/utils": "zero2ai-legacy-pi-bundled:@zero2ai/utils",
		};
	...
	it("mixes virtual and filesystem entries: virtuals always pass, filesystems gated", () => {
		const candidates = {
			"@zero2ai/ai": "zero2ai-legacy-pi-bundled:@zero2ai/ai",
			"@zero2ai/coding-agent": "/dev/source/legacy-pi-coding-agent-shim.ts",
			"@zero2ai/tui": "/missing/path.ts",
		};
...
describe("legacy pi compat typebox shim path resolution (issues #3414, #3423)", () => {
	it("returns the virtual specifier in compiled-binary mode without touching the filesystem", () => {
		...
		expect(result).toBe("zero2ai-legacy-pi-bundled:typebox");
		expect(probed).toBe(false);
	});
