...
import type { AssistantMessage, FetchImpl } from "@zero2ai/ai";
...
} from "@zero2ai/coding-agent/extensibility/legacy-pi-ai-shim";

// Issue #6859: pi extensions import runtime helpers from the `@earendil-works/pi-ai`
// (aliased to `@zero2ai/ai`) package root that zero2ai's barrel no longer forwards.
// `isContextOverflow` moved under `@zero2ai/ai/error` and the JSON-repair
// helpers moved to `@zero2ai/utils`, so `export * from "@zero2ai/ai"` left
// them off the shim surface and a named import tripped Bun's static
// "No matching export" check during plugin validation (e.g.
// `zero2ai plugin install pi-blackhole`). This pins the bridged root surface so it
