...
import { isValidJsonSchema, toolWireSchema } from "@zero2ai/ai/utils/schema";
import { validateToolArguments } from "@zero2ai/ai/utils/validation";
import { type TSchema, Type } from "@zero2ai/coding-agent/extensibility/legacy-typebox";
...
describe("pi.typebox compatibility shim", () => {
	...
	// Callable schema schemas intentionally have no enumerable JSON Schema
	...
	// Issue #8420: legacy extensions build raw documents that embed schema
	...
	describe("lowers embedded schema schemas inside Type.Unsafe documents", () => {
		...
		it("recovers the wire schema when a builder is spread into a new document", () => {
			...
			// `{ ...base, description }` copies schema's internal fields, not JSON
