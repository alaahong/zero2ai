...
import { __buildLegacyPiPackageRootOverrides } from "@zero2ai/coding-agent/extensibility/plugins/legacy-pi-compat";
...
// the same `zero2ai-legacy-pi-bundled:` virtual namespace as package roots without
...
describe("legacy pi compat compiled-mode subpath overrides (issue #3442)", () => {
	it("does not evaluate unrelated host modules while loading the registry", async () => {
		using tempDir = TempDir.createSync("@zero2ai-legacy-pi-loaders-");
...
`,
	...
	it("serves @zero2ai/ai/oauth through the bundled virtual namespace in compiled mode", () => {
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		expect(overrides["@zero2ai/ai/oauth"]).toBe("zero2ai-legacy-pi-bundled:@zero2ai/ai/oauth");
	});
	...
	it("expands wildcard exports for concrete on-disk targets (issue #3442 follow-up)", () => {
		...
		// importing `@(scope)/pi-ai/oauth/anthropic` (remapped via ZERO2AI_SUBPATH_REMAPS
		...
		expect(bundledModuleKeys.has("@zero2ai/ai/oauth/openai-codex")).toBe(true);
	});
	...
	it("actually loads the shim's shared Pi translation through the bundled registry", async () => {
		...
		const key = "@zero2ai/ai/providers/cursor-pi-args";
...
`,
		...
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		expect(overrides[key]).toBe(`zero2ai-legacy-pi-bundled:${key}`);
	});

	it("expands web search provider wildcard exports for compiled plugin imports", () => {
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		const providerKeys = [
			"@zero2ai/coding-agent/web/search/providers/xai",
			"@zero2ai/coding-agent/web/search/providers/tinyfish",
			"@zero2ai/coding-agent/web/search/providers/firecrawl",
			"@zero2ai/coding-agent/web/search/providers/duckduckgo",
		] as const;
	...
	it("serves coding-agent registry wildcard exports in compiled mode", () => {
		const key = "@zero2ai/coding-agent/registry/agent-registry";
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		expect(bundledModuleKeys.has(key)).toBe(true);
		expect(overrides[key]).toBe(`zero2ai-legacy-pi-bundled:${key}`);
	});
	...
	it("does not enumerate root catch-all wildcards (./* / ./*.js)", () => {
		...
		// Concrete check: `@zero2ai/coding-agent/cli` is NOT bundled.
		expect(bundledModuleKeys.has("@zero2ai/coding-agent/cli")).toBe(false);
		expect(bundledModuleKeys.has("@zero2ai/coding-agent/main")).toBe(false);
	});
	...
	it("does not bundle main-thread-unsafe worker entrypoints", () => {
		...
		expect(bundledModuleKeys.has("@zero2ai/coding-agent/eval/js/worker-entry")).toBe(false);
	});
	...
	it("keeps pi-ai/pi-coding-agent/pi-tui roots routed to their compat shims in compiled mode", () => {
		...
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		expect(overrides["@zero2ai/ai"]).toBeDefined();
		expect(overrides["@zero2ai/ai"]).not.toBe("zero2ai-legacy-pi-bundled:@zero2ai/ai/oauth");
		expect(overrides["@zero2ai/coding-agent"]).toBeDefined();
		expect(overrides["@zero2ai/tui"]).toBeDefined();
	});

	it("does not register subpath overrides in dev/install mode", () => {
		const overrides = __buildLegacyPiPackageRootOverrides(false);
		expect(overrides).not.toHaveProperty("@zero2ai/ai/oauth");
		expect(overrides).not.toHaveProperty("@zero2ai/coding-agent/tools");
	...
	it("bundles nested wildcard subpaths so a compiled extension can import them", () => {
		...
		expect(bundledModuleKeys.has("@zero2ai/coding-agent/slash-commands/helpers/active-oauth-account")).toBe(true);
		// Directory index modules stay excluded: `./x/*` must not serve `x/y`
		// from `y/index.ts`, which Node would not resolve either.
		expect(bundledModuleKeys.has("@zero2ai/coding-agent/modes/theme/defaults/index")).toBe(false);
	});
});
