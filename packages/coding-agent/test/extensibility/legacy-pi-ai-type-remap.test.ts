...
import type { Api, AssistantMessage, Context, Model, SimpleStreamOptions } from "@zero2ai/ai";
...
import { Type as TypeBoxShimType } from "@zero2ai/coding-agent/extensibility/legacy-typebox";
import {
	__resetLegacyPiResolutionCache,
	installLegacyPiSpecifierShim,
	loadLegacyPiModule,
} from "@zero2ai/coding-agent/extensibility/plugins/legacy-pi-compat";
import { removeWithRetries } from "@zero2ai/utils";

// pi-ai 15.1.0 removed the runtime `Type` export from `@zero2ai/ai`'s
...
async function writeFixtureExtension(source: string): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "zero2ai-pi-ai-type-remap-"));
	tempRoots.push(dir);
...
describe("legacy-pi @(scope)/pi-ai root `Type` remap (issue #1437)", () => {
	...
	it('redirects `import { Type } from "@zero2ai/ai"` for plugins published against the canonical scope', async () => {
		const entry = await writeFixtureExtension(
			['import { Type } from "@zero2ai/ai";', "export const probe = Type;"].join("\n"),
		);
	...
	it("does not redirect subpath imports such as @zero2ai/ai/utils/schema", async () => {
		const entry = await writeFixtureExtension(
			[
	...
	it("re-exports the legacy model catalog and schema helpers from the root", async () => {
		const loaded = (await loadLegacyPiModule(
			await writeFixtureExtension(
				[
					'import { calculateCost, clampThinkingLevel, getBundledModel, getBundledModels, getBundledProviders, getModel, getModels, modelsAreEqual, StringEnum } from "@zero2ai/ai";',
...
describe("legacy pi package root remaps (issue #1474)", () => {
	it("loads @earendil-works/pi-coding-agent root imports when host package resolution is unavailable", async () => {
		...
		vi.spyOn(Bun, "resolveSync").mockImplementation((specifier: string, from: string) => {
			if (specifier === "@zero2ai/coding-agent" && from.endsWith(path.join("src", "extensibility", "plugins"))) {
	...
	it("preserves legacy defineTool root imports and usable coding tools", async () => {
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "zero2ai-legacy-coding-tools-"));
		tempRoots.push(dir);
	...
	it("falls back to legacy-scoped subpath peers for direct plugin imports", async () => {
		const realResolveSync = Bun.resolveSync.bind(Bun);
		vi.spyOn(Bun, "resolveSync").mockImplementation((specifier: string, from: string) => {
			if (specifier === "@zero2ai/ai/oauth") {
				throw new Error(`canonical peer unavailable from ${from}`);
			}
			return realResolveSync(specifier, from);
		});

		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "zero2ai-legacy-direct-subpath-"));
		tempRoots.push(dir);
	...
	it("routes @earendil-works/pi-utils through canonical Bun.resolveSync in non-compiled mode", async () => {
		// Regression: when zero2ai runs from a node_modules install (not the monorepo
		// and not a compiled binary), the bundled packages live at
		// `node_modules/@zero2ai/*`, not next to the source tree. Hardcoding
