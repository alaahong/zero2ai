...
import { getConfigDirs } from "@zero2ai/coding-agent/config";
import { resolveClaudePaths } from "@zero2ai/coding-agent/config/claude-paths";
import { getUserPath } from "@zero2ai/coding-agent/discovery/helpers";
import { getAgentDir } from "@zero2ai/utils";

describe("ZERO2AI_CONFIG_DIR", () => {
	const original = process.env.ZERO2AI_CONFIG_DIR;
	afterEach(() => {
		if (original === undefined) {
			delete process.env.ZERO2AI_CONFIG_DIR;
		} else {
			process.env.ZERO2AI_CONFIG_DIR = original;
		}
	});
	...
	test("getUserPath resolves the native user scope via getAgentDir (profile-aware)", () => {
		...
		// The old behavior joined ctx.home + ".zero2ai/agent" and leaked the default
	...
	test("getConfigDirs respects ZERO2AI_CONFIG_DIR for user base", () => {
		process.env.ZERO2AI_CONFIG_DIR = ".config/zero2ai";
		const result = getConfigDirs("commands", { project: false });
		const expected = path.resolve(path.join(os.homedir(), ".config/zero2ai", "agent", "commands"));
		expect(result[0]).toEqual({ path: expected, source: ".zero2ai", level: "user" });
	});
});
