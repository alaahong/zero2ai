...
import { applyFilter } from "@zero2ai/coding-agent/modes/components/extensions/state-manager";
...
describe("Codex and OpenCode slash-command frontmatter", () => {
	...
	beforeEach(async () => {
		...
		root = await fs.mkdtemp(path.join(os.tmpdir(), "zero2ai-provider-commands-"));
