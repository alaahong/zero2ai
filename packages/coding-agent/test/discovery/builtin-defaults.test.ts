...
import {
	...
	type Rule,
	ruleCapability,
} from "@zero2ai/coding-agent/capability/rule";
import type { LoadContext } from "@zero2ai/coding-agent/capability/types";
// Register all discovery providers as a side effect.
import "@zero2ai/coding-agent/discovery";
import { TtsrManager, type TtsrMatchContext } from "@zero2ai/coding-agent/export/ttsr";
...
describe("builtin-defaults rule provider", () => {
	...
	it("fires ts-no-local-is-record on local function and lambda definitions", async () => {
		...
		manager.resetBuffer();
		expect(
			manager.checkDelta('import { isRecord } from "@zero2ai/utils";', {
				source: "tool",
				toolName: "write",
				filePaths: ["packages/x/src/foo.ts"],
			}),
