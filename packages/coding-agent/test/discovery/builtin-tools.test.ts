...
import { type CustomTool, toolCapability } from "@zero2ai/coding-agent/capability/tool";
...
import { discoverCustomToolPaths, loadCustomTools } from "@zero2ai/coding-agent/extensibility/custom-tools/loader";
...
describe("native executable custom tool discovery", () => {
	...
	beforeEach(async () => {
		...
		root = await fs.mkdtemp(path.join(os.tmpdir(), "zero2ai-builtin-tools-"));
		const home = path.join(root, "home");
		project = path.join(root, "project");
		projectTools = path.join(project, ".zero2ai", "tools");
		userTools = path.join(home, ".zero2ai", "agent", "tools");
