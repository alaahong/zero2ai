...
import {
	...
	type TtsrScanArgs,
	type TtsrTestArgs,
} from "@zero2ai/coding-agent/cli/ttsr-cli";
import { resetSettingsForTest } from "@zero2ai/coding-agent/config/settings";
import { getProjectAgentDir, getProjectDir, removeSyncWithRetries, setProjectDir } from "@zero2ai/utils";

let testTmpDir: string;

beforeAll(() => {
	testTmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "zero2ai-ttsr-tests-"));
});
...
describe("zero2ai ttsr", () => {
	...
	describe("test — context inference and matching", () => {
		it("infers tool/edit context when a positional resolves to a .ts file and --source is omitted", async () => {
			...
			// Simulate `zero2ai ttsr test --rule <rule> src/foo.ts`: the command layer
