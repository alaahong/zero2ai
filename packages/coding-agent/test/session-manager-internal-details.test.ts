...
import { type SkillPromptDetails, stripInternalDetailsFields } from "@zero2ai/coding-agent/session/messages";
import type { CustomMessageEntry } from "@zero2ai/coding-agent/session/session-entries";
	...
	it("F1: strips __queueChipText from persisted details while preserving all other SkillPromptDetails fields", () => {
		...
		const id = session.appendCustomMessageEntry<SkillPromptDetails>(
			...
			true,
			{
				...
				args: "bar",
				lineCount: 10,
				__queueChipText: "zero2ai-cmd-1-0",
			},
			"user",
		);
