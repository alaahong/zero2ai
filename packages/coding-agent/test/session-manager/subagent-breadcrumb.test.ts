...
import { readTerminalBreadcrumbEntry } from "@zero2ai/coding-agent/session/session-paths";
...
describe("SessionManager subagent breadcrumb isolation", () => {
	...
	beforeEach(async () => {
		...
		testAgentDir = await fsp.mkdtemp(path.join(os.tmpdir(), "zero2ai-subagent-crumb-"));
