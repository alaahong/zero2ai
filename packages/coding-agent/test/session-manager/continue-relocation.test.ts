...
import { writeTerminalBreadcrumb } from "@zero2ai/coding-agent/session/session-paths";
import { getTerminalId } from "@zero2ai/tui";
import { getConfigRootDir, getTerminalSessionsDir, setAgentDir } from "@zero2ai/utils";
...
describe("SessionManager.continueRecent relocation", () => {
	...
	beforeEach(async () => {
		...
		testAgentDir = await fsp.mkdtemp(path.join(os.tmpdir(), "zero2ai-reloc-test-"));
