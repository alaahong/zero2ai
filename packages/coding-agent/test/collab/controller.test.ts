 ...
 * the published access level caps what `zero2ai collab link` can obtain.
 *
...
import { CollabController } from "@zero2ai/coding-agent/collab/controller";
...
import { CollabHost, CollabHostStoppedError } from "@zero2ai/coding-agent/collab/host";
import { COLLAB_PROTO, type CollabFrame, parseCollabLink } from "@zero2ai/coding-agent/collab/protocol";
import * as registry from "@zero2ai/coding-agent/collab/registry";
...
import { beginStartupComposer, stopPendingStartupComposer } from "@zero2ai/coding-agent/modes/startup-composer";
...
beforeEach(async () => {
	tmp = await fs.mkdtemp(path.join(os.tmpdir(), "zero2ai-collabctl-"));
