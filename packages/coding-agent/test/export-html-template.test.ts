...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "zero2ai-html-template-"));
...
const bundledDependencyStubs: Record<string, string> = {
	"@zero2ai/utils": 'export const APP_NAME = "zero2ai"; export const isEnoent = () => false;',
	"../../modes/theme/theme":
...
describe("HTML export template", () => {
	...
	test("composes the exact source template once and returns the cached bytes", async () => {
		...
		expect(first).toContain("const THEME_STORAGE_KEY = 'zero2ai-export-theme';");
