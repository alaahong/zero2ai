...
import { type BtwHistoryRecord, BtwHistoryStore, getBtwCopyText } from "@zero2ai/coding-agent/session/btw-history";
import { acquireFileLock, withFileLock } from "@zero2ai/utils";
...
describe("BtwHistoryStore", () => {
	...
	beforeEach(async () => {
		directory = await fs.mkdtemp(path.join(os.tmpdir(), "zero2ai-btw-history-"));
