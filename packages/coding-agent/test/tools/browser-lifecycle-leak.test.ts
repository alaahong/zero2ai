...
import { acquireBrowser, getBrowsersMapForTest } from "@zero2ai/coding-agent/tools/browser/registry";
import {
	acquireTab,
	getTabsMapForTest,
	releaseTab,
...
describe("browser lifecycle — close deadlines", () => {
	...
	it("rejects a stuck close with the backend, tab, and pending resource", async () => {
		...
		const kind: CmuxKind = { kind: "cmux", socketPath: "/tmp/zero2ai-close-deadline.sock" };
		const browser = await acquireBrowser(kind, { cwd: "/tmp" });
