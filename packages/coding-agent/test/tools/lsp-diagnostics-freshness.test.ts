...
import { createLspWritethrough, type FileDiagnosticsResult, FileFormatResult } from "@zero2ai/coding-agent/lsp";
import * as lspClient from "@zero2ai/coding-agent/lsp/client";
import * as lspConfig from "@zero2ai/coding-agent/lsp/config";
import { formatContent } from "@zero2ai/coding-agent/lsp/diagnostics";
import type { Diagnostic, LinterClient, LspClient, ServerConfig } from "@zero2ai/coding-agent/lsp/types";
import { EquivalentUriMap, fileToUri } from "@zero2ai/coding-agent/lsp/utils";
import type { DeferredDiagnosticsEntry, ToolSession } from "@zero2ai/coding-agent/tools";
import { WriteTool } from "@zero2ai/coding-agent/tools/write";
import { type ptree, TempDir } from "@zero2ai/utils";
...
describe("LSP diagnostics freshness", () => {
	...
	beforeEach(() => {
		tempDir = TempDir.createSync("@zero2ai-lsp-freshness-");
	});
	...
	it("suppresses TypeScript project diagnostics for orphan files but keeps syntax errors", async () => {
		...
		const orphanDir = TempDir.createSync("@zero2ai-lsp-orphan-");
		try {
