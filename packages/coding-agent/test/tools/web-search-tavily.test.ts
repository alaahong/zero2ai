...
import { searchTavily } from "@zero2ai/coding-agent/web/search/providers/tavily";
import type { SearchProviderError } from "@zero2ai/coding-agent/web/search/types";
...
describe("Tavily web search provider", () => {
	...
	it("retries recency-filtered empty responses without time_range", async () => {
		...
		const response = await searchTavily({
			...makeParams("Oh My Pi zero2ai latest release notes advisor"),
			numSearchResults: 5,
			recency: "month",
			fetch: fetchMock,
		});
		...
		expect(requestBodies[0]).toMatchObject({
			query: "Oh My Pi zero2ai latest release notes advisor",
			max_results: 5,
			time_range: "month",
		});
