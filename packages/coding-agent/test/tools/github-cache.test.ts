 ...
 *
 * Each test isolates `ZERO2AI_GITHUB_CACHE_DB` to a temp file and clears
...
import { getOrFetchIssue, getOrFetchPr } from "@zero2ai/coding-agent/tools/gh";
import {
	clearAll,
	getCached,
	getOrFetchView,
...
import { ToolAbortError, throwIfAborted } from "@zero2ai/coding-agent/tools/tool-errors";
...
beforeEach(async () => {
	...
	process.env.ZERO2AI_GITHUB_CACHE_DB = path.join(tempDir, "github-cache.db");
	resetCacheForTests();
});
...
describe("github-cache db layer", () => {
	...
	it("does not chmod an existing cache parent directory", async () => {
		...
		process.env.ZERO2AI_GITHUB_CACHE_DB = path.join(parent, "github-cache.db");
		resetCacheForTests();
