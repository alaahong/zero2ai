 ...
 * ZERO2AI-launched headless worker tabs of the owning session that did not opt
...
import type { CmuxKind } from "@zero2ai/coding-agent/tools/browser/cmux/rpc";
import { CmuxSocketClient } from "@zero2ai/coding-agent/tools/browser/cmux/socket-client";
import { acquireBrowser } from "@zero2ai/coding-agent/tools/browser/registry";
import type { BrowserHandle } from "@zero2ai/coding-agent/tools/browser/registry";
import {
	acquireTab,
...
import type { PendingRun, TabSession } from "@zero2ai/coding-agent/tools/browser/tab-supervisor";
...
function makeKind(socketSuffix: string): CmuxKind {
	return { kind: "cmux", socketPath: `/tmp/zero2ai-test-${socketSuffix}.sock`, surface: `surface-${socketSuffix}` };
}
...
describe("browser settle — lifecycle freeze via CDP", () => {
	...
	describe("browser settle — ownership and bookkeeping", () => {
		...
		it("tears down once when two releases race the same tab", async () => {
			...
			const browser = await acquireBrowser(
				{ kind: "cmux", socketPath: "/tmp/zero2ai-test-settle-join.sock" },
				{ cwd: "/tmp" },
			);
	...
	describe.skipIf(!CHROMIUM_AVAILABLE)("browser settle — real headless Chromium", () => {
		...
		it("recreates a tab reused after eviction instead of resurrecting state", async () => {
			...
			const browser = await acquireBrowser(
				{ kind: "cmux", socketPath: "/tmp/zero2ai-test-settle-recreate.sock" },
				{ cwd: "/tmp" },
			);
