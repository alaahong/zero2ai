...
import { withStatsSyncLock } from "@zero2ai/stats/aggregator";
import { type GcResult, runGcCommand } from "@zero2ai/coding-agent/cli/gc-cli";
...
beforeEach(async () => {
	...
	root = await fs.mkdtemp(path.join(os.tmpdir(), "zero2ai-gc-"));
	writes = [];
	stderrWrites = [];
	process.exitCode = 0;
...
describe("runGcCommand blob sweep", () => {
	...
	test("--apply resolves breadcrumbed relative session paths from their recorded cwd", async () => {
		...
		const externalDir = path.join(projectDir, ".zero2ai-sessions");
