...
import { MCPManager, type MCPReconnectPolicy } from "@zero2ai/coding-agent/mcp/manager";
import type { McpConnectionStatusEvent } from "@zero2ai/coding-agent/mcp/startup-events";
import type { MCPTool } from "@zero2ai/coding-agent/mcp/tool-bridge";
import type { MCPHttpServerConfig } from "@zero2ai/coding-agent/mcp/types";
...
describe("MCP lost remote server retry schedule", () => {
	...
	async function connected(policy: MCPReconnectPolicy) {
		workDir = fs.mkdtempSync(path.join(os.tmpdir(), "zero2ai-mcp-lost-remote-"));
