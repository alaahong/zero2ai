...
import { ArtifactManager, writeArtifact } from "@zero2ai/coding-agent/session/artifacts";
...
describe("ArtifactManager write integrity", () => {
	...
	function freshDir(): string {
		const dir = path.join(os.tmpdir(), `zero2ai-artifact-integrity-${crypto.randomUUID()}`);
		dirs.push(dir);
		return dir;
	}
