...
} from "@zero2ai/coding-agent/tiny/dtype";
...
describe("tiny model dtype selection", () => {
	...
	it("rejects an unsupported precision", () => {
		expect(() => resolveTinyModelDtypeOverride("int4")).toThrow("Unsupported ZERO2AI_TINY_DTYPE");
	});
});

describe("tiny model dtype setting → ZERO2AI_TINY_DTYPE mapping", () => {
