...
import type { MCPServerConnection, MCPStdioServerConfig, MCPTransport } from "@zero2ai/coding-agent/mcp/types";
...
describe("MCPManager initial connection ownership", () => {
	...
	it("recovers tools after an initial handshake timeout", async () => {
		const workDir = await fs.mkdtemp(path.join(os.tmpdir(), "zero2ai-mcp-initial-recovery-"));
