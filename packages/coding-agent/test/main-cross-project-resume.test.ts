...
import { type Args, parseArgs } from "@zero2ai/coding-agent/cli/args";
import * as modelResolverModule from "@zero2ai/coding-agent/config/model-resolver";
import { Settings } from "@zero2ai/coding-agent/config/settings";
import * as pluginHelpers from "@zero2ai/coding-agent/discovery/helpers";
import { createSessionManager, runRootCommand } from "@zero2ai/coding-agent/main";
import { AuthStorage } from "@zero2ai/coding-agent/session/auth-storage";
import type { SessionHeader } from "@zero2ai/coding-agent/session/session-entries";
...
import { getProjectDir, normalizePathForComparison, setProjectDir } from "@zero2ai/utils";
...
describe("createSessionManager — cross-project --resume", () => {
	...
	beforeEach(async () => {
		existingProject = await fsp.mkdtemp(path.join(os.tmpdir(), "zero2ai-xproj-"));
...
describe("SessionManager.open — recorded cwd adoption", () => {
	it("keeps the launch cwd when the recorded cwd cannot be probed", async () => {
		const root = await fsp.mkdtemp(path.join(os.tmpdir(), "zero2ai-xproj-denied-"));
	...
	it("keeps the launch cwd when the recorded cwd denies search permission", async () => {
		const root = await fsp.mkdtemp(path.join(os.tmpdir(), "zero2ai-xproj-noexec-"));
		const launchProject = path.join(root, "launch");
		const deniedProject = path.join(root, "denied");
		await fsp.mkdir(launchProject);
...
describe("runRootCommand — cross-project --resume", () => {
	...
	beforeEach(async () => {
		...
		root = await fsp.mkdtemp(path.join(os.tmpdir(), "zero2ai-xproj-root-"));
...
describe("createSessionManager — cross-project --resume relocation (moved worktree)", () => {
	...
	beforeEach(async () => {
		missingRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "zero2ai-moved-xproj-"));
