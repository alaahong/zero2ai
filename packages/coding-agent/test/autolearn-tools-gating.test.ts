...
import { resetActiveSkillsForTests, type Skill, setActiveSkills } from "@zero2ai/coding-agent/extensibility/skills";
import type { HindsightSessionState } from "@zero2ai/coding-agent/hindsight/state";
import type { MnemopiSessionState } from "@zero2ai/coding-agent/mnemopi/state";
import { createTools, type ToolSession } from "@zero2ai/coding-agent/tools";
import { LearnTool } from "@zero2ai/coding-agent/tools/learn";
import { ManageSkillTool } from "@zero2ai/coding-agent/tools/manage-skill";
...
describe("manage_skill execute", () => {
	...
	beforeEach(async () => {
		...
		tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "zero2ai-manage-skill-"));
...
describe("learn execute", () => {
	...
	beforeEach(async () => {
		originalAgentDir = getAgentDir();
		tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "zero2ai-learn-"));
		spyOn(os, "homedir").mockReturnValue(tempHome);
		setAgentDir(path.join(tempHome, ".zero2ai", "agent"));
		remembered = [];
	});
