 ...
 *
 * Strategy: spy on the six `@zero2ai/utils` plugin-path getters so the
 ...
 * `piUtils.getInstallId`), so we know namespace spying on `zero2ai-utils` exports
...
describe("PluginManager.install with git sources", () => {
	...
	beforeEach(async () => {
		tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "zero2ai-plugin-git-"));
	...
	test("installs from github: shorthand and resolves real package name from deps diff", async () => {
		...
		await Bun.write(
			pluginsPkgJson,
			JSON.stringify({ name: "zero2ai-plugins", private: true, dependencies: {} }, null, 2),
		);
