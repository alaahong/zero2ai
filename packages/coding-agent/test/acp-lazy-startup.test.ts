...
import { createAcpConnection } from "@zero2ai/coding-agent/modes/acp/acp-mode";
...
beforeAll(() => {
	startupDir = TempDir.createSync("@zero2ai-acp-startup-shared-");
...
describe("ACP lazy startup", () => {
	it("applies schema defaults for ACP background jobs", async () => {
		const { runRootCommand } = await import("@zero2ai/coding-agent/main");
	...
	it("applies CLI runtime API keys after ACP lazy session creation resolves extension models", async () => {
		using tempDir = TempDir.createSync("@zero2ai-acp-lazy-api-key-");
...
`,
		...
		try {
			...
			const { runRootCommand } = await import("@zero2ai/coding-agent/main");
			const { createAgentSession } = await import("@zero2ai/coding-agent/sdk");
