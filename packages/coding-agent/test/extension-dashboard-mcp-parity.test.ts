...
import { initializeWithSettings, reset as resetDiscoveryCache } from "@zero2ai/coding-agent/discovery";
import { readMCPConfigFile, setMcpServerEnabled, setServerDisabled } from "@zero2ai/coding-agent/mcp/config-writer";
import { loadAllExtensions } from "@zero2ai/coding-agent/modes/components/extensions/state-manager";
import { __resetDirsFromEnvForTests, getMCPConfigPath, removeWithRetries, setAgentDir } from "@zero2ai/utils";
...
describe("loadAllExtensions MCP parity with /mcp list (issue #3827)", () => {
	...
	beforeEach(async () => {
		...
		projectDir = await fs.mkdtemp(path.join(os.tmpdir(), "zero2ai-3827-project-"));
		userAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "zero2ai-3827-user-"));
		...
		await fs.writeFile(
			path.join(projectDir, ".zero2ai", "mcp.json"),
			JSON.stringify({
				mcpServers: {
	...
	test("dashboard re-enable flips enabled:false in mcp.json (PR #3829 review)", async () => {
		...
		const projectMcpPath = path.join(projectDir, ".zero2ai", "mcp.json");
	...
	test("dashboard disable on a config-resident server writes enabled:false (not denylist)", async () => {
		...
		const projectConfig = await readMCPConfigFile(path.join(projectDir, ".zero2ai", "mcp.json"));
	...
	test("dashboard re-enable updates the row's non-primary source mcp.json before denylisting", async () => {
		const alternatePath = path.join(projectDir, ".zero2ai", ".mcp.json");
		await Bun.write(
			alternatePath,
			JSON.stringify({
				mcpServers: {
