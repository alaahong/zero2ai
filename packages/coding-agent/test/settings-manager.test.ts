...
import { __providerInFlightForTesting, streamSimple } from "@zero2ai/ai/stream";
...
import {
	...
	type SettingPath,
	Settings,
} from "@zero2ai/coding-agent/config/settings";
import { SETTINGS_SCHEMA } from "@zero2ai/coding-agent/config/settings-schema";
import * as discovery from "@zero2ai/coding-agent/discovery";
import { AgentStorage } from "@zero2ai/coding-agent/session/agent-storage";
import { AUTO_IMAGE_PROVIDER_ORDER } from "@zero2ai/coding-agent/tools/image-providers";
import { SEARCH_PROVIDER_ORDER } from "@zero2ai/coding-agent/web/search/types";
import { getAgentDbPath, getProjectAgentDir, logger, TempDir } from "@zero2ai/utils";
import * as fileLock from "@zero2ai/utils/file-lock";
...
describe("Settings", () => {
	...
	beforeEach(() => {
		...
		tempDir = TempDir.createSync("@zero2ai-settings-test-");
	...
	describe("config file failure safety", () => {
		...
		it("backs up a corrupted project config and retains the pending project role for retry", async () => {
			await writeSettings({});
			const projectConfigPath = path.join(projectDir, ".zero2ai", "config.yml");
			await Bun.write(
				projectConfigPath,
		...
		it("observes both failures when global and project configs are malformed", async () => {
			...
			await Promise.all([
				...
				Bun.write(path.join(projectDir, ".zero2ai", "config.yml"), malformed),
			]);
			...
			try {
				...
				expect(
					fs.readdirSync(path.join(projectDir, ".zero2ai")).some(name => name.startsWith("config.yml.broken-")),
				).toBe(true);
			} finally {
	...
	describe("model role overrides", () => {
		...
		it("preserves concurrent external per-role edits when saving one global role", async () => {
			...
			// External edit (another zero2ai instance / manual edit): changes advisor,
