...
import type { Model, ServiceTierByFamily } from "@zero2ai/ai";
...
import { parseAgentFields } from "@zero2ai/coding-agent/discovery/helpers";
import type { ToolPathWithSource } from "@zero2ai/coding-agent/extensibility/custom-tools";
import type { LoadExtensionsResult, PreparedExtension } from "@zero2ai/coding-agent/extensibility/extensions/types";
...
describe("runSubprocess parent-discovery pass-through (issue #2190)", () => {
	...
	it("forwards rules, extension-root policy, prepared extensions, and preloaded source paths to createAgentSession", async () => {
		...
		const preloadedExtensionPaths = ["/abs/parent/.zero2ai/extensions/foo.ts"];
