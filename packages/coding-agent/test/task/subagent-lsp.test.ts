...
import type { AgentDefinition, TaskParams } from "@zero2ai/coding-agent/task/types";
import type { IsolationHandle, WorktreeBaseline } from "@zero2ai/coding-agent/task/worktree";
...
import "@zero2ai/coding-agent/tools/yield";
...
describe("subagent LSP availability", () => {
	...
	it("opens isolated persisted subagent sessions with the worktree cwd", async () => {
		...
		mockIsolation();
		const { getOptions } = mockCreateAgentSession();
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zero2ai-isolated-session-cwd-"));
		try {
