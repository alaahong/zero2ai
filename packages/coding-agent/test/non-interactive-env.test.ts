...
import { buildNonInteractiveEnv, NON_INTERACTIVE_ENV } from "@zero2ai/coding-agent/exec/non-interactive-env";
...
describe("buildNonInteractiveEnv", () => {
	...
	it("drops CI when ZERO2AI_BASH_NO_CI or its legacy alias is set", () => {
		expect(buildNonInteractiveEnv(undefined, { ZERO2AI_BASH_NO_CI: "1" }, "linux")).not.toHaveProperty("CI");
		expect(buildNonInteractiveEnv(undefined, { CLAUDE_BASH_NO_CI: "1" }, "linux")).not.toHaveProperty("CI");
		expect(buildNonInteractiveEnv(undefined, { ZERO2AI_BASH_NO_CI: "1" }, "win32")).not.toHaveProperty("CI");
	});

	it("lets a per-command CI override win over the opt-out", () => {
		expect(buildNonInteractiveEnv({ CI: "0" }, { ZERO2AI_BASH_NO_CI: "1" }, "linux").CI).toBe("0");
	});
});

it("filters expanded dotenv values while preserving matching and empty launcher values", async () => {
	const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "zero2ai-env-"));
	try {
		await Bun.write(
			path.join(tmp, ".env"),
			[
				"BASE=loaded-by-zero2ai",
		...
		const script = [
			...
			"	inherited: env.ZERO2AI_TEST_INHERITED_MARKER ?? null,",
		...
		for (const bunArgs of bunArgSets) {
			const proc = Bun.spawn([process.execPath, ...bunArgs, "--no-install", "--eval", script], {
				...
				env: {
					...
					EMPTY_PARENT_VAR: "",
					ZERO2AI_TEST_INHERITED_MARKER: "keep-me",
					NODE_ENV: "development",
