...
import type { ObservableSession } from "@zero2ai/coding-agent/modes/session-observer-registry";
...
function makeSubagentRegistry(sessions: ObservableSession[]) {
	...
	} as unknown as import("@zero2ai/coding-agent/modes/session-observer-registry").SessionObserverRegistry;
}
...
describe("Agent hub silent-abort regression", () => {
	...
	beforeEach(async () => {
		...
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "zero2ai-overlay-test-"));
	});
