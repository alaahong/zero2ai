...
import { refreshDirsFromEnv } from "@zero2ai/utils";
...
beforeEach(async () => {
	temporaryRoot = await fs.mkdtemp(path.join(os.tmpdir(), "zero2ai-security-slash-"));
...
describe("/security", () => {
	...
	test("validate returns a static ZERO2AI-native residual prompt", async () => {
