...
import { createMockModel, type MockResponseSource, registerMockApi } from "@zero2ai/ai/providers/mock";
...
beforeAll(async () => {
	registryRoot = await fs.mkdtemp(path.join(os.tmpdir(), "zero2ai-security-coordinator-auth-"));
...
beforeEach(async () => {
	temporaryRoot = await fs.mkdtemp(path.join(os.tmpdir(), "zero2ai-security-coordinator-"));
...
describe("native security coordinator", () => {
	...
	test("restart recovery reconciles an interrupted persisted operation", async () => {
		...
		const interrupted: SecurityScanBundle = {
			scan: {
				documentType: "zero2ai-security.scan",
				schemaVersion: "1.0",
				id: scanId,
