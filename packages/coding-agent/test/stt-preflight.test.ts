...
import { Settings, settings } from "@zero2ai/coding-agent/config/settings";
import * as asrClient from "@zero2ai/coding-agent/stt/asr-client";
import * as downloader from "@zero2ai/coding-agent/stt/downloader";
import { STTController } from "@zero2ai/coding-agent/stt/stt-controller";
import { getTinyModelsCacheDir, removeWithRetries, setAgentDir } from "@zero2ai/utils";
...
describe("isSttModelCached completeness", () => {
	...
	beforeEach(async () => {
		...
		tmp = await fs.mkdtemp(path.join(os.tmpdir(), "zero2ai-stt-cache-"));
		setAgentDir(tmp);
