 ...
 * round-trips through `__zero2ai_worker_mnemopi_embed`, and `SIGKILL`s the child
...
} from "@zero2ai/coding-agent/mnemopi/embed-client";
import type {
	MnemopiEmbedWorkerInbound,
	MnemopiEmbedWorkerOutbound,
} from "@zero2ai/coding-agent/mnemopi/embed-protocol";
...
describe("issue #3031 — mnemopi embeddings live in an isolated subprocess", () => {
	it("ping/pongs through the spawned worker subprocess and tears it down cleanly", async () => {
		...
		// `zero2ai --smoke-test`. Run it in a child Bun process instead of this
		...
		const script =
			'const { smokeTestMnemopiEmbedWorker } = await import("@zero2ai/coding-agent/mnemopi/embed-client"); await smokeTestMnemopiEmbedWorker({ timeoutMs: 15000 });';
