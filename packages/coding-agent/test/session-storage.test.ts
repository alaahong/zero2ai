...
import { FileSessionStorage, SessionLockError } from "@zero2ai/coding-agent/session/session-storage";
import { type SessionTitleUpdate, serializeTitleSlot } from "@zero2ai/coding-agent/session/session-title-slot";
...
describe("FileSessionStorage writer", () => {
	...
	beforeEach(async () => {
		tempDir = await fsp.mkdtemp(path.join(os.tmpdir(), "zero2ai-session-writer-"));
...
describe("FileSessionStorage.deleteSessionWithArtifacts", () => {
	...
	beforeEach(async () => {
		tempDir = await fsp.mkdtemp(path.join(os.tmpdir(), "zero2ai-session-storage-"));
		storage = new FileSessionStorage();
	});
