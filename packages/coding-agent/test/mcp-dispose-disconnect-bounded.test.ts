 ...
 * default, unbounded when `ZERO2AI_MCP_TIMEOUT_MS=0`). `dispose()` wraps the
...
import { removeSyncWithRetries, withTimeout } from "@zero2ai/utils";
...
describe("owned-manager dispose disconnect is bounded (PR #2839)", () => {
	...
	beforeEach(() => {
		workDir = fs.mkdtempSync(path.join(os.tmpdir(), "zero2ai-mcp-dispose-"));
	});
