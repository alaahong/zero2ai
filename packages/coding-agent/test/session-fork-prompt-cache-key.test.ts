...
import type { ScopedModel } from "@zero2ai/coding-agent/config/model-resolver";
import { Settings } from "@zero2ai/coding-agent/config/settings";
import { buildSessionOptions } from "@zero2ai/coding-agent/main";
import { type CreateAgentSessionOptions, createAgentSession } from "@zero2ai/coding-agent/sdk";
import type { AgentSession } from "@zero2ai/coding-agent/session/agent-session";
import { AuthStorage } from "@zero2ai/coding-agent/session/auth-storage";
import { CURRENT_SESSION_VERSION, type SessionHeader } from "@zero2ai/coding-agent/session/session-entries";
...
describe("provider prompt-cache key session affinity", () => {
	...
	it("creates an agent whose prompt-cache key can differ from provider request lineage", async () => {
		using tempDir = TempDir.createSync("@zero2ai-prompt-cache-sdk-");
	...
	it("initializes a full fork with child request lineage and parent prompt-cache affinity", async () => {
		using tempDir = TempDir.createSync("@zero2ai-prompt-cache-fork-");
	...
	it("does not auto-inherit parent prompt-cache affinity when fork startup changes request-shaping inputs", async () => {
		...
		for (const entry of cases) {
			using tempDir = TempDir.createSync(`@zero2ai-prompt-cache-fork-${entry.name}-`);
	...
	it("does not pre-pin parent prompt-cache affinity when a scoped model selects the startup route", async () => {
		using tempDir = TempDir.createSync("@zero2ai-prompt-cache-scoped-model-");
