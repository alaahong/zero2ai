...
import { withAuth } from "@zero2ai/ai/auth-retry";
import type { Api, Context, FetchImpl, Model } from "@zero2ai/ai/types";
import { buildModel } from "@zero2ai/catalog/build";
import { resolveConfigValue } from "@zero2ai/coding-agent/config/model-config-values";
...
describe("ModelRegistry command-resolved models.yml values", () => {
	...
	beforeEach(async () => {
		tempDir = path.join(os.tmpdir(), `zero2ai-test-model-command-values-${Snowflake.next()}`);
