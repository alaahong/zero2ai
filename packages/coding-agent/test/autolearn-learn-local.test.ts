...
import {
	...
	saveLearnedLesson,
} from "@zero2ai/coding-agent/memories";
import { localBackend } from "@zero2ai/coding-agent/memory-backend/local-backend";
...
describe("learned-lesson storage (local backend)", () => {
	...
	beforeEach(async () => {
		tmp = await fs.mkdtemp(path.join(os.tmpdir(), "zero2ai-learned-"));
...
describe("learned-lesson read-back", () => {
	...
	beforeEach(async () => {
		tmp = await fs.mkdtemp(path.join(os.tmpdir(), "zero2ai-learned-read-"));
		agentDir = path.join(tmp, "agent");
	});
	afterEach(async () => {
		await removeWithRetries(tmp);
...
describe("learn tool (local backend)", () => {
	...
	beforeEach(async () => {
		tmp = await fs.mkdtemp(path.join(os.tmpdir(), "zero2ai-learn-local-"));
		agentDir = path.join(tmp, "agent");
		projCwd = path.join(tmp, "proj");
		learnedFile = path.join(getMemoryRoot(agentDir, projCwd), "learned.md");
	});
