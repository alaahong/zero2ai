...
import { executePython } from "@zero2ai/coding-agent/eval/py/executor";
import type { KernelExecuteOptions, KernelExecuteResult } from "@zero2ai/coding-agent/eval/py/kernel";
...
describe("executePython (per-call)", () => {
	...
	it("returns a cancelled timeout result when kernel startup exceeds the deadline", async () => {
		...
		using tempDir = TempDir.createSync("@zero2ai-python-executor-per-call-");
