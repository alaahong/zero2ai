...
- ESDLC phases gained configuration and evidence: analysis & design load configurable spec/skill sources (URL or project path, with a provenance artifact), build can seed a skeleton from a template directory and/or a scaffold command before the agent runs, the test phase measures multi-signal code quality into a scored report with history, and deploy derives repository facts (configs, scripts, env vars, ports) that the document must cite as evidence.
- Fixed ESDLC's analysis stage skipping its clarification question: the answer channel now exists on every path — the interactive screen asks and accepts a typed answer, non-interactive runs print the question with the `--clarify "<answer>"` remedy (and log it) instead of continuing silently, and `--clarify` answers deterministically for scripts.
- ESDLC streams each stage's command output into a run log while it runs, shown as a live panel in the stage view with elapsed time, so a long build or test run is watchable instead of opaque.
- Added a Code graph tab: the file-level import graph derived live from the working tree marks what a change reaches (transitive dependents) and what it depends on, with `zero2ai esdlc graph` for the same analysis in the terminal.
- ESDLC gained a Config tab where every stage's inputs are externalized: the documents it reads (URL or project path), an optional instruction for that stage, and the command it runs — persisted in the workspace, overridable per run by the existing flags, with only the fields a stage honours shown.
- The ESDLC workspace shows a live flow diagram (stage states, arrows, duration timeline) and per-stage panels for those reports and settings.
...
- Added `esdlc-work` and `esdlc-web` shortcuts for the engineering lifecycle workspace: `zero2ai esdlc-work` opens the terminal workspace (static status when piped), and `zero2ai esdlc-web` opens the same per-project workspace in the browser without passing `--web`.
- Added `zero2ai sync`: carries model configuration over from another agent root (default: the pre-rename `~/.omp`) — `models.yml`, the model settings (`modelRoles`, thinking level, tags, provider/cycle order) and provider credentials. `--dry-run` prints the plan, existing files and scalars are left alone unless `--force`, maps merge with the target's own entries winning, and credential rows are written only after confirmation (or `--yes`), with the store snapshotted first.
