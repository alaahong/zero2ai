...
// Re-exported from @zero2ai/utils so the whole workspace shares one
// `fetch`-compatible signature (tls-fetch's wrappers produce/accept it).
export type { FetchImpl } from "@zero2ai/utils";
...
export interface OpenAICompat {
	...
	/** Optional mapping from zero2ai-ai reasoning levels to provider/model-specific `reasoning_effort` values. */
...
export interface AnthropicCompat {
	 ...
	 *
	 * When unset, direct Anthropic streams use `ZERO2AI_STREAM_IDLE_TIMEOUT_MS`,
	 * then the legacy `ZERO2AI_OPENAI_STREAM_IDLE_TIMEOUT_MS` alias, then 300s.
	 */
	streamIdleTimeoutMs?: number;
	/**
...
export interface BedrockCompat {
	 ...
	 * Undefined defers to `ZERO2AI_STREAM_IDLE_TIMEOUT_MS`, then the legacy
	 * `ZERO2AI_OPENAI_STREAM_IDLE_TIMEOUT_MS` alias, then the 300s default.
	 */
	streamIdleTimeoutMs?: number;
}
...
export interface ResolvedBedrockCompat {
	 ...
	 * `ZERO2AI_STREAM_IDLE_TIMEOUT_MS`, then the legacy
	 * `ZERO2AI_OPENAI_STREAM_IDLE_TIMEOUT_MS` alias, then the 300s default.
	 */
	streamIdleTimeoutMs?: number;
}
 ...
 * (default) or Chat Completions (`ZERO2AI_OPENROUTER_RESPONSES=0`) with the same
...
export type ResolvedAnthropicCompat = Required<Omit<AnthropicCompat, "streamIdleTimeoutMs" | "thinkingLoopGuard">> & {
	 ...
	 * Undefined defers to `ZERO2AI_STREAM_IDLE_TIMEOUT_MS`, then the legacy
	 * `ZERO2AI_OPENAI_STREAM_IDLE_TIMEOUT_MS` alias, then 300s.
	 */
	streamIdleTimeoutMs?: number;
	/**
...
export interface Model<TApi extends Api = Api> {
	 ...
	 * than ZERO2AI's general image pipeline. `stb` local backends reject WebP.
	 */
	 ...
	 * output limit ZERO2AI cannot discover — sending the wrong value triggers 400s
	 * from the upstream provider.
	 */
	omitMaxOutputTokens?: boolean;
	headers?: Record<string, string>;
	/**
	 * Streaming transport override. When `"zero2ai-native"`, `streamSimple` routes
	 * the request to the model's `baseUrl` via the auth-gateway's
	 * `POST /v1/pi/stream` endpoint instead of dispatching the per-API
	 * provider client. The `baseUrl` must point at an `zero2ai auth-gateway`
	 * (or compatible) host; `headers.Authorization` (or `apiKey` resolved by
	 * the registry) carries the gateway bearer.
	 *
	 * Used by containerized zero2ai installs (e.g. robomp slots) to route every
