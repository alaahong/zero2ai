 ...
 * - failure returns an `Zero2AiErrors` with a single fast-fail entry
 */
import { MISSING, Zero2AiErrors } from "./errors";
...
/**
 * Validate `value` against `ir`; returns output value or `Zero2AiErrors`.
...
function fail(path: PropertyKey[], expected: string, data: unknown): Zero2AiErrors {
	const storedPath = path.length === 0 ? undefined : path.length === 1 ? path[0] : [...path];
	return new Zero2AiErrors(storedPath, expected, data);
}
...
function checkNode(ir: IR, v: unknown): boolean {
	switch (ir.k) {
		...
		case "sub":
			return !(ir.schema.run(v) instanceof Zero2AiErrors);
	}
}
...
function visitFinish(ir: IR, v: unknown, path: PropertyKey[]): unknown {
	...
	if (!(out instanceof Zero2AiErrors) || ir.cfg === undefined) return out;
...
function visitNode(ir: IR, v: unknown, path: PropertyKey[]): unknown {
	switch (ir.k) {
		...
		case "refine": {
			...
			if (base instanceof Zero2AiErrors) return base;
			try {
				const result = ir.pred(base);
				if (result instanceof Zero2AiErrors) return path.length === 0 ? result : prefixAll(result, path);
		...
		case "morph": {
			...
			if (input instanceof Zero2AiErrors) return input;
			const context = {
			...
			if (output instanceof Zero2AiErrors) return output;
		...
		case "union": {
			...
			let targeted: Zero2AiErrors | undefined;
			let targetCount = 0;
			for (const member of ir.members) {
		...
		case "array": {
			...
			for (let index = 0; index < v.length; index++) {
				...
				path.pop();
				if (element instanceof Zero2AiErrors) {
		...
		case "tuple": {
			...
			for (let index = 0; index < prefixCount; index++) {
				...
				path.pop();
				if (item instanceof Zero2AiErrors) {
			...
			for (let index = prefixCount; index < ir.prefix.length; index++) {
				...
				if (item.hasDefault && morph) {
					...
					if (item.defFactory && typeof payload === "function") {
						...
						path.pop();
						if (resolved instanceof Zero2AiErrors) {
...
function prefixAll(errs: Zero2AiErrors, path: PropertyKey[]): Zero2AiErrors {
 ...
 * "A or B" expectation.
 */
export function unionFail(ir: IR & { k: "union" }, v: unknown, path: PropertyKey[], expected?: string): Zero2AiErrors {
	...
	if (ir.members.every(member => unwrapBase(member).k === "object")) {
		const branches = ir.members.flatMap(member => {
			...
			return result instanceof Zero2AiErrors ? [[...result]] : [];
		});
		if (branches.length !== 0) {
			...
			const alternatives: Zero2AiErrors[] = [];
			if (common.length !== 0) {
				for (const entry of common) {
					...
					alternatives.push(
						new Zero2AiErrors(entry.path, [...expectations].join(" or "), entry.data, { preserveActual: true }),
					);
				}
			} else {
				for (const branch of branches) {
...
function discriminateFailure(members: IR[], value: unknown, path: PropertyKey[]): Zero2AiErrors | undefined {
	...
	for (const candidate of candidates) {
		...
		if (exact.length === 1) {
			...
			return result instanceof Zero2AiErrors ? result : undefined;
		}
