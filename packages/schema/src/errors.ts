 ...
 * `result instanceof type.errors` / `instanceof Zero2AiErrors`, lazy `.summary`,
...
export const MISSING: unique symbol = Symbol("schema.missing");
...
export class Zero2AiErrors implements Iterable<Zero2AiError> {
	#path: StoredPath;
	#expected: string;
	#data: unknown;
	#entry: Zero2AiError | undefined;
	#entries: Zero2AiError[] | undefined;
	#config: ErrorConfig | undefined;
	#separator = "\n";
	...
	static single(path: PropertyKey[], expected: string, data: unknown, config?: ErrorConfig): Zero2AiErrors {
		return new Zero2AiErrors(path, expected, data, config);
	}

	#getEntry(): Zero2AiError {
		if (this.#entry) return this.#entry;
		const path = this.#path === undefined ? [] : Array.isArray(this.#path) ? [...this.#path] : [this.#path];
		const entry = new Zero2AiError(path, this.#expected, this.#data, this.#config);
		this.#entry = entry;
		return entry;
	}

	/** Append all failures from `other`, preserving traversal order. */
	append(other: Zero2AiErrors): this {
	...
	get byPath(): Readonly<Record<string, Zero2AiError>> {
		const result: Record<string, Zero2AiError> = {};
		for (const entry of this) result[entry.path.map(String).join(".")] = entry;
		return result;
	}

	map<result>(fn: (error: Zero2AiError, index: number, errors: Zero2AiErrors) => result): result[] {
		const result: result[] = [];
		let index = 0;
	...
	filter(fn: (error: Zero2AiError, index: number, errors: Zero2AiErrors) => unknown): Zero2AiError[] {
		const result: Zero2AiError[] = [];
		let index = 0;
	...
	*[Symbol.iterator](): IterableIterator<Zero2AiError> {
		if (this.#entries) {
			yield* this.#entries;
		} else {
			yield this.#getEntry();
...
export class TraversalError extends Error {
	constructor(readonly errors: Zero2AiErrors) {
 ...
 * validation failures, which are returned as {@link Zero2AiErrors}.
 */
export class Zero2AiTypeError extends Error {
	constructor(message: string) {
		super(message);
		this.name = "Zero2AiTypeError";
	}
}
