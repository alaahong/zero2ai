...
import { type ErrorConfig, Zero2AiErrors, Zero2AiTypeError } from "./errors";
import { keywordIR, patternIR, templateIR } from "./keywords";

/** Brand carried by `Type` instances so the parser can embed them in defs. */
export const IR_BRAND: unique symbol = Symbol("schema.schema");

const kMorph: unique symbol = Symbol("schema.hasMorph");
const kMorphOwner: unique symbol = Symbol("schema.hasMorphOwner");
const kAlias: unique symbol = Symbol("schema.hasAlias");
const kAliasOwner: unique symbol = Symbol("schema.hasAliasOwner");
const kSimple: unique symbol = Symbol("schema.simple");
const kSimpleOwner: unique symbol = Symbol("schema.simpleOwner");
...
export interface MorphContext {
	...
	error(expectation: string): Zero2AiErrors;
	/** Alias of `error` matching ArkType's rejection vocabulary. */
	reject(expectation: string): Zero2AiErrors;
}
...
export type IR = IRAnalysis &
	(
		...
		| {
				k: "refine";
				base: IR;
				pred: (value: unknown) => boolean | Zero2AiErrors;
				expected: string;
...
function tokenize(src: string): Tok[] {
	...
	while (i < n) {
		...
		if (c === "d" && (src[i + 1] === "'" || src[i + 1] === '"')) {
			...
			if (end < 0) throw new Zero2AiTypeError(`unterminated date literal in "${src}"`);
			const source = src.slice(i + 2, end).trim();
			const value = /^\d+$/.test(source) ? new Date(Number(source)) : new Date(source);
			if (Number.isNaN(value.valueOf())) throw new Zero2AiTypeError(`invalid date literal in "${src}"`);
		...
		if (c === "'" || c === '"') {
			...
			if (j >= n) throw new Zero2AiTypeError(`unterminated string literal in "${src}"`);
		...
		if (c === "/") {
			...
			if (j >= n) throw new Zero2AiTypeError(`unterminated regular expression in "${src}"`);
			let end = j + 1;
			...
			try {
				...
				throw new Zero2AiTypeError(`invalid regular expression "${src.slice(i, end)}"`);
			}
			i = end;
			continue;
		}
		if ((c >= "0" && c <= "9") || (c === "-" && i + 1 < n && src[i + 1] >= "0" && src[i + 1] <= "9")) {
			...
			if (/^-?(?:0|[1-9]\d*)n$/.test(raw) && raw !== "-0n") {
				...
				if (!valid) throw new Zero2AiTypeError(`Malformed number literal '${raw}'`);
		...
		throw new Zero2AiTypeError(`unexpected character '${c}' in "${src}"`);
	}
	return toks;
}
...
class StrParser {
	...
	#next(): Tok {
		...
		if (!t) throw new Zero2AiTypeError(`unexpected end of definition "${this.#src}"`);
		return t;
	}
	...
	parseTop(): ParsedTop {
		...
		if (this.#eatOp("=")) {
			...
			else throw new Zero2AiTypeError(`unsupported default literal in "${this.#src}"`);
			hasDefault = true;
		}
	...
	#expectEnd(): void {
		if (this.#pos < this.#toks.length) {
			throw new Zero2AiTypeError(`trailing tokens in definition "${this.#src}"`);
		}
	}
		...
		if (literal && typeof literal.v === "number") {
			for (const member of members) {
				if (
					...
					throw new Zero2AiTypeError("literal is excluded by intersection");
				}
			}
			return literal;
		}
	...
	parseBounded(): IR {
		...
		if ((t?.t === "num" || t?.t === "date") && t1?.t === "op" && (t1.v === "<" || t1.v === "<=")) {
			...
			const t2 = this.#peek();
			if (!(t2?.t === "op" && CMP[t2.v])) {
				throw new Zero2AiTypeError(`left bound requires a corresponding right bound in "${this.#src}"`);
			}
			if (t2.v === ">" || t2.v === ">=") {
				throw new Zero2AiTypeError(`right bound must use < or <= in "${this.#src}"`);
			}
			this.#pos++;
			const hi = this.#next();
			if (hi.t !== "num" && hi.t !== "date") {
				throw new Zero2AiTypeError(`expected bound after comparator in "${this.#src}"`);
			}
		...
		if (t2?.t === "op" && t2.v === "==") {
			...
			if (limit.t !== "num" && limit.t !== "bigint" && limit.t !== "date") {
				throw new Zero2AiTypeError(`expected literal after == in "${this.#src}"`);
			}
	...
	#eatDivisor(node: IR): IR {
		...
		if (divisor.t !== "num") throw new Zero2AiTypeError(`expected number after % in "${this.#src}"`);
		if (node.k !== "number") throw new Zero2AiTypeError(`% requires number in "${this.#src}"`);
		if (!Number.isFinite(divisor.v) || !Number.isInteger(divisor.v) || divisor.v === 0)
			throw new Zero2AiTypeError(`divisor must be a non-zero integer in "${this.#src}"`);
	...
	#eatArraySuffixes(node: IR): IR {
		for (;;) {
			...
			this.#pos++;
			if (!this.#eatOp("]")) throw new Zero2AiTypeError(`expected ']' in "${this.#src}"`);
	...
	parsePostfix(): IR {
		...
		for (;;) {
			...
			if (t?.t === "op" && t.v === "#") {
				...
				if (name.t !== "id") throw new Zero2AiTypeError(`expected brand name after # in "${this.#src}"`);
				continue;
			}
			break;
		}
	...
	#parseGenericArguments(): IR[] {
		...
		for (;;) {
			...
			if (this.#eatOp(">")) return arguments_;
			if (!this.#eatOp(",")) throw new Zero2AiTypeError(`expected ',' or '>' in "${this.#src}"`);
			const next = this.#peek();
			if (next?.t === "op" && (next.v === "," || next.v === ">")) {
				throw new Zero2AiTypeError(`generic arguments cannot be empty in "${this.#src}"`);
			}
		}
	}
		...
		if (t.t === "id") {
			if (t.v === "keyof") {
				try {
					...
					if (error instanceof Zero2AiTypeError) throw new Zero2AiTypeError("keyof operand must be an object");
					throw error;
				}
			}
			...
			if (t.v === "Record" && this.#peek()?.t === "op" && this.#peek()?.v === "<") {
				...
				if (arguments_.length !== 2) throw new Zero2AiTypeError("Record requires two arguments");
			...
			if (this.#peek()?.t === "op" && this.#peek()?.v === "<" && this.#resolve?.hasGeneric?.(t.v)) {
				...
				if (!instantiated) throw new Zero2AiTypeError(`unknown generic "${t.v}" in "${this.#src}"`);
				return instantiated;
			}
			const scoped = this.#resolve?.(t.v);
			const make = KEYWORDS[t.v];
			const keyword = scoped ?? make?.() ?? keywordIR(t.v);
			if (!keyword) throw new Zero2AiTypeError(`unknown keyword "${t.v}" in "${this.#src}"`);
			return keyword;
		}
		throw new Zero2AiTypeError(`unexpected token in "${this.#src}"`);
	}
}
...
function resolveStructuralIR(ir: IR): IR {
	...
	while (ir.k === "alias") {
		if (seen.has(ir)) throw new Zero2AiTypeError(`cannot structurally transform recursive alias "${ir.name}"`);
		seen.add(ir);
		ir = ir.resolve();
	}
	return ir;
...
function mergeObjectIR(left: IR, right: IR): IR {
	...
	if (left.k !== "object" || right.k !== "object") {
		throw new Zero2AiTypeError("Merge requires object arguments");
	}
...
function parseGeneric(src: string, resolve?: AliasResolver): IR | undefined {
	...
	if ((generic.name === "Partial" || generic.name === "Required") && generic.args.length === 1) {
		...
		if (object.k !== "object") throw new Zero2AiTypeError(`${generic.name} requires an object`);
...
function parseRegexExec(src: string): IR | undefined {
	...
	if (end < 2) throw new Zero2AiTypeError(`unterminated regular expression in "${src}"`);
	let regex: RegExp;
	try {
		regex = new RegExp(src.slice(2, end), src.slice(end + 1));
	} catch {
		throw new Zero2AiTypeError(`invalid regular expression "${src.slice(1)}"`);
	}
	return {
		k: "morph",
...
function parseStringDef(src: string, resolve?: AliasResolver): ParsedTop {
	...
	if (pipeIndex >= 0) {
		...
		if (input.length === 0 || output.length === 0) {
			throw new Zero2AiTypeError(`pipe expression requires operands in "${src}"`);
		}
		const parsedInput = parseStringDef(input, resolve);
		if (parsedInput.hasDefault) {
			throw new Zero2AiTypeError(`unexpected pipe expression after default in "${src}"`);
		}
...
function applyEquality(node: IR, value: number | bigint | Date, src: string): IR {
	...
	if (value instanceof Date) {
		if (!acceptsDate(node)) throw new Zero2AiTypeError(`Date equality requires Date in "${src}"`);
	...
	if ((node.k === "string" || node.k === "array") && typeof value === "number") {
		if (!Number.isInteger(value) || value < 0) {
			throw new Zero2AiTypeError(`exact length must be a non-negative integer in "${src}"`);
		}
		return { ...node, min: value, max: value };
	}
	throw new Zero2AiTypeError(`equality literal is incompatible with ${node.k} in "${src}"`);
}
...
function applyBound(node: IR, op: string, value: number | Date, src: string): IR {
	...
	if (node.k === "union") {
		...
		if (kinds.size !== 1) throw new Zero2AiTypeError(`cannot apply one bound to multiple bound kinds in "${src}"`);
	...
	if (value instanceof Date) {
		if (!acceptsDate(node)) throw new Zero2AiTypeError(`date bound requires Date in "${src}"`);
	...
	if (node.k === "number") {
		...
		if (
			...
			throw new Zero2AiTypeError(`numeric range is unsatisfiable in "${src}"`);
		}
		return bounded;
	}
	if (node.k === "string" || node.k === "array") {
		if (!Number.isInteger(value) || value < 0) {
			throw new Zero2AiTypeError(`length bound must be a non-negative integer in "${src}"`);
		}
		const bounded = { ...node };
		switch (op) {
			case ">=":
		...
		if (
			...
			throw new Zero2AiTypeError(`length range is unsatisfiable in "${src}"`);
		}
		return bounded;
	}
	throw new Zero2AiTypeError(`cannot bound ${node.k} in "${src}"`);
}
...
function appendTupleItem(tuple: TupleIR, item: TupleItemIR): void {
	...
	if (item.hasDefault && tuple.prefix.some(prefixItem => prefixItem.opt && !prefixItem.hasDefault)) {
		throw new Zero2AiTypeError("A defaultable element may not follow an optional element without a default");
	}
	if (hasOptionalPrefix(tuple) && !item.opt) {
		throw new Zero2AiTypeError("required tuple elements cannot follow optional elements");
	}
...
function appendTuple(target: TupleIR, spread: TupleIR): void {
	if (target.variadic !== undefined && spread.variadic !== undefined) {
		throw new Zero2AiTypeError("a tuple may have one spread followed by an array definition");
	}
...
function spreadAlternatives(spread: IR): TupleIR[] {
	...
	throw new Zero2AiTypeError("tuple spread element must be an array");
}
...
export function keyOf(node: IR): IR {
	...
	if (node.k === "union") {
		...
		if (common.length === 0) throw new Zero2AiTypeError("keyof operand must be an object");
		const members = common.map(value => ({ k: "lit", v: value }) satisfies IR);
		return members.length === 1 ? members[0] : { k: "union", members };
	}
	throw new Zero2AiTypeError("keyof operand must be an object");
}
...
function parseArrayExpression(def: readonly unknown[], resolve?: AliasResolver): IR {
	...
	if (def[0] === "instanceof") {
		...
		for (let index = 1; index < def.length; index++) {
			...
			if (!isConstructor(ctor)) throw new Zero2AiTypeError("instanceof operands must be constructors");
			members.push({
				k: "instance",
				ctor,
	...
	if (def.length === 3 && def[1] === "=>") {
		if (!isCallback(def[2])) throw new Zero2AiTypeError("morph operator requires a function");
	...
	if (def.length === 3 && def[1] === ":") {
		if (!isCallback(def[2])) throw new Zero2AiTypeError("narrow operator requires a predicate");
		...
		return {
			...
			pred: value => {
				let errors: Zero2AiErrors | undefined;
				const error = (
					input:
						| string
						| {
				...
				return errors ?? (result instanceof Zero2AiErrors ? result : result === true);
			},
			expected,
		};
	}
...
function indexKeyKind(
	...
	throw new Zero2AiTypeError(`indexed key definition must resolve to a string or symbol (was ${expectedOf(key)})`);
}
...
function addObjectProp(props: PropIR[], spreadKeys: Set<PropertyKey> | undefined, prop: PropIR): void {
	...
	if (!spreadKeys.delete(prop.key)) throw new Zero2AiTypeError(`duplicate object key ${String(prop.key)}`);
	props[previous] = prop;
}
...
function parseObjectDefinition(def: Record<PropertyKey, unknown>, resolve?: AliasResolver): IR {
	...
	for (const originalKey in def) {
		...
		if (originalKey === "+") {
			...
			else throw new Zero2AiTypeError(`bad "+" value ${String(val)}`);
			continue;
		}
		if (originalKey === "...") {
			...
			if (spread === undefined) {
				throw new Zero2AiTypeError(`object spread must resolve to an object literal (was ${expectedOf(parsed)})`);
			}
		...
		if (typeof originalKey === "string" && originalKey.startsWith("[") && originalKey.endsWith("]")) {
			...
			if (typeof val === "string") {
				...
				if (parsed.hasDefault) throw new Zero2AiTypeError("index signatures cannot specify a default");
				value = parsed.ir;
			} else {
				if (Array.isArray(val) && val.length === 3 && val[1] === "=") {
					throw new Zero2AiTypeError("index signatures cannot specify a default");
				}
			...
			if (regex === null) {
				...
				try {
					...
					throw new Zero2AiTypeError(`invalid index signature pattern ${keyDefinition}`);
				}
			}
			const objectIndexes = indexes ?? { patterns: [] };
			indexes = objectIndexes;
		...
		if (key !== originalKey) {
			if (!spreadKeys?.has(key) && props.some(candidate => candidate.key === key)) {
				throw new Zero2AiTypeError(`duplicate object key ${String(key)}`);
			}
		...
		if (opt && prop.hasDefault) throw new Zero2AiTypeError(`optional key ${String(key)} cannot specify a default`);
...
export function parseDef(def: unknown, resolve?: AliasResolver): IR {
	if (typeof def === "string") {
		const parsed = parseStringDef(def, resolve);
		if (parsed.hasDefault) {
			throw new Zero2AiTypeError("A default may only be specified for an object property or tuple element");
		}
		if (parsed.optional) {
			throw new Zero2AiTypeError(`optional "?" marker is only valid on object property values`);
		}
		return parsed.ir;
	}
	...
	if (typeof def === "function") {
		...
		if (!isEmbedded(resolved)) {
			throw new Zero2AiTypeError(`thunk must return a Type (was ${typeof resolved})`);
		}
		return embed(resolved);
	}
	if (isObjectDefinition(def)) return parseObjectDefinition(def, resolve);
	throw new Zero2AiTypeError(`unsupported definition ${String(def)} (was ${typeof def})`);
}
