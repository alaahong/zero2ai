import { type Zero2AiErrors, Zero2AiTypeError } from "./errors";
...
interface Decoratable<out Out> extends EmbeddableSchema {
	(value: unknown): Out | Zero2AiErrors;
	narrow(predicate: (value: Out, context: NarrowContext) => unknown): Decoratable<Out>;
	pipe<Next>(transform: (value: Out, context: NarrowContext) => Next): Decoratable<Exclude<Next, Zero2AiErrors>>;
...
/** A callable schema schema carrying the Zod-v4-style fluent surface. */
...
function lengthBound(kind: "min" | "max", schema: Decoratable<unknown>, bound: number): void {
	...
	if (!Number.isSafeInteger(bound) || bound < 0) {
		throw new Zero2AiTypeError(`${kind} length must be a nonnegative safe integer`);
	}
}
...
function decorate<Out>(schema: Decoratable<Out>, optional = false): ZodLikeSchema<Out> {
	...
	const withObjectExtras = (extras: "keep" | "reject" | "delete"): ZodLikeSchema<Out> => {
		if (schema.ir.k !== "object") throw new Zero2AiTypeError("object mode requires an object schema");
	...
	return Object.assign(schema, {
		...
		min(bound: number): ZodLikeSchema<Out> {
			...
			if (ir.k === "number") {
				if (Number.isNaN(bound)) throw new Zero2AiTypeError("number min must not be NaN");
			...
			if (
				...
				if (!Number.isSafeInteger(bound) || bound < 0)
					throw new Zero2AiTypeError("min length must be a nonnegative safe integer");
			...
			throw new Zero2AiTypeError(`cannot apply min to ${ir.k}`);
		},
		max(bound: number): ZodLikeSchema<Out> {
			const ir = schema.ir;
			if (ir.k === "string" || ir.k === "array") {
			...
			if (ir.k === "number") {
				if (Number.isNaN(bound)) throw new Zero2AiTypeError("number max must not be NaN");
			...
			if (
				...
				if (!Number.isSafeInteger(bound) || bound < 0)
					throw new Zero2AiTypeError("max length must be a nonnegative safe integer");
			...
			throw new Zero2AiTypeError(`cannot apply max to ${ir.k}`);
		},
		int(): ZodLikeSchema<Out> {
			if (schema.ir.k !== "number") throw new Zero2AiTypeError(`cannot apply int to ${schema.ir.k}`);
			return next(restrictBase(schema, { ...schema.ir, int: true }));
		},
		positive(): ZodLikeSchema<Out> {
			if (schema.ir.k !== "number") throw new Zero2AiTypeError(`cannot apply positive to ${schema.ir.k}`);
			const ir = schema.ir;
			if (ir.min !== undefined && ir.min > 0) return next(restrictBase(schema, ir));
			return next(restrictBase(schema, { ...ir, min: 0, xmin: true }));
		},
		nonnegative(): ZodLikeSchema<Out> {
			if (schema.ir.k !== "number") throw new Zero2AiTypeError(`cannot apply nonnegative to ${schema.ir.k}`);
			return this.min(0);
		},
		regex(expression: RegExp, message?: string): ZodLikeSchema<Out> {
			const ir = schema.ir;
			...
			if (!isStringLike) throw new Zero2AiTypeError(`cannot apply regex to ${ir.k}`);
		...
		url(): ZodLikeSchema<Out> {
			...
			throw new Zero2AiTypeError(`cannot apply url to ${ir.k}`);
		},
		optional(): ZodLikeSchema<Out | undefined> & OptionalSchemaMarker {
		...
		trim(): ZodLikeSchema<Out> {
			...
			if (ir.k === "morph" || schema.hasSteps) {
				let trimmed = schemaFromIR<Out>({
					...
					fn: v => {
						...
						if (typeof r !== "string") throw new Zero2AiTypeError("trim requires a string output");
						return r.trim();
					},
					out: { k: "string" },
				});
			...
			throw new Zero2AiTypeError(`cannot apply trim to ${ir.k}`);
		},
		transform<Next>(transformer: (value: Out) => Next): ZodLikeSchema<Next> {
			return decorate(
		...
		partial(): Out extends object ? ZodLikeSchema<Partial<Out>> : ZodLikeSchema<Out> {
			if (schema.ir.k !== "object") throw new Zero2AiTypeError(`cannot apply partial to ${schema.ir.k}`);
...
const enumSchema = <const Values extends readonly [string, ...string[]]>(
	...
	if (values.length === 0) throw new Zero2AiTypeError("enum requires at least one value");
...
export function record<Key extends string, Value>(
	...
	if (!isStringKeyIR(keySchema.ir)) throw new Zero2AiTypeError("record keys must use a string schema");
