 ...
 * output, or an `Zero2AiErrors` on failure (`result instanceof type.errors`).
 */
import { compile, compileAllows } from "./compile";
import { type ErrorConfig, Zero2AiErrors, Zero2AiTypeError, TraversalError } from "./errors";
...
export interface NarrowContext {
	readonly path: readonly PropertyKey[];
	error(error: string | NarrowErrorInput): Zero2AiErrors;
	mustBe(expectation: string): false;
	reject(problem: string | NarrowErrorInput): Zero2AiErrors | false;
}
...
interface FluentMethods<t, i> {
	...
	filter(fn: (data: i, ctx: NarrowContext) => boolean | Zero2AiErrors): FluentType<t, i>;
	narrow<narrowed extends t>(fn: (data: t, ctx: NarrowContext) => data is narrowed): FluentType<narrowed, i>;
	narrow(fn: (data: t, ctx: NarrowContext) => boolean | Zero2AiErrors): FluentType<t, i>;
...
interface PipeMethod<t, i> {
	<r>(fn: (data: t, ctx: NarrowContext) => r): FluentType<Exclude<r, Zero2AiErrors>, i>;
...
class Ctx implements NarrowContext {
	...
	errors: Zero2AiErrors | undefined;
	readonly path: readonly PropertyKey[];
	#data: unknown;
	...
	error(input: string | NarrowErrorInput): Zero2AiErrors {
		const detail = typeof input === "string" ? { expected: input } : input;
		const error = Zero2AiErrors.single([...(detail.path ?? detail.relativePath ?? [])], detail.expected, this.#data, {
			preserveActual: true,
	...
	reject(input: string | NarrowErrorInput): Zero2AiErrors | false {
...
const kBase = Symbol("schema.base");
const kSteps = Symbol("schema.steps");
...
function invalidDefault(label: string, errors: Zero2AiErrors): never {
	...
	throw new Zero2AiTypeError(`ParseError: ${heading} ${error.problem}`);
}

function rejectMutableStaticDefault(value: unknown): void {
	if (value !== null && typeof value === "object" && !(value instanceof Date)) {
		throw new Zero2AiTypeError("ParseError: A mutable default value must be specified as a factory");
	}
}
	...
	switch (ir.k) {
		case "object":
			for (const prop of ir.props) {
				...
				if (output instanceof Zero2AiErrors) invalidDefault(`Default for ${String(prop.key)}`, output);
		...
		case "tuple":
			for (let index = 0; index < ir.prefix.length; index++) {
				...
				if (output instanceof Zero2AiErrors) invalidDefault(`Default for [${index}]`, output);
...
const typeMethods = {
	...
	default(this: InternalType, value: unknown): InternalType {
		...
		if (output instanceof Zero2AiErrors) invalidDefault("Default", output);
	...
	overlaps(this: InternalType, def: Def): boolean {
		try {
			...
			if (error instanceof Zero2AiTypeError) return false;
			throw error;
		}
	},
	...
	divisibleBy(this: InternalType, divisor: number): InternalType {
		if (this.ir.k !== "number") throw new Zero2AiTypeError(`cannot apply divisibility to ${this.ir.k}`);
		if (!Number.isFinite(divisor) || divisor === 0) throw new Zero2AiTypeError("divisor must be non-zero");
	...
	allows(this: InternalType, data: unknown): data is unknown {
		...
		if (out instanceof Zero2AiErrors) return false;
		for (const step of steps) {
	...
	assert(this: InternalType, data: unknown): unknown {
		...
		if (out instanceof Zero2AiErrors) throw new TraversalError(out);
		return out;
	},
	...
	get(this: InternalType): StandardSchemaV1.Props {
		...
		return {
			...
			validate: (value: unknown): StandardSchemaV1.Result<unknown> => {
				...
				return out instanceof Zero2AiErrors
		...
		steps.length === 0
			...
			: (data: unknown, contextPath: readonly PropertyKey[] = []): unknown => {
					if (filterInput !== undefined) {
						...
						if (inputResult instanceof Zero2AiErrors) return inputResult;
					}
					for (const step of steps) {
						if (step.kind !== "filter") continue;
						const ctx = new Ctx(data, contextPath);
						const result = step.fn(data, ctx);
						if (result instanceof Zero2AiErrors)
					...
					let out = base(data);
					if (out instanceof Zero2AiErrors) return out;
					for (const step of steps) {
						if (step.kind === "filter") continue;
						...
						if (step.kind === "narrow") {
							...
							try {
								...
								return Zero2AiErrors.single([], `valid (morph threw ${detail})`, out, errorConfig);
							}
							if (out instanceof Zero2AiErrors) {
...
function appendPipes(
	...
	for (const candidate of pipes) {
		...
		if (typeof candidate !== "function") throw new Zero2AiTypeError("pipe operands must be functions or Types");
		schema = makeType(
			schema.ir,
		...
		for (let leftIndex = 0; leftIndex < ir.members.length; leftIndex++) {
			...
			for (let rightIndex = leftIndex + 1; rightIndex < ir.members.length; rightIndex++) {
				...
				try {
					...
					if (error instanceof Zero2AiTypeError) continue;
					throw error;
				}
				throw new Zero2AiTypeError("an unordered union with overlapping morph inputs is indeterminate");
			}
		}
	}
	switch (ir.k) {
...
function getPathIR(ir: IR, requestedKey: unknown): IR {
	...
	if ((typeof key === "function" || (typeof key === "object" && key !== null)) && IR_BRAND in key) {
		...
		if (keySchema.ir.k === "symbol") key = Symbol.for("schema.index");
		else {
	...
	if (typeof key !== "string" && typeof key !== "number" && typeof key !== "symbol") {
		throw new Zero2AiTypeError(`get keys must be strings, numbers, or symbols`);
	}
	...
	if (ir.k === "array") {
		...
		if (!Number.isSafeInteger(index) || index < 0) throw new Zero2AiTypeError(`key ${String(key)} is not declared`);
	...
	if (ir.k === "tuple") {
		...
		if (ir.variadic === undefined) throw new Zero2AiTypeError(`key ${String(key)} is not declared`);
		return unionOf([{ k: "undefined" }, ir.variadic, ...ir.postfix]);
	}
	if (ir.k === "undefined") return ir;
	if (ir.k !== "object") throw new Zero2AiTypeError("get requires an object schema");
	...
	if (typeof key === "string") {
		...
		for (const index of ir.patternIndexes ?? []) {
			if (!(walk(index.key, key) instanceof Zero2AiErrors)) matches.push(index.val);
		}
	} else if (typeof key === "symbol" && ir.symbolIndex !== undefined) {
		matches.push(ir.symbolIndex);
	}
	if (matches.length === 0) throw new Zero2AiTypeError(`key ${String(key)} is not declared`);
...
function requireObject(ir: IR, operation: string): ObjectIR {
	if (ir.k !== "object") throw new Zero2AiTypeError(`${operation} requires an object schema`);
	return ir;
}
function selectObjectProps(ir: IR, keys: readonly PropertyKey[], keepSelected: boolean, operation: string): ObjectIR {
	...
	for (const key of selected) {
		if (!object.props.some(prop => prop.key === key)) throw new Zero2AiTypeError(`key ${String(key)} does not exist`);
	}
...
function propertyToIR(property: TypeProperty): PropIR {
	if (property.kind !== "required" && property.kind !== "optional") {
		throw new Zero2AiTypeError(`mapped property ${String(property.key)} has invalid kind`);
	}
	if (!(IR_BRAND in property.value)) {
		throw new Zero2AiTypeError(`mapped property ${String(property.key)} must contain a schema value`);
	}
...
function dateRefinement(
	...
	if (!Number.isFinite(timestamp)) throw new Zero2AiTypeError("date bound must be valid");
	if (!acceptsDateIR(schema.ir)) throw new Zero2AiTypeError("date bounds require a Date type");
...
function withShallowExtras(ir: IR, extras: ObjectIR["extras"]): IR {
	...
	throw new Zero2AiTypeError("onUndeclaredKey requires an object schema");
}
...
function intersectTuples(left: TupleIR, right: TupleIR): IR {
	...
	if (minimum > maximum) throw new Zero2AiTypeError("tuple length intersection is unsatisfiable");
	...
	for (let index = 0; index < prefixLength; index++) {
		...
		try {
			...
			if (required || !(error instanceof Zero2AiTypeError)) throw error;
			break;
		}
	}
	const variadic =
...
const kIntersections = Symbol("schema.intersections");
...
function intersectResolved(a: IR, b: IR): IR {
	if (a.k === "never" || b.k === "never") throw new Zero2AiTypeError("intersection with never is unsatisfiable");
	if (a.k === "unknown") return b;
	if (b.k === "unknown") return a;
	if (a === b) return a;
	if (a.k === "morph" && b.k === "morph") {
		if (a.fn !== b.fn || a.out !== b.out) {
			throw new Zero2AiTypeError("intersection of distinct morphs is indeterminate");
		}
	...
	if (a.k === "union" || b.k === "union") {
		...
		if (union === undefined) throw new Zero2AiTypeError("union intersection invariant failed");
		...
		for (const branch of branches) {
			try {
				...
				if (!(error instanceof Zero2AiTypeError)) throw error;
			}
		}
		if (members.length === 0) throw new Zero2AiTypeError("intersection has no satisfiable branches");
		return members.length === 1 ? members[0] : { k: "union", members };
	}
	if (a.k === "lit") {
		if (walk(b, a.v) instanceof Zero2AiErrors) throw new Zero2AiTypeError("literal is excluded by the intersection");
		return a;
	}
	...
	if (a.k === "string" && b.k === "string") {
		...
		if (min !== undefined && max !== undefined && min > max) {
			throw new Zero2AiTypeError("string length intersection is unsatisfiable");
		}
	...
	if (a.k === "number" && b.k === "number") {
		...
		if (min !== undefined && max !== undefined && (min > max || (min === max && (xmin || xmax)))) {
			throw new Zero2AiTypeError("numeric range intersection is unsatisfiable");
		}
	...
	if (a.k === "array" && b.k === "array") {
		...
		const max = minOf(a.max, b.max);
		if (min !== undefined && max !== undefined && min > max) {
			throw new Zero2AiTypeError("array length intersection is unsatisfiable");
		}
	...
	if (a.k === "instance" && b.k === "instance") {
		...
		throw new Zero2AiTypeError(`intersection of ${a.expected} and ${b.expected} is unsatisfiable`);
	}
	...
	if (leftDomain !== undefined && rightDomain !== undefined && leftDomain !== rightDomain) {
		throw new Zero2AiTypeError(`intersection of ${leftDomain} and ${rightDomain} is unsatisfiable`);
	}
...
function isSubtype(source: IR, target: IR, seen = new WeakMap<IR, Set<IR>>()): boolean {
	...
	if (source.k === "lit") return !(walk(target, source.v) instanceof Zero2AiErrors);
...
function withLengthBound(ir: IR, side: "min" | "max", bound: number): IR {
	...
	throw new Zero2AiTypeError(`cannot apply length bound to ${ir.k}`);
}

function withNumericBound(ir: IR, side: "min" | "max", bound: number, exclusive = false): IR {
	if (!Number.isFinite(bound)) throw new Zero2AiTypeError("numeric bound must be finite");
	if (ir.k === "number") {
	...
	throw new Zero2AiTypeError(`cannot apply numeric bound to ${ir.k}`);
}
...
const GENERIC_META = Symbol("schema.generic");
...
function validateGenericParameters(parameters: readonly GenericParameter[]): void {
	...
	for (const parameter of parameters) {
		if (!/^[A-Za-z_$]\w*$/.test(parameter.name)) {
			throw new Zero2AiTypeError(`invalid generic parameter "${parameter.name}"`);
		}
		if (names.has(parameter.name)) throw new Zero2AiTypeError(`duplicate generic parameter "${parameter.name}"`);
		names.add(parameter.name);
	}
	if (parameters.length === 0) throw new Zero2AiTypeError("generic declarations require at least one parameter");
}
...
function parseGenericArgument(definition: unknown, outer?: AliasResolver): IR {
	if (outer === undefined) {
		try {
			...
			if (!(error instanceof Zero2AiTypeError) || !error.message.includes('unknown keyword "this"')) throw error;
		}
	}
	...
	const self: IR = {
		...
		resolve: () => {
			if (root.current === undefined || root.current === self)
				throw new Zero2AiTypeError('"this" cannot be used as a root definition');
			return root.current;
		},
	};
	...
	if (root.current === self) throw new Zero2AiTypeError('"this" cannot be used as a root definition');
	return root.current;
}
		...
		instantiateIR(arguments_) {
			...
			for (let index = 0; index < parameters.length; index++) {
				...
				if (!isSubtype(arguments_[index], constraint)) {
					throw new Zero2AiTypeError(`${parameter.name} must be assignable to its constraint`);
				}
			}
...
): FluentType<Exclude<output, Zero2AiErrors>, input>;
...
function keywordSchema<output = string, input = output>(name: string): FluentType<output, input> {
	...
	if (ir === undefined) throw new Zero2AiTypeError(`missing built-in keyword ${name}`);
...
export type Matcher<input = unknown, output = unknown> = FluentType<output, input> &
	(<const value extends input>(value: value, ...args: readonly unknown[]) => output | Zero2AiErrors);
...
function caseResolver(value: unknown): (input: unknown, ...args: readonly unknown[]) => unknown {
	if (typeof value !== "function") throw new Zero2AiTypeError("match case values must be functions");
...
function publicMatcher<input, output>(
	...
	if (state.key === undefined || state.branches.length === 0) {
		...
		if (first.k !== "object") throw new Zero2AiTypeError("match.at cases must define object schemas");
		const propertyKey = String(state.key);
		const values: IR[] = [];
		for (const branch of state.branches) {
			if (branch.schema.ir.k !== "object") throw new Zero2AiTypeError("match.at cases must define object schemas");
			const property = branch.schema.ir.props.find(candidate => candidate.key === propertyKey);
			if (property === undefined) throw new Zero2AiTypeError(`match.at case is missing ${propertyKey}`);
	...
	const execute = (input: unknown, args: readonly unknown[]): unknown => {
		...
		if (state.input !== undefined) {
			...
			if (validated instanceof Zero2AiErrors) return validated;
			matchedInput = validated;
		}
		for (const branch of state.branches) {
			const matched = branch.schema.run(matchedInput);
			if (!(matched instanceof Zero2AiErrors)) return branch.resolve(matched, ...args);
		}
	...
	const callable = ((value: unknown, ...args: readonly unknown[]): unknown => {
		...
		if ((fallback === "assert" || fallback === "never") && result instanceof Zero2AiErrors) {
	...
	for (const rawDefinition of Reflect.ownKeys(cases)) {
		...
		if (rawDefinition === "default") {
			if (value !== "assert" && value !== "never" && value !== "reject" && typeof value !== "function") {
				throw new Zero2AiTypeError('match default must be "assert", "never", "reject" or a function');
			}
...
function createMatchParser(state: MatchState): MatchParser {
	...
	function at(key: PropertyKey, cases?: Record<PropertyKey, unknown>): unknown {
		if (state.key !== undefined) throw new Zero2AiTypeError("match.at may only be specified once");
		const next = createMatchParser({ ...state, key });
		return cases === undefined ? next : next.match(cases);
	}
	parser.at = at;
	parser.strings = cases => {
		if (state.key === undefined) throw new Zero2AiTypeError("match.strings requires match.at(key)");
	...
	parser.in = (...args: unknown[]) => {
		if (state.input !== undefined) throw new Zero2AiTypeError("match.in may only be specified once");
		return createMatchParser({
			...state,
	...
	function parser(...definitions: readonly unknown[]): unknown {
		...
		if (spreadIndexes.length > 1) {
			...
			if (
				...
				throw new Zero2AiTypeError("An optional element may not follow a variadic element");
			}
			throw new Zero2AiTypeError("A tuple may have at most one variadic element");
		}
		if (spreadIndexes.length === 1 && spreadIndexes[0] + 2 < (marker === -1 ? definitions.length : marker)) {
			...
			if (
				...
				throw new Zero2AiTypeError("A postfix required element cannot follow an optional or defaultable element");
			}
		}
...
function requireNaryObject(ir: IR): ObjectIR {
	...
	if (resolved.k !== "object") throw new Zero2AiTypeError("merge requires an object schema");
	return resolved;
}
...
export namespace type {
	...
	export const errors = Zero2AiErrors;
	export type errors = Zero2AiErrors;
	...
	export const keywords = {
		...
		> {
			...
			if (keyIR.k !== "string" && keyIR.k !== "symbol") {
				throw new Zero2AiTypeError("Record key must be assignable to string or symbol");
			}
	...
	export function instanceOf<const ctor extends Constructor>(ctor: ctor): FluentType<Constructed<ctor>> {
		if (typeof ctor !== "function" || ctor.prototype === undefined) {
			throw new Zero2AiTypeError("instanceof operands must be constructors");
		}
	...
	export function generic(...arguments_: readonly (GenericParameterSpec | unknown)[]): Generic | GenericBuilder {
		...
		const parameters: GenericParameter[] = arguments_.map(parameter => {
			...
			throw new Zero2AiTypeError("generic parameters must be names or [name, constraint] pairs");
		});
		validateGenericParameters(parameters);
	...
	export function withJsonSchema<t, i = t>(schema: Type<t, i>, json: Record<string, unknown>): Type<t, i> {
		...
		if (internal.hasDefault || hasMorph(internal.ir) || internal[kSteps].some(step => step.kind === "pipe")) {
			throw new Zero2AiTypeError("type.withJsonSchema cannot wrap schemas with defaults or output-changing morphs");
		}
		return makeType<t, i>(
			{
				k: "refine",
				...
				pred: value => {
					...
					return result instanceof Zero2AiErrors ? result : true;
				},
...
const MODULE_SCOPE = Symbol("schema.moduleScope");
...
function buildScope(aliases: Record<string, unknown>, options?: ScopeOptions): TypeScope {
	...
	for (const sourceName in aliases) {
		...
		if (entries.has(name)) throw new Zero2AiTypeError(`alias "${name}" is declared as both public and private`);
		entries.set(name, {
			name,
			sourceName,
			private: isPrivate,
	...
	const genericFor = (entry: ScopeAlias): RuntimeGeneric => {
		...
		if (parameters === undefined) throw new Zero2AiTypeError(`alias "${entry.name}" is not generic`);
	...
	const targetFor = (name: string): IR => {
		...
		if (resolved === undefined) throw new Zero2AiTypeError(`unknown alias "${name}"`);
		...
		import(...names: readonly string[]) {
			const selected =
				names.length === 0
					...
					: names.map(name => {
							const entry = entries.get(name);
							if (entry === undefined) throw new Zero2AiTypeError(`unknown alias "${name}"`);
							return entry;
						});
...
/**
 * Minimal structural constraint matching any schema schema.
 *
