/**
 * JSON Schema → schema importer.
 *
 * Rebuilds a callable schema from JSON Schema documents — the inverse of
 * `Type.toJsonSchema()` for the dialect schema (and its Zod/TypeBox
 ...
 * schema IR and passes the rest through.
 */
...
class Importer {
	...
	resolveRef(ref: string): IR {
		...
		if (ref === "#") {
			...
			if (defsMatch === null) throw new Zero2AiTypeError(`unsupported $ref: ${ref}`);
			const defs = this.#root[defsMatch[1]];
			target = typeof defs === "object" && defs !== null ? (defs as JsonSchema)[defsMatch[2]] : undefined;
			if (target === undefined) throw new Zero2AiTypeError(`unresolved $ref: ${ref}`);
		}
	...
	lower(schema: unknown): IR {
		...
		if (typeof schema !== "object" || schema === null) {
			throw new Zero2AiTypeError("JSON Schema nodes must be booleans or objects");
		}
	...
	#lowerTyped(node: JsonSchema, kind: string): IR {
		switch (kind) {
			...
			default:
				throw new Zero2AiTypeError(`unsupported JSON Schema type: ${kind}`);
		}
	}
...
/**
 * Build a callable schema schema from a JSON Schema document.
 *
 * # Errors
 * Throws {@link Zero2AiTypeError} on malformed nodes, unresolvable `$ref`s, or
 * types schema cannot represent.
 */
