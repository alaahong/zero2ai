 ...
 * - failure allocates a single `Zero2AiErrors` (`E(path, expected, data)`); path
...
class CompiledMorphContext implements MorphContext {
	...
	error(expectation: string): Zero2AiErrors {
		return new Zero2AiErrors(this.#path, expectation, this.#data);
	}
...
class Builder {
	...
	buildAllows(ir: IR): (value: unknown) => value is unknown {
		...
		return make(this.#refs, Zero2AiErrors, own);
	}
}

function prefixErrors(errs: Zero2AiErrors, parts: PropertyKey[]): Zero2AiErrors {
	for (let i = parts.length - 1; i >= 0; i--) errs.prefix(parts[i]);
	return errs;
}

const kWalk = Symbol("schema.boundWalk");
...
export function compileAllows(ir: IR): (value: unknown) => value is unknown {
	...
	if (validator === undefined) {
		...
		allowsCache.set(root, ((value: unknown) =>
			built.value === undefined ? !(walk(root, value) instanceof Zero2AiErrors) : built.value(value)) as (
			value: unknown,
		) => value is unknown);
