import { Zero2AiTypeError } from "./errors";
import type { Def, EmbeddableSchema } from "./ir";
import { type NarrowContext, type Type as Zero2AiType, type ToJsonSchemaOptions, type } from "./type";
...
const OPTIONAL_INNER = Symbol("schema.typebox.optionalInner");
const OBJECT_INFO = Symbol("schema.typebox.objectInfo");
 ...
 * schema schemas included (those carry no legacy compat members).
 *
...
export type TTyped<T> = Zero2AiType<T> & LegacyTypeBoxCompat<T>;
...
interface RuntimeType<T> extends Zero2AiType<T> {
...
function withLegacyCompat<T>(schema: Zero2AiType<T>): CompatRuntime<T> {
...
function checkFiniteOption(name: string, value: number | undefined): void {
	if (value !== undefined && !Number.isFinite(value)) throw new Zero2AiTypeError(`${name} must be finite`);
}
...
function tString(opts?: StringOpts): TString {
	...
	if (opts?.pattern !== undefined) {
		...
		try {
			...
			throw new Zero2AiTypeError(`invalid regular expression pattern ${JSON.stringify(opts.pattern)}`);
		}
...
function tNumber(opts?: NumberOpts, integer = false): TNumber {
	...
	if (opts?.multipleOf !== undefined && opts.multipleOf <= 0)
		throw new Zero2AiTypeError("multipleOf must be greater than zero");
...
function tIntersect<const E extends readonly AnySchema[]>(
	...
	try {
		...
		if (error instanceof Zero2AiTypeError) return applyMeta(validateAll(), opts);
		throw error;
	}
...
function tObject<const P extends Record<string, AnySchema>>(properties: P, opts?: ObjectOpts): TObject<P> {
	...
	for (const key in properties) {
		...
		// schema (like ArkType) rejects `key?` with a default, and a default
...
function requireObject(schema: AnySchema, operation: string): ObjectInfo {
	...
	if (!info) throw new Zero2AiTypeError(`Type.${operation} requires a schema created by Type.Object`);
	return info;
}
...
function tUnsafe<T = unknown>(_jsonSchema: Record<string, unknown> = {}): TUnsafe<T> {
	...
	// schema cannot honestly implement that contract without importing a second validator.
	return withLegacyCompat(type.unknown as Zero2AiType<T>);
}
