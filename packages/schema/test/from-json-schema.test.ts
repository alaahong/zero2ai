...
import { fromJsonSchema, Zero2AiErrors, type } from "../src";
...
describe("fromJsonSchema", () => {
	it("round-trips schemas emitted by toJsonSchema", () => {
		...
		expect(imported({ name: "job", mode: "delete", tags: [] })).toBeInstanceOf(Zero2AiErrors);
		expect(imported({ name: "", mode: "read", tags: [] })).toBeInstanceOf(Zero2AiErrors);
		expect(imported({ mode: "read", tags: [] })).toBeInstanceOf(Zero2AiErrors);
		expect(imported({ name: "job", mode: "read", tags: [1] })).toBeInstanceOf(Zero2AiErrors);
	});
	...
	it("treats defaulted properties as fillable, matching emitted required semantics", () => {
		...
		expect(imported({})).toBeInstanceOf(Zero2AiErrors);
	});

	it("supports enum, const, unions, intersections, and boolean schemas", () => {
		expect(fromJsonSchema({ enum: ["a", "b"] })("a")).toBe("a");
		expect(fromJsonSchema({ enum: ["a", "b"] })("c")).toBeInstanceOf(Zero2AiErrors);
		...
		expect(bounded(4)).toBeInstanceOf(Zero2AiErrors);
		expect(fromJsonSchema(true)("anything")).toBe("anything");
		expect(fromJsonSchema(false)("anything")).toBeInstanceOf(Zero2AiErrors);
	});
	...
	it("validates string formats and patterns", () => {
		...
		expect(email("nope")).toBeInstanceOf(Zero2AiErrors);
		const patterned = fromJsonSchema({ type: "string", pattern: "^G-[A-Z0-9]+$", minLength: 3 });
		expect(patterned("G-X1")).toBe("G-X1");
		expect(patterned("bad")).toBeInstanceOf(Zero2AiErrors);
		expect(patterned("G-")).toBeInstanceOf(Zero2AiErrors);
	});
	...
	it("imports tuples with variadic tails and rejects undeclared keys on closed objects", () => {
		...
		expect(pair([1])).toBeInstanceOf(Zero2AiErrors);
		...
		expect(closed({ a: "x", b: 1 })).toBeInstanceOf(Zero2AiErrors);
	});
	...
	it("emits recursive scopes as $defs/$ref and round-trips them", () => {
		...
		expect(imported({ value: 1, next: { value: "two" } })).toBeInstanceOf(Zero2AiErrors);
	...
	it("emits io-aware schemas: input keeps morph sources and optional defaults, output the reverse", () => {
		...
		expect(imported({ raw: 42 })).toBeInstanceOf(Zero2AiErrors);
	});
	...
	it("io output uses the .to() target for piped schemas", () => {
		...
		expect(Uid(0)).toBeInstanceOf(Zero2AiErrors);
