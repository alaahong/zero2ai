...
import { Zero2AiErrors, Type, type } from "../src";
...
function normalized(result: unknown): unknown {
	return result instanceof Zero2AiErrors ? { error: result.summary } : result;
}
...
describe("validation", () => {
	...
	it("rejects wrong primitive, bad literal, broken bound, and missing key with path-aware errors", () => {
		...
		if (missing instanceof Zero2AiErrors) {
			expect(missing[0].path).toEqual(["path"]);
			expect(missing.summary).toContain("path");
		}
		expect(tool({ path: "x", mode: "delete" })).toBeInstanceOf(Zero2AiErrors);
		expect(tool({ path: "x", mode: "read", offset: 0 })).toBeInstanceOf(Zero2AiErrors);
		expect(tool({ path: "x", mode: "read", offset: 1.5 })).toBeInstanceOf(Zero2AiErrors);
		expect(tool({ path: "x", mode: "read", tags: ["a", 1] })).toBeInstanceOf(Zero2AiErrors);
	});
	...
	it("bounds: numbers (exclusive/inclusive) and string length", () => {
		...
		expect(s({ t: 0, name: "ok" })).toBeInstanceOf(Zero2AiErrors);
		expect(s({ t: 3601, name: "ok" })).toBeInstanceOf(Zero2AiErrors);
		expect(s({ t: 1, name: "" })).toBeInstanceOf(Zero2AiErrors);
		expect(s({ t: 1, name: "toolongname" })).toBeInstanceOf(Zero2AiErrors);
	});
	...
	it("array suffix binds outside a trailing bound: string>0[]", () => {
		...
		expect(s({ files: [""] })).toBeInstanceOf(Zero2AiErrors);
		expect(s({ files: "" })).toBeInstanceOf(Zero2AiErrors);
	});
	...
	it("null/undefined union members", () => {
		...
		expect(s("x")).toBe("x");
		expect(s(5)).toBeInstanceOf(Zero2AiErrors);
	});
});
...
describe("methods", () => {
	...
	it(".narrow() with ctx.mustBe records the expectation", () => {
		...
		expect(bad).toBeInstanceOf(Zero2AiErrors);
		if (bad instanceof Zero2AiErrors) expect(bad.summary).toContain("a body unless deleting");
	});

	it(".narrow() runs after morphs (sees defaults)", () => {
		const s = type({ n: "number = 5" }).narrow(v => v.n === 5);
		expect(s({})).toEqual({ n: 5 });
		expect(s({ n: 6 })).toBeInstanceOf(Zero2AiErrors);
	});
	...
	it(".and() merges object schemas", () => {
		...
		expect(s({ b: 2 })).toBeInstanceOf(Zero2AiErrors);
	});
	...
	it("type.enumerated builds literal unions from runtime arrays", () => {
		...
		expect(s("py")).toBe("py");
		expect(s("rs")).toBeInstanceOf(Zero2AiErrors);
	});
...
describe("embedded schemas", () => {
	it("stepped sub-schemas morph inside parents and prefix error paths", () => {
		...
		expect(bad).toBeInstanceOf(Zero2AiErrors);
		if (bad instanceof Zero2AiErrors) {
	...
	it("union failures discriminate object members by literal props", () => {
		...
		for (let i = 0; i < JIT; i++) {
			...
			expect(out).toBeInstanceOf(Zero2AiErrors);
...
describe("advanced ArkType compatibility", () => {
	it("re-exports recursive scopes through the Ark compatibility facade", () => {
		...
		expect(invalid).toBeInstanceOf(Zero2AiErrors);
		if (invalid instanceof Zero2AiErrors) expect(invalid[0].path).toEqual(["next", "value"]);
	});
	...
	it("resolves recursive scopes, modules, utility generics, and runtime generics", () => {
		...
		expect(bad).toBeInstanceOf(Zero2AiErrors);
		if (bad instanceof Zero2AiErrors) expect(bad[0].path).toEqual(["manager", "name"]);
		...
		expect(box("number")({ value: "2" })).toBeInstanceOf(Zero2AiErrors);
	});
	...
	it("supports fixed, optional, defaulted, and variadic tuple elements", () => {
		...
		expect(fixed(["x"])).toBeInstanceOf(Zero2AiErrors);

		const optional = type(["string", ["number", "?"]]);
		expect(optional(["x"])).toEqual(["x"]);
		expect(optional(["x", 1])).toEqual(["x", 1]);
		expect(optional(["x", 1, 2])).toBeInstanceOf(Zero2AiErrors);
		...
		expect(variadic(["x", true])).toBeInstanceOf(Zero2AiErrors);
	...
	it("validates Date literals and bounds before and after JIT compilation", () => {
		...
		for (let index = 0; index < JIT; index++) {
			...
			expect(exact(new Date("2024-01-03T00:00:00.000Z"))).toBeInstanceOf(Zero2AiErrors);
			expect(bounded(new Date("2024-01-01T00:00:00.000Z"))).toEqual(new Date("2024-01-01T00:00:00.000Z"));
			expect(bounded(new Date("2023-12-31T23:59:59.999Z"))).toBeInstanceOf(Zero2AiErrors);
		}
	});
	...
	it("supports disjointness-aware intersections and semantic comparison", () => {
		...
		expect(bounded(0)).toBeInstanceOf(Zero2AiErrors);
	...
	it("maps and selects structural nodes and distributes unions", () => {
		...
		expect(distributed(["c"])).toBeInstanceOf(Zero2AiErrors);
	});
...
describe("Standard Schema V1", () => {
	...
	it("materializes root defaults for undefined input at the standard boundary", () => {
		...
		expect(staticDefault(5)).toBeInstanceOf(Zero2AiErrors);
...
describe("type.withJsonSchema", () => {
	it("emits the override verbatim even when embedded, and still validates", () => {
		...
		expect(inner("c")).toBeInstanceOf(Zero2AiErrors);
		expect(object({ mode: "b" })).toEqual({ mode: "b" });
		expect(object({ mode: "c" })).toBeInstanceOf(Zero2AiErrors);
	});
