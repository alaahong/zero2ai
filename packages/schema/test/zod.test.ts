...
import { Zero2AiTypeError } from "../src/errors";
...
describe("zod-like parsing", () => {
	it("exposes callable schema schemas with JSON Schema metadata", () => {
	...
	it("dispatches min and max by string, number, and array kind", () => {
		...
		expect(() => z.boolean().min(1)).toThrow(Zero2AiTypeError);
	});
	...
	it("validates records with string key and value schemas", () => {
		...
		expect(() => z.record(z.number() as never, z.string())).toThrow(Zero2AiTypeError);
