...
```sh
npm install @zero2ai/schema
# or
bun add @zero2ai/schema
```
...
Failed validation returns `Zero2AiErrors`; each entry exposes `code`, `path`,
...
TypeBox-style and Zod-style builders produce native schema schemas:
...
`@zero2ai/schema/ark` provides the repository's ArkType compatibility facade
...
```sh
bun packages/schema/bench/bench.ts
```
...
candidate was than schema in this run. Results vary with hardware, runtime,
