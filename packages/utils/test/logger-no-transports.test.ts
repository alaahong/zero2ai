...
beforeAll(() => {
	tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zero2ai-logger-no-transports-"));
...
describe("logger with no transports", () => {
	...
	it("resumes writing once a transport is re-enabled", async () => {
		...
		for (let i = 0; i < 40 && !found; i++) {
			for (const f of fs.readdirSync(tempDir).filter(n => n.startsWith("zero2ai.") && n.endsWith(".log"))) {
