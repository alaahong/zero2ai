...
/** Run `fn` with ZERO2AI_DEBUG_STARTUP set, capturing `[startup]` stderr markers. */
function withMarkerCapture<T>(fn: () => T): { result: T; markers: string[] } {
	const prev = process.env.ZERO2AI_DEBUG_STARTUP;
	process.env.ZERO2AI_DEBUG_STARTUP = "1";
	...
	try {
		...
		if (prev === undefined) {
			delete process.env.ZERO2AI_DEBUG_STARTUP;
		} else {
			process.env.ZERO2AI_DEBUG_STARTUP = prev;
		}
	}
}

describe("ZERO2AI_DEBUG_STARTUP streaming markers", () => {
	// Contract: with ZERO2AI_DEBUG_STARTUP set, every logger.time phase leaves a
	...
	// must work without startTiming() (markers are independent of ZERO2AI_TIMING).
	...
	it("emits nothing when ZERO2AI_DEBUG_STARTUP is unset", () => {
		const prev = process.env.ZERO2AI_DEBUG_STARTUP;
		delete process.env.ZERO2AI_DEBUG_STARTUP;
		const writes: string[] = [];
		const writeSpy = spyOn(fs, "writeSync").mockImplementation(((_fd: number, data: string) => {
		...
		try {
			...
			writeSpy.mockRestore();
			if (prev !== undefined) process.env.ZERO2AI_DEBUG_STARTUP = prev;
		}
