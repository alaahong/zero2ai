...
describe("getInstallId", () => {
	...
	beforeEach(async () => {
		...
		const slug = `zero2ai-install-id-${Snowflake.next()}`;
		tempRoot = path.join(os.tmpdir(), slug);
		await fs.mkdir(tempRoot, { recursive: true });
		// Point the resolver's config root at the temp dir. Using ZERO2AI_CONFIG_DIR
		// keeps the parent equal to os.homedir() but flips the basename, so the
		// install-id file lands inside our temp tree.
		process.env.ZERO2AI_CONFIG_DIR = path.relative(os.homedir(), tempRoot);
