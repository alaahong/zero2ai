...
import { __resetExtraCaCache, ExtraCaError, type FetchImpl, wrapFetchForExtraCa } from "@zero2ai/utils/tls-fetch";
...
describe("wrapFetchForExtraCa", () => {
	...
	beforeEach(async () => {
		...
		tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zero2ai-extra-ca-"));
	});
