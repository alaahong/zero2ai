#!/usr/bin/env bun
//
// Render the Homebrew formula for `zero2ai` from a published GitHub release and write
// it to a tap checkout. The release publishes per-platform bare binaries
// (zero2ai-<platform>-<arch>); this reads their sha256 digests straight from the
// release metadata so the formula never drifts from the shipped assets.
//
// Usage:
//   bun scripts/ci-update-brew-formula.ts <tag> --out <path/to/Formula/zero2ai.rb>
...
export function renderFormula(version: string, sums: Record<string, string>): string {
	...
	// which nests the file outside the staging CWD; `Dir["zero2ai-*"].first` then
	// returns `nil` and `bin.install nil => "zero2ai"` raises.
	//
	...
	// not touch the real `/Users/<user>/.zero2ai` (denied by Homebrew's sandbox
	// profile, which would otherwise fail the popen).
	return `class Zero2AI < Formula
  desc "${DESC}"
  homepage "${HOMEPAGE}"
  version "${version}"
  license "MIT"
  ...
  on_macos do
    on_arm do
      url "https://github.com/${REPO}/releases/download/v#{version}/zero2ai-darwin-arm64",
          using: :nounzip
      sha256 "${sums["zero2ai-darwin-arm64"]}"
    end
    on_intel do
      url "https://github.com/${REPO}/releases/download/v#{version}/zero2ai-darwin-x64",
          using: :nounzip
      sha256 "${sums["zero2ai-darwin-x64"]}"
    end
  end

  on_linux do
    on_arm do
      url "https://github.com/${REPO}/releases/download/v#{version}/zero2ai-linux-arm64",
          using: :nounzip
      sha256 "${sums["zero2ai-linux-arm64"]}"
    end
    on_intel do
      url "https://github.com/${REPO}/releases/download/v#{version}/zero2ai-linux-x64",
          using: :nounzip
      sha256 "${sums["zero2ai-linux-x64"]}"
    end
  end

  def install
    bin.install Dir["zero2ai-*"].first => "zero2ai"
    (bin/"zero2ai").chmod 0555
    with_env(HOME: buildpath) do
      generate_completions_from_executable(bin/"zero2ai", "completions", shells: [:bash, :zsh, :fish])
    end
  end

  test do
    assert_match version.to_s, shell_output("#{bin}/zero2ai --version")
  end
end
`;
}
...
async function main(): Promise<void> {
	...
	const targets = ["zero2ai-darwin-arm64", "zero2ai-darwin-x64", "zero2ai-linux-arm64", "zero2ai-linux-x64"];
