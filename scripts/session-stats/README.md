...
(`~/.zero2ai/agent/sessions/`). SQLite-backed; data is synced once into the same
`~/.zero2ai/stats.db` that `packages/stats` uses, then queried by short Python
scripts.
...
scripts/session-stats/
  sync.py          # walks ~/.zero2ai/agent/sessions/ and populates ss_* tables
...
The LLM phase (default `anthropic/claude-sonnet-4-6` via `@zero2ai/ai`,
credentials resolved through zero2ai's auth storage — stored key, OAuth, or env
...
Verdicts are cached in `~/.zero2ai/stats-audit-cache.json` (keyed by session id +
