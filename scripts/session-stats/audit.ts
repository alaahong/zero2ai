#!/usr/bin/env bun
/**
 * Token-usage audit over the local zero2ai session corpus (~/.zero2ai/agent/sessions/).
 *
 ...
 * asks a small model (default: anthropic/claude-sonnet-4-6 via @zero2ai/ai)
 * to judge:
 ...
 *
 * Auth: resolves an API key for the classifier provider through zero2ai's auth
 * storage (~/.zero2ai/agent/agent.db: stored key, OAuth, or env var fallback).
 */
...
import { getAgentDbPath, isEnoent } from "@zero2ai/utils";
import SYSTEM_PROMPT from "./audit-prompt.md" with { type: "text" };

const SESSIONS_ROOT = path.join(os.homedir(), ".zero2ai", "agent", "sessions");
const DEFAULT_MODEL = "anthropic/claude-sonnet-4-6";
const CACHE_PATH = path.join(os.homedir(), ".zero2ai", "stats-audit-cache.json");
...
function parseCli(argv: string[]): CliOptions {
	...
	if (values.help) {
		console.log(
			`session audit — token usage analysis over ~/.zero2ai/agent/sessions\n\n` +
				...
				`  --no-cache               skip the verdict cache (~/.zero2ai/stats-audit-cache.json)\n` +
...
async function openClassifier(modelSpec: string): Promise<Classifier> {
	...
	if (!apiKey) {
		throw new Error(`no credentials for provider "${provider}" (zero2ai login or env var required)`);
	}
	return { model, apiKey };
}
...
/** Narrow view of zero2ai-ai's AssistantMessage used here (content + usage). */
