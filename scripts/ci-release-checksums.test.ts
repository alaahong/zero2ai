...
describe("formatChecksums", () => {
	it("sorts entries by basename and formats as `sha256sum -c`-compatible lines", () => {
		const output = formatChecksums([
			{ name: "zero2ai-linux-x64", sha256: "b".repeat(64) },
			{ name: "zero2ai-darwin-arm64", sha256: "a".repeat(64) },
		]);
		expect(output).toBe(`${"a".repeat(64)}  zero2ai-darwin-arm64\n${"b".repeat(64)}  zero2ai-linux-x64\n`);
	});

	it("hashes assets and writes a sorted checksum manifest", async () => {
		const dir = await mkdtemp(path.join(tmpdir(), "zero2ai-release-checksums-"));
		tempDirs.push(dir);
		const aPath = path.join(dir, "zero2ai-a");
		const zPath = path.join(dir, "zero2ai-z");
		...
		expect(await readFile(outPath, "utf8")).toBe(
			"ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad  zero2ai-a\n" +
				"e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855  zero2ai-z\n",
		);
	});
