...
const SUMS = {
	"zero2ai-darwin-arm64": "darwin_arm64_sha",
	"zero2ai-darwin-x64": "darwin_x64_sha",
	"zero2ai-linux-arm64": "linux_arm64_sha",
	"zero2ai-linux-x64": "linux_x64_sha",
};
...
describe("renderFormula", () => {
	...
	// outside the staging CWD, `Dir["zero2ai-*"].first` returns `nil`, and
	// `bin.install nil => "zero2ai"` raises (issue #2398).
	it("attaches `using: :nounzip` to every per-platform url stanza", () => {
		const matches = formula.match(/using: :nounzip/g) ?? [];
		expect(matches).toHaveLength(4);
		for (const arch of ["zero2ai-darwin-arm64", "zero2ai-darwin-x64", "zero2ai-linux-arm64", "zero2ai-linux-x64"]) {
	...
	// popened binary doesn't touch the real `~/.zero2ai` (denied by Homebrew's
	...
	it("wraps `generate_completions_from_executable` with a HOME redirect to buildpath", () => {
		expect(formula).toMatch(
			/with_env\(HOME: buildpath\) do\n\s+generate_completions_from_executable\(bin\/"zero2ai", "completions", shells: \[:bash, :zsh, :fish\]\)\n\s+end/,
		);
