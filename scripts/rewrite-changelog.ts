 ...
 * to a small model (default `openai-codex/gpt-5.6-luna` via `@zero2ai/ai`),
 ...
 *
 * Auth: resolves the provider API key through zero2ai's auth storage
 * (~/.zero2ai/agent/agent.db: stored key, OAuth, or env var fallback).
 */

import * as path from "node:path";
import { parseArgs } from "node:util";
import { type } from "@zero2ai/schema";
import { type Api, completeSimple, Effort, type Model, type Tool, type ToolCall } from "@zero2ai/ai";
import { discoverAuthStorage } from "@zero2ai/ai/auth-broker";
...
async function openModel(modelSpec: string): Promise<RewriteModel> {
	...
	try {
		...
		if (!apiKey) {
			throw new Error(
				`no credentials for provider "${provider}" via ${storage.sourceLabel ?? "auth storage"} (check broker or run \`zero2ai login\`)`,
			);
		}
