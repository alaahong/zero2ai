 ...
 * `ZERO2AI_NATIVE_BUILD_BACKEND=bazel` or by passing extra bazel args after `--`;
...
async function main(): Promise<void> {
	...
	const backend = Bun.env.ZERO2AI_NATIVE_BUILD_BACKEND?.trim();
	if (backend && backend !== "cargo" && backend !== "bazel") {
		throw new Error(`Unknown ZERO2AI_NATIVE_BUILD_BACKEND "${backend}" (expected "cargo" or "bazel")`);
	}
	...
	// path; bazel is opt-in for host via ZERO2AI_NATIVE_BUILD_BACKEND=bazel or
	...
	if (cargoBackend && !options.source) {
		if (!hostOnly) {
			...
			throw new Error("ZERO2AI_NATIVE_BUILD_BACKEND=cargo supports only the host target");
		}
	...
	if (options.source) {
		...
		const rcPath = Bun.env.ZERO2AI_BAZEL_RC?.trim();
