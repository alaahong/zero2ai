...
describe("published legal payloads", () => {
	...
	it("stages missing legal files without replacing package-local text", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "zero2ai-publish-legal-"));
	...
	it("lists every legal file explicitly in the native core package", async () => {
		const pkgDir = await fs.mkdtemp(path.join(os.tmpdir(), "zero2ai-native-core-"));
		try {
			await Bun.write(
...
describe("published manifest topology", () => {
	it("repoints schema runtime entries to dist/js with a bun source condition", async () => {
		const pkg = packages.find(entry => entry.dir === "packages/schema");
		if (!pkg) throw new Error("schema missing from publish set");
