...
// The zero2ai-kata runner pods may inject cloud credentials (`AWS_*`) pod-wide via
...
// Override with ZERO2AI_TEST_CHUNK_TIMEOUT (seconds).
function chunkTimeoutMs(): number {
	const raw = Number(Bun.env.ZERO2AI_TEST_CHUNK_TIMEOUT?.trim());
...
// ZERO2AI_TEST_CHUNK_TIMEOUT" and "lower this bucket's chunkSize", so say it.
export function describeChunkFailure(exitCode: number, timedOut: boolean): string {
	if (timedOut) {
		return `exceeded the ${Math.round(chunkTimeoutMs() / 1000)}s chunk watchdog and was killed (exit ${exitCode}; ZERO2AI_TEST_CHUNK_TIMEOUT to change)`;
	}
	if (exitCode === 137) {
...
// Defaults to the machine's available parallelism; `ZERO2AI_TEST_CONCURRENCY`
...
function testConcurrency(total: number): number {
	const raw = Bun.env.ZERO2AI_TEST_CONCURRENCY?.trim().toLowerCase();
	...
	throw new Error(`Invalid ZERO2AI_TEST_CONCURRENCY=${JSON.stringify(raw)}; expected a positive integer, all, or max`);
}
...
// Override with ZERO2AI_TEST_TIMEOUT (seconds); per-test `it(name, fn, ms)` still wins.
function testTimeoutMs(): number {
	const raw = Number(Bun.env.ZERO2AI_TEST_TIMEOUT?.trim());
	if (Number.isFinite(raw) && raw >= 1) return raw * 1000;
	return 30_000;
}
...
export async function runTestCommandsInParallel(commands: TestCommand[], concurrency: number): Promise<void> {
	...
	console.log(
		`Running ${commands.length} test command(s), up to ${concurrency} in parallel ` +
			`(ZERO2AI_TEST_CONCURRENCY=<n>|all to change); ${os.availableParallelism()} cores, ` +
	...
	async function runAttempt(
		...
		return {
			exitCode,
			timedOut,
			output: `${stdout.text}${stderr.text}${timedOut ? `\n[watchdog] chunk exceeded ${Math.round(chunkTimeoutMs() / 1000)}s; killed with SIGKILL (ZERO2AI_TEST_CHUNK_TIMEOUT to change)\n` : ""}`,
		};
	}
...
if (import.meta.main) {
	...
	const explicitConcurrency = Boolean(Bun.env.ZERO2AI_TEST_CONCURRENCY?.trim());
