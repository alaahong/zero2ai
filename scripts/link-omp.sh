#!/bin/sh
# Install the dev `zero2ai` wrapper into Bun's global bin directory.
#
...
# `packages/coding-agent/scripts/zero2ai`. See that wrapper's header comment for the
...
# through that error would expand `$(bun pm -g bin)/zero2ai` to `/zero2ai` and try to
# write under `/` — see https://github.com/can1357/oh-my-pi/issues/3701.
set -e

repo_root=$(CDPATH='' cd -- "$(dirname -- "$0")/.." && pwd -P)
target=$repo_root/packages/coding-agent/scripts/zero2ai

if [ ! -x "$target" ]; then
	echo "link-zero2ai: target wrapper not found or not executable: $target" >&2
	exit 1
fi
...
mkdir -p "$global_bin"
ln -sfn "$target" "$global_bin/zero2ai"
echo "link-zero2ai: linked $global_bin/zero2ai -> $target"
