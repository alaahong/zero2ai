 ...
 * addon build → coding-agent link → zero2ai link). The native host build uses
 * the local Cargo/N-API backend by default; set
 * `ZERO2AI_NATIVE_BUILD_BACKEND=bazel` to opt into bazel. Flags after `--` are
...
const steps: Step[] = [
	...
	{ label: "link zero2ai", cmd: ["sh", "scripts/link-omp.sh"] },
];
