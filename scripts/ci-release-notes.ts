 ...
 * `ZERO2AI_RELEASE_NOTES_FLOOR=v15.12.4` to override (empty string forces
 * single-version mode, matching the pre-#2596 behavior). `ZERO2AI_REPO` /
...
const REPO = process.env.ZERO2AI_REPO ?? process.env.GITHUB_REPOSITORY ?? "can1357/oh-my-pi";
 ...
 *
 * Failure semantics:
 *   - `ZERO2AI_RELEASE_NOTES_FLOOR` set → honored verbatim (`""` forces null).
 ...
 *     Local runs without `gh` should set `ZERO2AI_RELEASE_NOTES_FLOOR=` to opt
 *     into legacy mode explicitly.
 */
async function resolvePublishedFloorTag(targetVersion: string): Promise<string | null> {
	const override = process.env.ZERO2AI_RELEASE_NOTES_FLOOR;
	...
	if (res.exitCode !== 0) {
		...
		throw new Error(
			`gh release list exited ${res.exitCode}.\nstderr: ${stderr || "(empty)"}\n` +
				...
				`Locally without gh, set ZERO2AI_RELEASE_NOTES_FLOOR= to fall back to single-version notes.`,
		);
	}
	let raw: unknown;
	try {
...
async function main(): Promise<void> {
	...
	if (/^\d+\.\d+\.\d+-canary\.\d+$/.test(version)) {
		...
		await Bun.write(
			outputPath,
			`This is a canary prerelease of ${upcomingVersion}. Install it with \`zero2ai update --canary\`.\n`,
		);
