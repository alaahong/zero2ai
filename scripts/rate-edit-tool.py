...
sys.path.insert(0, str(REPO_ROOT / "python/zero2ai-rpc/src"))

from zero2ai_rpc import (  # noqa: E402
    AgentEndEvent,
    AutoRetryEndEvent,
    AutoRetryStartEvent,
    ExtensionUiRequest,
...
def resolve_omp_bin(raw: str | None) -> str:
    if raw:
        return raw
    found = shutil.which("zero2ai")
    if not found:
        raise SystemExit("Could not find `zero2ai` on PATH. Set --zero2ai-bin or ZERO2AI_BIN.")
    return found
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run OpenRouter fixture evaluations through zero2ai RPC mode."
    )
    parser.add_argument("--zero2ai-bin", default=os.environ.get("ZERO2AI_BIN"))
...
async def run_all(args: argparse.Namespace) -> int:
    ...
    results_dir = (
        ...
        if args.results_dir
        else tmp_root / f"zero2ai-fixture-runs-{timestamp}"
    )
