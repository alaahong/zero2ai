...
`scripts/probe.ts`: `@zero2ai/ai` `completeSimple`; production-matching model/auth/provider behavior.

```bash
bun .zero2ai/skills/tool-prompt-optimization/scripts/probe.ts \
...
```bash
bun .zero2ai/skills/tool-prompt-optimization/scripts/probe-builtin.ts --tool <name> [--no-summary] [--show]
```
...
```ts
import { arkToWireSchema } from "@zero2ai/ai"; // or toolWireSchema(tool)
